...
def opt_fairness(opt_o: dict, empty_lines_end: int) -> str:
    """Text content for the fairness (protected variables) adjustment."""
    prot_vars = [*opt_o.var_cfg.protected_ord_name,
                 *opt_o.var_cfg.protected_unord_name
                 ]
    mat_vars = [*opt_o.var_cfg.material_ord_name,
                *opt_o.var_cfg.material_unord_name
                ]
    txt = (
        '\nBACKGROUND'
    ...
    if opt_o.fair_cfg.protected_disc_method == 'NoDiscretization':
        ...
        if opt_o.fair_cfg.protected_disc_method == 'EqualCell':
            txt += ('equal cell sizes for each discrete feature of not more '
                    f'than {opt_o.fair_cfg.protected_disc_method} '
                    'observations.'
                    )
        else:
            txt += ('k-means++ clustering with '
                    f'{opt_o.fair_cfg.protected_disc_method} clusters. '
                    )
    if mat_vars:
        txt += (
        ...
        if opt_o.fair_cfg.material_disc_method == 'NoDiscretization':
            ...
            if opt_o.fair_cfg.material_disc_method == 'EqualCell':
                txt += ('equal cell sizes for each discrete feature of not '
                        f'more than {opt_o.fair_cfg.material_disc_method} '
                        'observations.'
                        )
            else:
                txt += ('k-means++ clustering with '
                        f'{opt_o.fair_cfg.material_disc_method} clusters. '
                        )
...
def opt_estimationrisk(opt_o: dict, empty_lines_end: int) -> str:
    ...
    txt += ('\n\nIMPLEMENTATION '
            ...
            f'{", ".join(opt_o.var_cfg.polscore_se_name)}. '
...
def opt_evaluation(opt_o: dict) -> tuple[str, list[tuple[str, pd.DataFrame]]]:
...
def opt_training(opt_o: dict, empty_lines_end: int = 2) -> str:
    """Text training part of description."""
    txt = '\nCOMPUTATION'
    txt += (f'\n{opt_o.gen_cfg.mp_parallel} logical cores are used for '
            'processing.')
    ...
    if screen or percorr or clean or dummy:
        ...
        if dummy:
            txt += ('\nDummy variables with less than '
                    f'{opt_o.dc_cfg.min_dummy_obs} observations in the '
    ...
    if opt_o.gen_cfg.method in ('best_policy_score', 'bps_classifier'):
        ...
        if opt_o.other_cfg.restricted:
            ...
            if not all(cm_1):
                mult = [str(mul)
                        for mul in opt_o.other_cfg.costs_of_treat_mult]
        ...
        txt += (
            ...
            f'only {opt_o.pt_cfg.no_of_evalupoints} values are considered. '
        ...
        if opt_o.pt_cfg.depth_tree_2 > 2:
            txt += (
                ...
                f'tree of depth {opt_o.pt_cfg.depth_tree_1-1}, in each '
                'leaf of this tree, a second optimal tree of depth '
                f'{opt_o.pt_cfg.depth_tree_2-1} is built. Subsequently, '
                'these trees are combined to form the final tree of depth '
                f'{opt_o.pt_cfg.depth-1}. For given final tree depth, '
        ...
        if opt_o.pt_cfg.eva_cat_mult != 1:
            txt += (
                ...
                f'variables: {opt_o.pt_cfg.eva_cat_mult}'
                )
...
def opt_general(opt_o: dict, empty_lines_end: int) -> str:
    ...
    match method:
        ...
        case 'policy_tree' | 'policy tree old':
            ...
            if opt_o.gen_cfg.method == 'policy tree old':
                txt += ('\n   Older, less efficient method used for tree '
                        'building.'
                        )
        case 'bps_classifier':
