...
from mcf import mcf_cuda as mcf_c
...
def prepare_data_for_forest(mcf_: 'ModifiedCausalForest',
    ...
    if not var_cfg.x_name_always_in == []:
        x_ai_ind = np.array([i for i in range(p_x) if x_name[i] in var_cfg.x_name_always_in])
    else:
        x_ai_ind = []              # Indices of variables used for all splits
    if len(data_df.columns) > len(set(data_df.columns)):
        duplicates = [x for x in list(data_df.columns) if list(data_df.columns).count(x) > 1]
        raise RuntimeError(f'Duplicate variable names.{duplicates}'
                           ' Names are NOT case sensitive')
    y_dat = data_df[var_cfg.y_tree_name].to_numpy(copy=True)
    pen_mult = cf_cfg.p_diff_penalty * np.var(y_dat) if cf_cfg.mtot in (1, 4) else 0

    len_d_name = 1  # Only main treatment used
    len_y_match_name = len(var_cfg.y_match_name)

    y_i, d_i = 0, 1

    if mcf_.gen_tv_cfg.yes or (isinstance(var_cfg.d_name, (tuple, list)) and len_d_name > 1):
        d_dat = data_df[[var_cfg.d_name[0]]].to_numpy(copy=True)
    else:
        d_dat = data_df[var_cfg.d_name].to_numpy(copy=True)

    if mcf_.gen_cfg.d_type != 'continuous':
        check_forest_treatment_codes(d_dat,
                                     no_of_treat=mcf_.gen_cfg.no_of_treat,
                                     d_values=mcf_.gen_cfg.d_values,
                                     zero_tol=mcf_.int_cfg.zero_tol,
                                     )
    y_nn = (np.zeros((len(d_dat), len_y_match_name)) if no_y_nn
            else data_df[var_cfg.y_match_name].to_numpy()
            )
    y_nn_i = np.arange(d_i + len_d_name, d_i + len_d_name + len_y_match_name)
    x_dat = data_df[x_name].to_numpy(copy=True)

    # for col_indx in range(np.shape(x_dat)[1]):
    for col_indx in range(x_dat.shape[1]):
        if x_type[col_indx] > 0:
            x_dat[:, col_indx] = np.around(x_dat[:, col_indx])
    x_i = np.arange(
        d_i + len_d_name + len_y_match_name,
        d_i + len_d_name + len_y_match_name + len(var_cfg.x_name))
...
def m_n_grid(cf_cfg: 'CfCfg', no_vars: int) -> dict:
...
def nn_matched_outcomes(mcf_: 'ModifiedCausalForest',
    ...
    if len(data_df) > 50000:   # Otherwise ***_issue17 is faster
        data_new_df, mcf_.var_cfg = nn_matched_outcomes_072(mcf_, data_df, print_out)
    else:
        ...
        data_nn, txt = nn_matched_outcomes_issue_17(
            ...
            match_nn_prog_score=mcf_.cf_cfg.match_nn_prog_score,
            lc_yes=mcf_.lc_cfg.yes,
            d_name=d_name,
            d_values=d_values,
            no_of_treats=no_of_treat,
            mtot=mcf_.cf_cfg.mtot,
            boot=mcf_.cf_cfg.boot,
            mp_parallel=mcf_.gen_cfg.mp_parallel,
            nn_main_diag_only=mcf_.cf_cfg.nn_main_diag_only,
            zero_tol=mcf_.int_cfg.zero_tol,
            )
        data_new_df = pd.concat((data_df.reset_index(drop=True), data_nn.reset_index(drop=True)),
                                axis=1,
                                )
        ...
        if mcf_.gen_cfg.with_output:
            ...
            if print_out:
                ...
                mcf_ps.print_descriptive_df(mcf_.gen_cfg,
                                            data_new_df,
                ...
                if mcf_.int_cfg.descriptive_stats:
                    ...
                    if mcf_.gen_tv_cfg.yes or (isinstance(d_name, (tuple, list))
                                               and len(d_name) > 1):
                        d_name = [d_name[0]]
                    mcf_ps.statistics_by_treatment(mcf_.gen_cfg, data_new_df,
...
def nn_matched_outcomes_072(mcf_: 'ModifiedCausalForest',
    ...
    if gen_cfg.x_type_1 or gen_cfg.x_type_2:  # Expand cat var to dummy
        var_names_unordered = mcf_gp.dic_get_list_of_key_by_item(var_x_type, [1, 2])
        x_dummies = pd.get_dummies(data_df[var_names_unordered].astype('category'), dtype=int)
    else:
        x_dummies = None
    ...
    x_dat = x_df.to_numpy(copy=True)
    y_dat = data_df[var_cfg.y_tree_name].to_numpy(copy=True)
    if cf_cfg.match_nn_prog_score:
        y_dat_match = data_df[var_cfg.y_tree_name_unc].to_numpy() if lc_cfg.yes else y_dat
    else:
        y_dat_match = None
    d_name, d_values, no_of_treat = mcf_data.get_treat_info(mcf_)
    d_dat = data_df[d_name].to_numpy(copy=True)
    ...
    if (cf_cfg.match_nn_prog_score
        ...
        for midx, d_val in enumerate(d_values):
            ...
            d_m = np.isclose(d_dat, d_val, atol=int_cfg.zero_tol, rtol=int_cfg.zero_tol)
    ...
    if cf_cfg.mtot in [1, 4]:
        ...
        if k[1] > 1:
            ...
            cov_x_inv = get_inv_cov(x_dat, cf_cfg.nn_main_diag_only, cuda=False)
        else:
        ...
        if int_cfg.cuda and len(cov_x_inv) > gen_cfg.no_of_treat:
            ...
            if (isinstance(cov_x_inv, (int, float))
                ...
                if gen_cfg.mp_automatic:
                    maxworkers = mcf_gp_sys.find_no_of_workers(gen_cfg.mp_parallel,
                                                               gen_cfg.sys_share,
        ...
        if gen_cfg.with_output and gen_cfg.verbose:
            ...
            if maxworkers > 1:
                print_runtime_info(gen_cfg, int_cfg, maxworkers, txt_method='matching')
            else:
                txt = ''
            ...
            if txt != '':
                mcf_ps.print_mcf(gen_cfg, txt, summary=False)
        kw_args = {'obs': obs, 'zero_tol': int_cfg.zero_tol}
        if maxworkers == 1:
            for i_treat, i_value in enumerate(d_values):
                if cuda:
                    y_match[:, i_treat] = nn_neighbour_mcf2_cuda(
                        y_dat, x_dat, d_dat,
                        obs=obs, cov_x_inv_np=cov_x_inv, treat_value=i_value, i_treat=i_treat,
                        zero_tol=int_cfg.zero_tol,
                        )[0].flatten()
                elif gen_cfg.mp_parallel < 1.5:
                    y_match[:, i_treat] = nn_neighbour_mcf2(y_dat, x_dat, d_dat,
                                                            cov_x_inv=cov_x_inv,
                                                            treat_value=i_value, i_treat=i_treat,
                                                            **kw_args,
                                                            )[0].flatten()
                else:  # Parallelized with prange of Numba (fastest)
                    y_match[:, i_treat] = nn_neighbour_mcf2_parallel(y_dat.astype(np.float64),
            ...
            ray_err_txt='Ray did not start in nearest neighbour matching.'
            if int_cfg.mp_use_old_ray:
                if ray is None or ray_nn_neighbour_mcf2 is None:
                ...
                (x_dat_ref, y_dat_ref, d_dat_ref, cov_x_inv_ref
                 ) = mcf_ray.ray_put_all(x_dat, y_dat, d_dat, cov_x_inv)

                still_running = [ray_nn_neighbour_mcf2.remote(y_dat_ref, x_dat_ref, d_dat_ref,
                                                              cov_x_inv=cov_x_inv_ref,
                                                              treat_value=d_values[idx],
                                                              i_treat=idx, cuda=cuda,
                                                              **kw_args,
                                                              )
                ...
                while len(still_running) > 0:
                    ...
                    for res in finished_res:
                        y_match[:, res[1]] = res[0]
                (x_dat_ref, y_dat_ref, d_dat_ref, cov_x_inv_ref, finished_res, finished
                 ) = mcf_ray.ray_del_refs(x_dat_ref, y_dat_ref, d_dat_ref, cov_x_inv_ref,
                                          f1=finished_res, f2=finished,
                                          mp_ray_del=int_cfg.mp_ray_del,
                                          )
            else:
                shared_nn_data = {'x_dat': x_dat, 'y_dat': y_dat, 'd_dat': d_dat,
                                  'cov_x_inv': cov_x_inv,
                                  }
                fail_txt='Failed to make executor in nearest neighbour matching.'
                with forest_executor_with_shared(int_cfg=int_cfg, maxworkers=maxworkers,
                                                 shared_obj=shared_nn_data,
                                                 shared_name='nn_matching_data',
                                                 ray_err_txt=ray_err_txt,
                                                 fail_txt=fail_txt,
                                                 ) as (executor, data_handle, maxworkers):
                    tasks = [TaskSpec(func=nn_neighbour_mcf2_backend,
    ...
    y_match_df = pd.DataFrame(data=y_match, columns=y_match_name, index=data_df.index)
    data_new_df = pd.concat([data_df.reset_index(drop=True), y_match_df.reset_index(drop=True)],
                            axis=1,
                            )
    if gen_cfg.with_output and print_out:
        mcf_ps.print_mcf(gen_cfg, '\nMatched outcomes', summary=True)
        mcf_ps.print_descriptive_df(gen_cfg, data_new_df, varnames=y_match_name, summary=True)
    if gen_cfg.with_output and int_cfg.descriptive_stats and print_out:
        ...
        d_name = var_cfg.grid_nn_name if gen_cfg.d_type == 'continuous' else var_cfg.d_name
        if mcf_.gen_tv_cfg.yes or (isinstance(d_name, (tuple, list)) and len(d_name) > 1):
            d_name = [d_name[0]]
        mcf_ps.statistics_by_treatment(gen_cfg, data_new_df, d_name, var_cfg.y_match_name,
...
def nn_neighbour_mcf2_backend(*, data: dict[str, Any],
                              treat_value: Any,
                              i_treat: int,
                              cuda: bool,
    ...
    """
    Backend-agnostic wrapper for one treatment-specific nearest-neighbour task.

    Replace `nn_neighbour_mcf2_plain(...)` by the plain non-Ray implementation
    that contains the actual logic of `ray_nn_neighbour_mcf2`.
    """
    y_dat = data['y_dat']
    x_dat = data['x_dat']
    d_dat = data['d_dat']
    cov_x_inv = data['cov_x_inv']

    return nn_neighbour_mcf2(y_dat, x_dat, d_dat,
                             cov_x_inv=cov_x_inv, treat_value=treat_value, i_treat=i_treat,
                             cuda=cuda,
                             **kw_args,
                             )
...
#     if cuda:
#         y_all, i_treat = nn_neighbour_mcf2_cuda(y_dat, x_dat, d_dat,
#                                                 obs=obs, cov_x_inv_np=cov_x_inv,
#                                                 treat_value=treat_value,
#                                                 i_treat=i_treat, zero_tol=zero_tol,
#                                                 )
#     else:
#         y_all, i_treat = nn_neighbour_mcf2(y_dat, x_dat, d_dat,
#                                            obs=obs, cov_x_inv=cov_x_inv, treat_value=treat_value,
#                                            i_treat=i_treat, zero_tol=zero_tol,
#                                            )
#     y_all = y_all.flatten()

#     return y_all, i_treat
def _ray_nn_neighbour_mcf2_impl(y_dat: NDArray[Any],
                                x_dat: NDArray[Any],
                                d_dat: NDArray[Any], *,
                                obs: int,
                                cov_x_inv: NDArray[Any],
    ...
    if cuda:
        y_all, i_treat = nn_neighbour_mcf2_cuda(y_dat, x_dat, d_dat,
                                                obs=obs, cov_x_inv_np=cov_x_inv,
                                                treat_value=treat_value, i_treat=i_treat,
                                                zero_tol=zero_tol,
                                                )
    else:
        y_all, i_treat = nn_neighbour_mcf2(y_dat, x_dat, d_dat,
    ...
    return y_all.flatten(), i_treat


ray_nn_neighbour_mcf2 = ray.remote(_ray_nn_neighbour_mcf2_impl) if ray is not None else None
...
def nn_neighbour_mcf2_cuda(y_dat_np: NDArray[Any],
                           ...
                           treat_value: list | tuple | NDArray[Any] | int | float,
                           i_treat: int,
                           zero_tol: float = 1e-10,
                           ) -> tuple[NDArray[Any], int]:
    ...
    mask = np.isclose(d_dat_np, treat_val, atol=zero_tol, rtol=zero_tol).reshape(-1)
    ...
    cov_x_inv = torch.from_numpy(cov_x_inv_np).to(device=device, dtype=float_dtype)
    non_treat_idx = torch.from_numpy(non_treat_idx_np).to(device=device, dtype=torch.long)
...
def nn_neighbour_mcf2(y_dat: NDArray[Any],
    ...
    i_treat: see above (included to ease mulithreading which may confuse positions).

    """
    mask = np.isclose(d_dat, treat_value, atol=zero_tol, rtol=zero_tol).reshape(-1)
    x_t = x_dat[mask]
    y_t = y_dat[mask]
...
def nn_neighbour_mcf2_parallel(y_dat: NDArray[Any],
    ...
    for idx in prange(n_obs):                                 # pylint: disable=not-an-iterable
    ...
    if cuda:
        ...
        while rank_not_ok:
            ...
            if counter > 20:
                ...
                break
            if torch.linalg.matrix_rank(cov_x) < cols_x:   # pylint: disable=not-callable
                cov_x += 0.5 * torch.diag(cov_x) * torch.eye(cols_x, device='cuda')
                counter += 1
            else:
                cov_x_inv_t = torch.linalg.inv(cov_x)   # pylint: disable=not-callable
                rank_not_ok = False
...
def nn_matched_outcomes_issue_17(df: pd.DataFrame, *,
    ...
    ...
    use_n_gb_memory: int
        Estimate the amount of GB you want to use to calculate the nearest neighbours.
        If the matrix-broadcasting is too large, the calculation will be chunked.

    Returns
    -------
    pd.DataFrame
        DataFrame containing the matched potential outcomes for each treatment condition.
    ...
    ...
    var_names_ordered = [var_name for var_name, type_ in var_x_type.items() if type_ in [0]]
    var_names_unordered = [var_name for var_name, type_ in var_x_type.items() if type_ in [1, 2]]
    txt = ''
    ...
    if var_names_unordered:
        df_x_u = pd.get_dummies(data=df[var_names_unordered],
                                columns=var_names_unordered,
                                prefix=var_names_unordered,
                                dtype=int,
                                )
        np_x = pd.concat([df_x_u, df_x_o], axis=1).to_numpy(dtype=np.float32)
    else:
        np_x = df_x_o.to_numpy(dtype=np.float32)

    np_y = df[y_tree_name].to_numpy(dtype=np.float32)
    np_d = df[d_name].to_numpy(dtype=np.float64)   # Required for the logical comparison below
    ...
    if match_nn_prog_score:
        ...
        if (mtot in [1, 4]) and (np_x.shape[1] >= (2 * no_of_treats)):
            ...
            for idx, d_treatment in enumerate(d_values):
                ...
                treatments = np.isclose(np_d, d_treatment, atol=zero_tol, rtol=zero_tol).squeeze()
            ...
            txt += "\nPrognostic scores not used for matching (too few covariates)"
    else:
        np_match_y = None
    ...
    if mtot in [1, 4]:
        ...
        for i_treat, i_value in enumerate(d_values):
            ...
            mask = np.isclose(np_d, i_value, atol=zero_tol, rtol=zero_tol).squeeze()
            ...
            for i_chunk in range(0, len(temp_np_control), chunk_size):
                ...
                x_diff = (temp_np_x[None, :, :] - temp_np_control[i_chunk: i_chunk + chunk_size,
                                                                  None, :]
                          )  # type: ignore
                dist = np.sum((x_diff @ inv_cov_x) * x_diff, axis=2)

                # Find minimal distance matches
                is_min_dist = dist <= (dist.min(axis=1, keepdims=True) + zero_tol)

                # n_element for the mean
                n_min = is_min_dist.sum(axis=1)

                # calculate the mean
                np_match_y[np.flatnonzero(~mask)[i_chunk: i_chunk + chunk_size], i_treat] = (
                    (is_min_dist @ temp_np_y).squeeze() / n_min
                    )  # type: ignore
    ...
    return pd.DataFrame(data=np_match_y, columns=match_y_names, index=df.index,), txt


def check_forest_treatment_codes(d_dat: NDArray[Any], *,
                                 no_of_treat: int,
                                 d_values: list[int | float] | NDArray[Any],
                                 zero_tol: float,
                                 ) -> None:
    """Check that forest splitting receives zero-based discrete treatment codes."""
    d_flat = np.asarray(d_dat).reshape(-1)

    if not np.all(np.isfinite(d_flat)):
        raise ValueError('Treatment variable contains NaN or infinite values.')

    d_int = d_flat.astype(np.int64, copy=False)

    if not np.all(np.isclose(d_flat, d_int, atol=zero_tol, rtol=zero_tol)):
        raise ValueError('Forest splitting expects discrete treatment codes to be integer-valued. '
                         f'Observed treatment values are {np.unique(d_flat).tolist()}.'
                         )
    expected = np.arange(no_of_treat, dtype=np.int64)
    observed = np.unique(d_int)

    if not np.array_equal(observed, expected):
        raise ValueError('Forest splitting expects drecoded internal treatment labels 0, ..., '
                         f'no_of_treat - 1. Expected {expected.tolist()}, '
                         f'observed {observed.tolist()}, gen_cfg.d_values is {list(d_values)}. '
                         'This indicates that the treatment was not recoded before forest building.'
                         )
