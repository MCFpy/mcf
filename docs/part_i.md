...
The data file has to be stored in *csv* format and its name has to be assigned to [indata](./core_6.md#indata) without adding the file extension *.csv*. Optionally, a separate data file for estimating treatment effects can be specified in [preddata](./core_6.md#preddata).

The program creates the folder ***out*** and stores it in your default working directory or in the directory specified in [outpfad](./core_6.md#outpfad) if the path ends with *\out*.  This ***out*** folder contains the results. The text document summarizes descriptive statistics and results on estimation and inference. The sub-folders *fig_csv*, *fig_jpeg*, and *fig_pdf* comprise *csv* data files and *jpeg* and *pdf* plots of the results.  The csv file ending with the name *Predpred.csv* stores predicted effects and standard errors for all units. If [save_forest](./core_6.md#save_forest) is *True*, files with information on the optimal forest are passed to the ***out*** folder. User-specific name for these files can be assigned with [forest_files](./core_6.md#forest_files). Finally, the folder *\_tempmcf_*, filled with temporary data files, will be temporarily stored in ***out*** and deleted after program execution.

### Variables

The program requires the provision of at least three distinct variables. You have to define a treatment, one or more outcomes, and either ordered or unordered features.
...
For a more detailed analysis, further variables can be used optionally.
...
| [w_name](./core_6.md#w_name)                                 | Sample weights.                                              |
...
Features with low variability may not be well suited to explain the outcome of interest. The keyword argument [min_dummy_obs](./core_6.md#min_dummy_obs) provides the option to delete binary features with low variation. The user can pass the minimum number of units for a binary category to this keyword argument, so that dummies not fulfilling this condition will be removed.

### Reweighting

The estimated effects are computed as differences in weighted outcomes. The program offers several options to reweight the forest weights. The user can specify different weights for each treatment state and each unit.

The flag [choice-based sampling](./core_6.md#choice-based sampling) activates the option for unequal treatment weights. The new weights have to be assigned to [choice-based weights](./core_6.md#choice-based weights).

The user can further specify varying weights for each unit of the sample. This option can be selected with [weighted](./core_6.md#weighted) and the sampling weights have to be defined in [w_name](./core_6.md#w_name).

### Keyword arguments for data cleaning and sampling weights

| Keyword                                                    | Details                                                      |
| ---------------------------------------------------------- | ------------------------------------------------------------ |
| [clean_data_flag](./core_6.md#clean_data_flag)             | Removing units with missing values and keeping only variables available in the data set and defined as variable arguments. The default is *True*. |
...
| [min_dummy_obs](./core_6.md#min_dummy_obs)                 | Threshold in units per category determining if dummies are dropped due to low variation. The default is 10. |
| [choice-based sampling](./core_6.md#choice-based sampling) | If *True*, assignment of choice-based probabilities for treatment states is activated. The default is *False*. |
| [choice-based weights](./core_6.md#choice-based weights)   | List with one entry (=weight) for each treatment. If the user specifies unequal weights for the treatment states, [choice-based sampling](./core_6.md#choice-based sampling) has to be activated. |
| [weighted](./core_6.md#weighted)                           | If *True*, sampling weights for each unit $i$ specified in  [w_name](./core_6.md#w_name) will be used. The default is *False*. |
...
When the number of irrelevant features increases, random forests become more likely to pick sample splits based on those features, which will generally lead to a deterioration of the quality of the estimates. Also computational speed decreases with the number of features. Therefore, it should be advantageous to remove such features prior to starting estimation. Here, the program selects features based on the variable importance measures of Causal Forests prior to full-scale estimation. Details on the procedure and its corresponding keyword arguments are indicated below.
...
| [fs_other_sample](./core_6.md#fs_other_sample)             | Creating a separate sample for feature selection. The default is *False*. |
| [fs_other_sample_share](./core_6.md#fs_other_sample_share) | Share of data used for feature selection. The default is 0.2. |
| [fs_rf_threshold](./core_6.md#fs_rf_threshold)             | Threshold for loss of variable importance in percent. The default is 0. |

### Method

After activating feature selection, the program builds a forest to compute variable importance measures. If the variable importance measure for a specific variable is lower than [fs_rf_threshold](./core_6.md#fs_rf_threshold), this variable is (in principle) deleted. The program further offers an option to run the feature selection in a separate sample. The algorithm for forest building is identical to the main algorithm, which is described in more detail in [Forest Growing](#Forest Growing).

The feature selection procedure deselects unimportant variables. To this end, it computes variable importance statistics (VIS) for each single feature, where higher VIS indicate higher importance. However, deselecting features based on small single VIS may result in removing relevant variables. For example, if two features are highly correlated, both of their single VIS are close to zero, because each feature on its own adds barley new information to the model, given the other features. To avoid this trap of dropping such potentially relevant variables in settings with strong correlations, the program forms groups of features ordered by their VIS and computes new variable importance statistics for each group (VISG). It also builds aggregated groups ordered by their VISG and calculates variable importance statistics for each aggregated group (VISaG). Then, only those features are regarded as unimportant, whose values of VIS, VISG and VISaG are jointly below the user-defined importance threshold [fs_rf_threshold](./core_6.md#fs_rf_threshold). These features are not considered for further analysis.

Note that the program assigns a selection probability of 1 to features that are a priori considered to be very important, as defined by the input arguments [x_name_always_in_unord](./core_6.md#x_name_always_in_ord), [x_name_always_in_ord](./core_6.md#x_name_always_in_unord), [x_name_remain_ord](./core_6.md#x_name_remain_ord), [x_name_remain_unord](./core_6.md#x_name_remain_unord), [z_name_list](./core_6.md#z_name_list)**, **[z_name_split_ord](./core_6.md#z_name_split_ord), [z_name_split_unord](./core_6.md#z_name_split_unord), [z_name_mgate](./core_6.md#z_name_mgate), and [z_name_amgate](./core_6.md#z_name_amgate).

The [Technical Appendix](./techn_app.md) presents information on the tuning parameters for the forest built for feature selection. It also gives details on the computation of the variable importance measures and the construction of the groups.
...
where $\hat{y}^{-i}(X_i)$ are estimates of the conditional expectation $\mathbb{E}[Y_i \vert X=x]$, computed without considering the $i$-th unit.

In this program, local centering runs after activating the flag [l_centering](./core_6.md#l_centering).

### Implementation

Generally, this program randomly divides the data into at least two distinct and mutually exclusive samples, with equal size by default. The first sample is a training sample to build the forest, the second sample is created to compute the effects. Optionally, a third sample for feature selection can be generated with [fs_other_sample_share](./core_6.md#fs_other_sample_share).

Next, the local centering procedure applies the *RandomForestRegressor* method of the *sklearn.ensemble* module to compute the predicted outcomes $\hat{y}^{-i}(X_i)$ for each unit $i$ non-parametrically. These predictions are computed in distinct subsamples by cross validation, where the number of folds can be specified in [l_centering_cv_k](./core_6.md#l_centering_cv_k). Finally, the centered outcomes are obtained by subtracting the predicted from the observed outcomes.

Alternatively, two separate samples can be generated for running the local centering procedure with  [l_centering_new_sample](./core_6.md#l_centering_new_sample). In this case, the size of the first sample can be defined in  [l_centering_share](./core_6.md#l_centering_share) and it is used for training a Random Forest, again by applying the *RandomForestRegressor* method. The predicted and centered outcomes $\hat{y}^{-i}(X_i)$ and $\tilde{Y}_i$, respectively, are computed in the second sample. Finally, this second sample is divided in mutually exclusive datasets for feature selection (optionally), tree building, and effect estimation.
...
| [l_centering_new_sample](./core_6.md#l_centering_new_sample) | Generating separate samples for tree building and estimation of $\mathbb{E}[Y_i \vert X=x]$. The default is *False*. |
| [l_centering_share](./core_6.md#l_centering_share)           | Share of data used for estimating $\mathbb{E}[Y_i \vert X=x]$. The default is 0.25. |
...
The program computes three types of average treatment effects, which differ in their aggregation level of features and are discussed in depth by Lechner (2018). The estimands are the individualized average treatment effect (IATE), the group average treatment effect (GATE), and the average treatment effect (ATE). They can be defined in the following way:

#### Definition of average effects
...
The program estimates IATEs for each unit in the prediction sample without the need of specifying any input arguments. Predicted IATEs and their standard errors will be saved to a CSV file and descriptive statistics are printed to the text file in the ***out*** folder of your project directory.

#### GATEs

By default, the program smooths and discretizes the distribution of the GATEs for continuous features. A smoothing procedure evaluates the effects at a local neighborhood around a pre-defined number of evaluation points. The flag [smooth_gates](./core_6.md#smooth_gates) activates this procedure. The level of discretization depends on the number of evaluation points, which can be defined in [smooth_gates_no_evaluation_points](./core_6.md#smooth_gates_no_evaluation_points). The local neighborhood is based on an Epanechnikov kernel estimation using Silverman's bandwidth rule. The keyword argument [smooth_gates_bandwidth](./core_6.md#smooth_gates_bandwidth) specifies a multiplier for Silverman's bandwidth rule.

#### ATEs

The program estimates ATEs by aggregating over the full distribution of the features, as defined in *General remarks* above. These estimates are computed without the need of specifying any input arguments.

### Marginal effects

The program further calculates two forms of marginal treatment effects. The estimands presented are the marginal treatment effect evaluated at fixed reference points (MTE) and the average marginal treatment effect (AMTE). The marginal effects can be approximated by a discrete version of the definition of a derivative as:
...
Marginal effects for continuous features of $Z$ are evaluated at a pre-defined number of equally spaced quantiles of evaluation points. The default number of quantiles is 50 and can be changed in [gmate_no_evaluation_points](./core_6.md#gmate_no_evaluation_points).

Computing AMTEs is much more time consuming than calculating MTEs. The reason is that an MTE for a specific category $z \in Z$ evaluated at single reference points of the remaining features requires the computation of a single IATE only. To compute an AMTE for the same $z \in Z$, it is necessary to estimate a GATE, which aggregates the IATEs for all units assigned to that category, evaluated at the same observed values of the other features. To save computation time, the AMTEs are based on random sub-samples of size $N_{SS}$. Details on $N_{SS}$ are shown in the [Technical Appendix](./techn_app.md).
...
| ------------------------------------------------------------ | ------------------------------------------------------------ |
| [smooth_gates](./core_6.md#smooth_gates)                     | Flag for smoothing the distribution of the estimated GATEs. The default is True. |
...
| [gmate_sample_share](./core_6.md#gmate_sample_share)         | Number in the interval $(0,1]$ determining the size of random sub-samples $N_{SS}$ for the computation of AMTEs. Note that $N_{SS}$ also depends on the number of evaluation points, see [Technical Appendix](./techn_app.md). |
| [atet_flag](./core_6.md#atet_flag)                           | If *True*, average treatment effects for subpopulations are computed. This only works if at least one GATE feature is specified. The default is *False*. |
| [gatet_flag](./core_6.md#gatet_flag)                         | If *True*, group average treatment effects for subpopulations are computed.  This only works if at least one GATE feature is specified. The default is *False*. |

### Plotting the treatment effects


The program generates graphical representations of the treatment effects. The user can customize the font size of the legends, the number of pixels of the figures, and the level of the confidence bounds. The corresponding input arguments are [fontsize](./core_6.md#fontsize), [dpi](./core_6.md#dpi), and [ci_level](./core_6.md#ci_level), respectively. For line plots of GATEs, MTEs, and AMTEs, you can fill the area between lines if the number of evaluation points exceeds a threshold specified in [no_filled_plot](./core_6.md#no_filled_plot).
...
| [ci_level](./core_6.md#ci_level)             | Number in the $ (0,1)$ interval determining the confidence level for bounds. The default is 0.9. |
| [no_filled_plot](./core_6.md#no_filled_plot) | Threshold for number of evaluation points to fill the area between lines. The default is 20. This works for line plots for GATEs, MTEs, and AMTEs. |
...
The program offers two ways of conducting inference, which are discussed in more detail by Lechner (2018) . The default is a weights-based inference procedure, which is particularly useful for gaining information on the precision of estimators that have a representation as weighted averages of the outcomes. The user can then select among different tuning parameters for weights-based inference. The second inference procedure simply estimates the variance of treatment effect estimates as the sum of the variance of weighted outcomes.

### Methods

One way for conducting inference for treatment effects is to estimate the variance of the treatment effect estimator based on a variance decomposition into the expectation of the conditional variance and the variance of the conditional expectation, given the weights. This variance decomposition takes heteroscedasticity in the weights into account. The conditional means $\mu_{Y \vert \hat{W}} (\hat{w}_i)$ and variances $\sigma^2_{Y \vert \hat{W}} (\hat{w}_i)$ are estimated non-parametrically, either by the Nadaraya-Watson kernel estimator (default) or by the *k-Nearest Neighbor* (*k-NN*) estimator. Details on the parameters of the *k-NN* and kernel estimators are shown in the [Technical Appendix](./techn_app.md#Technical Appendix).
...
After estimating treatment effects and inference, the program conducts a  heterogeneity analysis. It evaluates the correlation of treatment effects and presents graphical representations of the effects and their densities. In addition, it investigates heterogeneity for different clusters with respect to treatment effects, potential outcomes, and features. Finally, the program measures the variable importance of the features that are used for estimation.

### Starting the diagnostics

To start the post-estimation diagnostics, the flag [post_est_stats](./core_6.md#post_est_stats) has to be activated. The input argument [relative_to_first_group_only](./core_6.md#relative_to_first_group_only) specifies the reference group. If [relative_to_first_group_only](./core_6.md#relative_to_first_group_only) is *True*, the comparison group comprises the units assigned to the first treatment state, which is often the control group. Otherwise, all possible treatment combinations are compared among each other. The confidence bounds are specified by [ci_level](./core_6.md#ci_level).
...
Plots can be printed with [post_plots](./core_6.md#post_plots). This generates sorted effects plots of both, the IATEs and their deviations from the ATE, estimated for the units in the prediction sample defined in [preddata](./core_6.md#preddata). The plots of the effects and the confidence bounds are based on moving averages of the IATEs and their standard errors. The number of neighbors *k* for the moving averages is computed by *k-NN* estimation, with input arguments [knn_min_k](./core_6.md#knn_min_k) and [knn_const](./core_6.md#knn_const), as discussed in the [Technical Appendix](./techn_app.md). The program further creates kernel density plots for the IATEs, using the Epanechnikov kernel and Silverman's bandwidth rule.


### *k-Means* clustering

To analyze heterogeneity in different groups (clusters), the program applies *k-Means* clustering if [post_kmeans_yes](./core_6.md#post_kmeans_yes) is set to *True*. It uses the *k-means++* algorithm of the *KMeans* method provided by Python's sklearn.cluster module. Clustering refers to the IATEs, the potential outcomes for each treatment state, and the features.

The user can define the number of clusters by specifying the input argument  [post_kmeans_no_of_groups](./core_6.md#post_kmeans_no_of_groups). To avoid local extrema, the number of replications with random start centers can be defined in [post_kmeans_replications](./core_6.md#post_kmeans_replications). The argument [post_kmeans_max_tries](./core_6.md#post_kmeans_max_tries) sets the maximum number of iterations in each replication to achieve convergence.


### Post-estimation variable importance

The post-estimation variable importance procedure runs if the flag  [post_random_forest_vi](./core_6.md#post_random_forest_vi) is activated. The procedure builds a predictive random forest to learn major features influencing the IATEs. The variable importance measures are presented in percentage points of the coefficient of determination *R<sup>2</sup>* lost when randomizing single features. The *R<sup>2</sup>* statistics are obtained by the *score* method provided by the *RandomForestRegressor* object of Python's sklearn.ensemble module.
...
| [post_kmeans_replications](./core_6.md#post_kmeans_replications) | Number of replications with random start centers for *k-Means* clustering. The default is 10. |
...
Treatment effects may be subject to selection bias if the distribution of the confounding features differs across treatment states. This program runs non-parametric balancing tests to check the statistical equality of the distribution of features after adjustment by the modified causal forest.  
...
The tests are based on estimations of ATEs by replacing the outcomes with user-specified confounding features. For multiple treatments, the results consider all possible treatment combinations. Features with ATEs not significantly different from zero can be regarded as balanced across treatment states.

Note that these balancing tests are not validated yet.
...
Estimation methods adjusting for differences in features require common support in all treatment arms. This program thus drops units considered to be off support.

### Implementation

First, the program uses the training sample to build one forest per treatment by applying the *RandomForestRegressor* method of the *sklearn.ensemble* module. It then predicts in the full sample the  propensity scores $P(D = m \vert X)$ for all mutually exclusive treatments $m = 1,\cdots,M$, where $M$ denotes the number of treatments.

The program gives the opportunity to choose between data-driven and user-defined support conditions, which can be selected with [support_check](./core_6.md#support_check). For data-driven support, you have to specify a quantile in [support_quantil](./core_6.md#support_quantil). Denoting by $q$ the quantile chosen, the program drops units with propensities scores smaller than the largest $q$ or larger than the smallest ($1-q$) quantile of the treatment groups. Contrary to this condition, user-defined support directly specifies the support threshold of the propensity scores in [support_min_p](./core_6.md#support_min_p). The program then drops units outside the propensity score range $[P(D = m \vert X),1-P(D = m \vert X)]$.

### Truncation of estimated forest weights

To obtain stable estimates, the program provides the option to excludes units with extreme weights from estimation. This can be enforced with the input argument [max_weight_shares](./core_6.md#max_weight_shares).

After reweighting the forest weights, the program renormalizes the weights for estimation. Because of the renormalization step, the final weights can be slightly above the threshold defined in [max_weight_shares](./core_6.md#max_weight_shares).  

### Input arguments for common support

| Argument                                           | Description                                                  |
| -------------------------------------------------- | ------------------------------------------------------------ |
| [support_check](./core_6.md#support_check)         | Three integers (0,1,2). 0 does not consider common support. 1 carries out data-driven support, 2 is for user-defined support. The default is 1. |
| [support_quantil](./core_6.md#support_quantil)     | Float in the $(0,1]$ interval. The default is 1.             |
| [support_min_p](./core_6.md#support_min_p)         | Float in the $(0,0.5)$ interval. The default is $\min(0.025,r)$, where $r$ is the ratio between the share of the smallest treatment group in the training sample and the number of treatments. |
