...
from mcf.mcf_print_stats import print_dic
...
def change_keywords(mcf_: 'ModifiedCausalForest',
    ...
    check_if_good_instance(mcf_.instance_used_for_training, mcf_.instance_used_for_prediction,
                           prediction, analysis,
                           )
    ...
    for key, value in new_keywords.items():
        mcf_ = change_method_for_prediction_and_analysis(mcf_, key, value, iv=iv)
    if mcf_.gen_cfg.with_output:
        (mcf_.gen_cfg.outfiletext, mcf_.gen_cfg.outfilesummary, mcf_.gen_cfg.outpath,
         mcf_.p_cfg.paths) = change_output_locations(mcf_.gen_cfg.outfiletext,
...
def change_method_for_prediction_and_analysis(mcf_: 'ModifiedCausalForest',
    ...
    match key:
        ...
        case 'var_x_name_balance_test_ord':
            mcf_.var_cfg.x_name_balance_test_ord = check_valid_names(key, value,
        ...
        case 'cs_type':
            mcf_.cs_cfg.type_ = int_allow_only_specific_values(key, value, (0,))
        case 'p_ba':
            mcf_.p_ba_cfg.yes = change_bool_one_way(key, value, False)
        case 'p_ba_adj_method':
            mcf_.p_ba_cfg.adj_method = mcf_initvals.p_ba_adjust_method(value, mcf_.p_ba_cfg.yes)
        ...
        case 'p_gates_smooth_no_evalu_points':
            mcf_.p_cfg.gates_smooth_no_evalu_points = value_integer(key, value, minv=2, maxv=100000)
        case 'p_gates_no_evalu_points':
            mcf_.p_cfg.gate_no_evalu_points = value_integer(key, value, minv=2, maxv=100000)
        ...
        case 'p_qiate_smooth_bandwidth':
            mcf_.p_cfg.qiate_smooth_bandwidth = value_pos_max(key, value, maxv=float('inf'))
        ...
        case 'p_choice_based_probs':
            mcf_.p_cfg.choice_based_probs = check_cb_probs(key, value, mcf_.gen_cfg.no_of_treat)
        case 'p_cond_var':
        ...
        case 'p_knn_min_k':
            mcf_.p_cfg.knn_min_k = value_integer(key, value, minv=1, maxv=float('inf'))
        case 'p_nw_bandw':
        ...
        case 'p_iv_aggregation_method':
            if iv:
                mcf_.p_cfg.iv_aggregation_method = mcf_initvals.iv_agg_method_fct(value)
            else:
        ...
        case 'post_kmeans_no_of_groups':
            mcf_.post_cfg.kmeans_no_of_groups = value_integer( key, value, minv=2, maxv=100)
        case 'post_kmeans_max_tries':
            mcf_.post_cfg.kmeans_max_tries = value_integer(key, value, minv=1, maxv=10000)
        case 'post_kmeans_min_size_share':
            mcf_.post_cfg.kmeans_min_size_share = value_pos_max(key, value, maxv=33)
        case 'post_kmeans_replications':
            mcf_.post_cfg.kmeans_replications = value_integer(key, value, minv=1, maxv=1000)
        ...
        case 'post_relative_to_first_group_only':
            mcf_.post_cfg.relative_to_first_group_only = change_bool_both_ways(key, value)
        case 'post_plots':
            mcf_.post_cfg.plots = change_bool_both_ways(key, value)
        case 'post_tree':
            mcf_.post_cfg.tree = change_bool_both_ways(key, value)

        case 'no_treatment_versions':
            if len(mcf_.var_cfg.d_name) == 2:
                del mcf_.var_cfg.d_name[-1]   # Remove versions from treatments
                mcf_.gen_tv_cfg.d_dict = {}
            mcf_.gen_tv_cfg.yes = False
            mcf_.var_cfg.x_name_tv = []
...
def check_if_int(key: str, value: Any):
...
def check_if_string(key: Any):
    ...
    if not isinstance(key, str):
        raise TypeError('Key of dictionary used to change parameters for '
...
def check_if_dictionary(new_keywords: Any) -> None:
    ...
    if not isinstance(new_keywords, dict):
        raise TypeError('Keyword "new_keywords" must be passed as dictionary.')
...
def check_if_good_instance(used_for_training: bool,
    ...
    if prediction:
        if not used_for_training:
            raise ValueError('ModifiedCausalForest instance has not yet used for training. '
                             'Train forest before using it for prediction. '
                             )
        if used_for_prediction:
            raise ValueError('ModifiedCausalForest instance has already been used for prediction. '
    ...
    if analysis:
        if not used_for_prediction:
            raise ValueError('ModifiedCausalForest instance must be used for '
                             'prediction before the "analyse" method can be used. '
                             )


def not_a_valid_key(key: Any) -> None:
    """Raise exception when key is not among valid keys."""
    raise ValueError(f'{key} cannot be changed in the predict method (or is not a valid keyword). ')
...
def check_valid_names(
    ...
    names_unord_prime = [name + '_prime' for name in names if name + '_prime' in valid_names_unord]
    names_unord_org = [name for name in names if name + '_prime' in valid_names_unord]
...
def int_allow_only_specific_values(key: str, value: Any, allowed_values: int
                                   ) -> int:
    """Change value of an integer to allowed value."""
    check_if_int(key, value)
    if value not in allowed_values:
        raise ValueError('{key} can only be changed to one of the following'
                         'values {" ".join(target_ok)}'
                         )
    return value
...
def value_pos_max(key: str, value: Any, maxv=1) -> Real:
    ...
    if not 0 < value <= maxv:
        raise ValueError(f'Value of {key} must be positive and not larger than {maxv}.')
    return value
...
def check_cb_probs(key: str, probs: Any, no_of_treat: int) -> list[np.floating]:
    ...
    if not isinstance(probs, (list, tuple,)):
        raise TypeError(f'{key} must be specified as list or tuple.')
    if any(not isinstance(prob, (Real, np.floating)) for prob in probs):
        raise TypeError('All variable names must be specified as numbers.')
    if len(probs) != no_of_treat:
        raise TypeError(f'{key}: Number of probabilites is not equal to '
...
def int_valid_values(key: str, value: Any, valid_vals: list | tuple) -> int:
    ...
    if not isinstance(value, Real):
        raise TypeError(f'{key} must be specified as number.')

    value = round(value)
    if not round(value) in valid_vals:
        raise TypeError(f'{key} must have one of the following values: {" ".join(valid_vals)}')
...
def check_bootstrap_se(key: str, value: Any):
    ...
    if not isinstance(value, (bool, Real)):
        raise TypeError(f'{key} must be boolean or a positive number.')
