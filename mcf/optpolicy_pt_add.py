...
if TYPE_CHECKING:
    ...
    from mcf.optpolicy_init import GenCfg, VarCfg, FairCfg, OtherCfg, PtCfg
            ...
            for splits_dic in splits[j]:  # Iterate over leaf information
                ...
                if splits_dic['x_type'] == 'unord':
                    ...
                    for s_i in values_to_print:
                        if isinstance(s_i, int) or ((s_i - np.round(s_i)) < 0.00001):
                    ...
                    tree_dic[leaf_key]['cut-off or set'] = splits_dic['cut-off or set']
            txt += f'\nAlloc Treatment: {treat[i][j]:3d} '
            if obs is not None:
                txt += f'  Obs: {obs[i][j]:6d}  '
            if score_val is not None:
                txt += f'Avg.score: {score_val[i][j] / obs[i][j]:7.3f} '
                tmp = score_val[i][j] / obs[i][j] - other_cfg.costs_of_treat[treat[i][j]]
...
def describe_protected_material_strata(fair_cfg: 'FairCfg',
                                       var_cfg: 'VarCfg',
    ...
    mat_rel_ord_name = [var for var in mat_rel_name
                        if var in var_cfg.material_ord_name or var in var_cfg.protected_ord_name
                        ]
    ...
    txt += txt_strata + '\n' + '- ' * 50
    txt += f'\nNote: Strata formed using {fair_cfg.protected_disc_method} for protected variables.'
...
def strata_specific_stats(var_name: list[str],
    ...
    for name in var_name:
        sub_name = ('Min'.rjust(int(var_width/2)) + 'Max'.rjust(int(var_width/2))
...
def automatic_cost(optp_: 'OptimalPolicy', data_df: DataFrame) -> list[float]:
    ...
    if gen_cfg.with_output:
        ...
        for j in costs_of_treat:
            txt += f'{j:8.3f}'
        mult_str = [str(round(mult, 3)) for mult in other_cfg.costs_of_treat_mult]
        mult = ' '.join(mult_str)
        txt += f'\nMultipliers to be used: {mult}'
        txt += ('\n' + '- ' * 50
                + '\nAdjusted cost values and allocation in unconstrained optimization'
                )
        for idx, cost in enumerate(costs_of_treat_neu):
            txt += f'\nCost: {cost:8.3f}    Obs.: {alloc[idx]:6d}    Share: {alloc[idx] / obs:6.2%}'
        txt += '\n' + '-' * 100
        mcf_ps.print_mcf(gen_cfg, txt, summary=True)
...
def combinations_categorical(single_x_np: NDArray[Any],
    ...
    if no_of_combinations < no_of_evalupoints:
        ...
        values_sorted, no_of_ps = get_values_ordered(single_x_np, ps_np_diff, values, no_of_values,
                                                     with_numba=with_numba, zero_tol=zero_tol,
                                                     )
        combinations_t = sorted_values_into_combinations(values_sorted, no_of_ps, no_of_values)
...
def get_values_ordered(single_x_np: NDArray[Any],
    ...
    """
    if with_numba:
        values_sorted, no_of_ps = get_values_ordered_numba(single_x_np, ps_np_diff, values,
                                                           no_of_values, zero_tol=zero_tol,
                                                           )
    else:
        values_sorted, no_of_ps = get_values_ordered_no_numba(single_x_np, ps_np_diff, values,
                                                              no_of_values, zero_tol=zero_tol,
                                                              )
...
def get_values_ordered_numba(single_x_np:  NDArray[Any],
    ...
    for r in range(n):
        ...
        for i in range(no_of_values):
            if np.isclose(values[i], x, rtol=zero_tol, atol=zero_tol):
                break
...
def get_values_ordered_numba_prange(single_x_np: NDArray[Any],
                                    ps_np_diff: NDArray[Any],
                                    values: NDArray[Any],
                                    no_of_values: int, *,
                                    zero_tol: float = 1e-10,
    ...
    no_val = len(values)
    for i in prange(no_val):                      # pylint: disable=E1133
        val = values[i]
        # ps_group = ps_np_diff[single_x_np == val
        ps_group = ps_np_diff[np.isclose(single_x_np, val, atol=zero_tol, rtol=zero_tol), :]
...
def get_values_ordered_no_numba(single_x_np: NDArray[Any],
    ...
    for i, val in enumerate(values):
        ps_group = ps_np_diff[np.where(np.isclose(single_x_np, val, rtol=zero_tol, atol=zero_tol))]
...
def adjust_reward(*, no_by_treat_l: NDArray[Any],
    ...
    """
    if with_numba:
        reward_l, reward_r = adjust_reward_numba(no_by_treat_l, no_by_treat_r,
                                                 reward_l, reward_r, max_by_treat
                                                 )
    else:
        reward_l, reward_r = adjust_reward_no_numba(no_by_treat_l, no_by_treat_r,
                                                    reward_l, reward_r, max_by_treat
                                                    )
...
def prepare_data_for_tree_building(
    ...
    data_ps = data_df[var_cfg.polscore_name].to_numpy(copy=True)
    ...
    for j, key in enumerate(x_type.keys()):
        ...
        values_x[j] = sorted(x_values[key]) if x_values[key] is not None else None
    #                  this is None for continuous variables
    ...
    if gen_cfg.x_unord_flag:
        for m_i in range(no_of_x):
            if type_x[m_i] == 'unord':
                ...
                values_x[m_i] = combinations_categorical(data_x[:, m_i], data_ps_diff,
                                                         pt_cfg.no_of_evalupoints,
                                                         int_cfg.with_numba,
                                                         seed=seed,
...
@njit
def only_1st_tree_fct3(data_ps: NDArray[Any], costs_of_treat: NDArray[Any]) -> bool:
...
def evaluate_leaf(data_ps: NDArray[Any],
    ...
    """
    if with_numba:
        indi, reward_by_treat, obs_all = evaluate_leaf_numba(data_ps, gen_cfg.no_of_treat,
                                                             other_cfg.max_by_treat,
                                                             (other_cfg.restricted
                                                              and pt_cfg.enforce_restriction),
                                                             pt_cfg.cost_of_treat_restrict
                                                             )
    else:
        indi, reward_by_treat, obs_all = evaluate_leaf_no_numba(data_ps, gen_cfg.no_of_treat,
                                                                other_cfg, pt_cfg
                                                                )
...
def get_values_cont_x(data_vector: NDArray[Any],
    ...
    """
    if with_numba:
        data_vector_new = get_values_cont_x_numba(data_vector, no_of_evalupoints)
    else:
        data_vector_new = get_values_cont_x_no_numba(data_vector, no_of_evalupoints)

    return data_vector_new


@njit
def get_values_cont_x_numba(data_vector: NDArray[Any], no_of_evalupoints: int) -> NDArray[Any]:
    """Get cut-off points for tree splitting for continuous variables.
...
def get_values_cont_x_numba_prange(data_vector: NDArray[Any],
    ...
    if no_of_evalupoints > (obs - 10):
        ...
        indices = np.linspace(obs / no_of_evalupoints, obs, no_of_evalupoints+1)
        data_vector_new = np.empty(no_of_evalupoints)
        for i in prange(no_of_evalupoints):              # pylint: disable=E1133
...
def get_values_cont_x_no_numba(data_vector: NDArray[Any],
    ...
    if no_of_evalupoints > (obs - 10):
        return data_vector
    indices = np.uint32(np.linspace(obs / no_of_evalupoints, obs, no_of_evalupoints, endpoint=False)
                        )
