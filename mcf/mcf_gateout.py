...
if TYPE_CHECKING:
    ...
    from mcf.mcf_init import IntCfg
    from mcf.mcf_init_predict import PCfg


def make_gate_figures_discr(titel: str,
                            z_name: str,
                            z_vals: list,
                            z_type: int, *,
                            effects: NDArray[Any],
    ...
    file_name_jpeg, file_name_pdf, file_name_csv = gate_fig_file_helper(p_cfg, gate_type, titel_f)
    ...
    if (z_type == 0) and (len(z_values) > int_cfg.no_filled_plot):
        if z_smooth:
            file_name_f_jpeg, file_name_f_pdf, _ = gate_fig_file_helper(p_cfg, gate_type,
                                                                        titel_f + 'fill'
                                                                        )
            figs, axs = plt.subplots()
            axs.plot(z_values, effects, label=label_m, color='b')
            axs.fill_between(z_values, upper, lower, alpha=0.3, color='b', label=label_ci)
            line_ate = '-r'
            if ate_se is not None:
                axs.fill_between(z_values, ate_upper.reshape(-1), ate_lower.reshape(-1),
    ...
    if ate_se is not None:
        ...
        effects_et_al = np.concatenate(
            ...
            axis=1)
        cols = ['upper', 'effects', 'lower', 'ate', 'ate_upper', 'ate_lower', 'z_values']
    else:
...
def make_gate_figures_cont(titel: str,
                           z_name: str,
                           z_vals: list, *,
                           effects: NDArray[Any],
                           int_cfg: 'IntCfg',
    ...
    match gate_type:
        case 'GATE' | 'CBGATE' | 'BGATE' as gt:
            prefix = gt.lower()
            file_name_jpeg = p_cfg.paths[f'{prefix}_fig_pfad_jpeg'] / f'{titel_f}.jpeg'
            file_name_pdf = p_cfg.paths[f'{prefix}_fig_pfad_pdf'] / f'{titel_f}.pdf'
            suffix_map = {'GATE': 'ATE',
                          'CBGATE': 'avg(CBGATE)',
                          'BGATE': 'avg(BGATE)'
                          }
...
def gate_tables_nice(p_cfg: 'PCfg', d_values: ListTupleNdArray, gate_type: str = 'GATE') -> None:
    """
    ...
    try:
        ...
        print(f'Folder {folder} not found.', f' No nice printing of {gate_type}')
        return None
...
def gate_tables_nice_by_hugo_bodory(path: Path, filenames: Path, treatments: list[int]) -> None:
    """
...
def generate_gate_table(params: dict, label_row: bool = False) -> pd.DataFrame:
    ...
    data_0 = np.array(data.pivot(index='z_values', columns="d", values="effects"))
    data_lower = np.array(data.pivot(index='z_values', columns="d", values="lower"))
    data_upper = np.array(data.pivot(index='z_values', columns="d", values="upper"))
    results = np.concatenate((data_0, data_lower, data_upper), axis=1)
    df_new = pd.DataFrame(np.round(results, params['number_of_decimal_places']))
...
def create_dataframe_for_results(data: pd.DataFrame, n_1: int = 2, n_2: int = 3) -> pd.DataFrame:
    """
...
def tables(params: dict) -> None:
    ...
    if stats_table is not None:
        ...
        for k_int in range(0, d_f.shape[0], params['multiplier_rows']):
            ...
            if not isnan(d_f.index[k_int]) and d_f.index[k_int] == int(d_f.index[k_int]):
                # No decimal if int.
                idx_list = d_f.index.tolist()
                d_f.index = idx_list[:k_int] + [int(d_f.index[k_int])] + idx_list[int(k_int) + 1:]
        directory_table = params['path'] / ('_' + params['effect_name'] + '_table.csv')
...
def gate_effects_print(
    ...
    if p_cfg.gates_minus_previous:
        effect_type_label = iv_lab + gate_type, iv_lab + gate_type + '(change)'
    else:
        effect_type_label = iv_lab + gate_type, iv_lab + gate_type + f' - {ate_type}'
    ...
    try:
        for z_name_j, z_name in enumerate(var_cfg.z_name):
            ...
            txt_weight = txt_all[z_name_j] if gate_type == 'GATE' else ''
            txt = ''
            if m_ate_yes:
                    ...
                    for zj_idx, _ in enumerate(z_values):
                        ...
                        if m_ate_yes:
                            ret = mcf_est.effect_from_potential(
                                ...
                                y_pot_m_ate_var[zj_idx, a_idx, :, o_idx].reshape(-1),
                                d_values_dr, continuous=continuous
                                )
                        ...
                        for t_idx, t_lab in enumerate(treat_comp_label):
                            for e_idx, e_lab in enumerate(effect_type_label):
                                ...
                                if e_idx == 0:
                                    ...
                                    if m_ate_yes and not p_cfg.gates_minus_previous:
                                ...
                                if not continuous and effects is not None:
                                    figure_disc = make_gate_figures_discr(
                                        e_lab + ' ' + z_name_ + ' ' + a_lab + ' ' + o_lab + ' '
                                        + t_lab,
                                        z_name, z_values_f, z_type,
                                        effects=effects, stderr=ste, int_cfg=int_cfg, p_cfg=p_cfg,
                                        ate=ate_f, ate_se=ate_f_se, gate_type=gate_type,
                                        z_smooth=z_smooth, gatet_yes=gatet_yes,
                                        x_values_unord_org=gate_est_dic['var_x_values_unord_org']
                                        )
                                if continuous and t_idx == len(treat_comp_label)-1:
                                    ...
                                    figure_cont = make_gate_figures_cont(
                                        e_lab + ' ' + z_name_ + ' ' + a_lab + ' ' + o_lab, z_name,
                                        z_values_f,
                                        effects=effects, int_cfg=int_cfg, p_cfg=p_cfg, ate=ate_f,
                                        gate_type=gate_type, d_values=d_values_dr,
                                        x_values_unord_org=gate_est_dic['var_x_values_unord_org'],
                                        )
                                figure_file = figure_disc if figure_cont is None else figure_cont
                                d_values = d_values_dr if continuous else gen_cfg.d_values
                                vs0 = continuous or int(t_lab[-1]) == int(d_values[0])
                                if a_idx == 0 and vs0 and ((m_ate_yes and e_idx == 1)
...
def get_names_values(mcf_: 'ModifiedCausalForest',
    ...
    for z_name in est_dic['var_cfg'].z_name:
        if mcf_.var_x_type[z_name] > 0:
            values, _ = mcf_gp.recode_if_all_prime(est_dic['var_x_values'][z_name].copy(),
                                                   z_name,
...
def gate_fig_file_helper(p_cfg: 'PCfg', gate_type: str, titel: str) -> tuple[Path, Path, Path]:
    """Get the Gate file names to save the figures."""
    match gate_type:
        case 'GATE' | 'CBGATE' | 'BGATE':
            prefix = gate_type.lower()
            file_name_jpeg = p_cfg.paths[f'{prefix}_fig_pfad_jpeg'] / f'{titel}.jpeg'
            file_name_pdf = p_cfg.paths[f'{prefix}_fig_pfad_pdf']/ f'{titel}.pdf'
            file_name_csv = p_cfg.paths[f'{prefix}_fig_pfad_csv'] / f'{titel}plotdat.csv'
        case _:
