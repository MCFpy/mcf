...
def local_centering(mcf_: 'ModifiedCausalForest',
    ...
    y_0_dtype = np.float64 if len(tree_df) < int_cfg.obs_bigdata else np.float32
    ...
    if train:
        if lc_cfg.cs_cv:   # Crossvalidate ... only tree data is used
            ...
            if fill_y_df is None:
                ...
                tree_mcf_df, tree_lc_df = train_test_split(tree_df, test_size=lc_cfg.cs_share,
                                                           random_state=seed
                                                           )
            else:
                tree_mcf_df, tree_lc_df = train_test_split(tree_df, test_size=lc_cfg.cs_share,
                ...
                fill_y_mcf_df, fill_y_lc_df = train_test_split(fill_y_df, test_size=lc_cfg.cs_share,
                                                               random_state=seed
                                                               )
            data_lc_df = pd.concat((tree_lc_df.reset_index(drop=True),
                                    fill_y_lc_df.reset_index(drop=True)),
                                   axis=0,
                                   )
    ...
    if names_unordered:  # List is not empty; predict and train
        x_tree_df, dummy_names = mcf_data.dummies_for_unord(x_tree_df, names_unordered,
    ...
    if gen_cfg.with_output and gen_cfg.verbose and int_cfg.memory_print:
        print_mememory_statistics(gen_cfg, 'Local centering: Start')
    lc_r2_txt = None
    if train:
        max_workers = 1 if int_cfg.replication else gen_cfg.mp_parallel
        if names_unordered:
            if fill_y_df is not None:
                x_fy_df, _ = mcf_data.dummies_for_unord(x_fy_df, names_unordered,
                                                        data_train_dict=data_train_dict
                                                        )
            if not lc_cfg.cs_cv:
                x_lc_df, _ = mcf_data.dummies_for_unord(x_lc_df, names_unordered,
                                                        data_train_dict=data_train_dict
                                                        )
        if not lc_cfg.cs_cv:
        ...
        y_tree_np = to_numpy_big_data(tree_mcf_df[var_cfg.y_name], int_cfg.obs_bigdata)

        # Find best estimator for first outcome
        estimator, params, txt_sel, _, transform_x, txt_mse = mcf_gf.best_regression(
            x_tree_np,  y_tree_np[:, 0].ravel(), estimator=lc_cfg.estimator,
            boot=cf_cfg.boot, seed=seed, max_workers=max_workers, test_share=lc_cfg.cs_share,
            cross_validation_k=lc_cfg.cs_cv_k if lc_cfg.cs_cv else 0,
            obs_bigdata=int_cfg.obs_bigdata
            )
        if gen_cfg.with_output and gen_cfg.verbose and int_cfg.memory_print:
            print_mememory_statistics(gen_cfg, 'Local centering training: After best_regression')
        forests_all = []
        y_x_tree = np.zeros((len(x_tree_np), len(var_cfg.y_name)), dtype=y_0_dtype)
        if fill_y_df is not None:
            y_x_fy = np.zeros((len(x_fy_np), len(var_cfg.y_name)), dtype=y_0_dtype)
        if lc_cfg.cs_cv:
            ...
            for fold_pred in range(lc_cfg.cs_cv_k):
                ...
                if transform_x:
                    scaler, x_train, x_pred = mcf_gf.scale(x_tree_np[index_train],
                                                           x_tree_np[index_pred]
                                                           )
                    x_fy_np_ = None if fill_y_df is None else scaler.transform(x_fy_np)
                else:
                    x_train = x_tree_np[index_train]
                    x_pred = x_tree_np[index_pred]
                    x_fy_np_ = None if fill_y_df is None else x_fy_np.copy()
                    scaler = None
                forests_y = []
                for idx, _ in enumerate(var_cfg.y_name):
                    ...
                    if gen_cfg.with_output and gen_cfg.verbose and int_cfg.memory_print:
                        print_mememory_statistics(gen_cfg,
                                                  'Local centering training: During cross-fitting',
                                                  )
                forests_all.append(deepcopy(forests_y))
            if gen_cfg.with_output:
                for idx, y_name in enumerate(var_cfg.y_name):
                    txt_return = print_lc_info(y_name,
                                               y_as_classifier=False,
                                               gen_cfg=gen_cfg, txt_sel=txt_sel,
                                               y_pred=y_x_tree[:, idx].ravel(),
                                               y_true=y_tree_np[:, idx].ravel(),
                                               )
                    if idx == 0:
            ...
            forests_y = []
            if transform_x:
                scaler, x_lc_np_, x_tree_np_ = mcf_gf.scale(x_lc_np, x_test=x_tree_np)
                if fill_y_df is not None:
            ...
            for idx, y_name in enumerate(var_cfg.y_name):
                ...
                if gen_cfg.with_output:
                    txt_return = print_lc_info(y_name,
                                               ...
                                               gen_cfg=gen_cfg, txt_sel=txt_sel,
                                               y_pred=y_x_tree[:, idx].ravel(),
                                               y_true=to_numpy_big_data(tree_mcf_df[y_name],
                                                                        int_cfg.obs_bigdata).ravel()
                                               )
                    if idx == 0:
                        lc_r2_txt = txt_return
            if gen_cfg.with_output and gen_cfg.verbose and int_cfg.memory_print:
                print_mememory_statistics(gen_cfg, 'Local centering training: During prediction')
            forests_all.append(deepcopy(forests_y))
        ...
        y_m_yx_tree = to_numpy_big_data(tree_mcf_df[var_cfg.y_name], int_cfg.obs_bigdata) - y_x_tree
        ...
        tree_add_y_lc_df = pd.concat((tree_mcf_df, y_m_yx_tree_df, y_x_tree_df), axis=1)
        if fill_y_df is not None:
            y_m_yx_fy = (to_numpy_big_data(fill_y_mcf_df[var_cfg.y_name], int_cfg.obs_bigdata)
                         - y_x_fy)
            ...
            fill_add_y_lc_df = pd.concat((fill_y_mcf_df, y_m_yx_fy_df, y_x_fy_df), axis=1)
        else:
        ...
        if gen_cfg.with_output:
            ...
            if names_unordered:
                names_unordered_print = data_train_dict['prime_old_name_dict']
            else:
            ...
            for y_idx, objects in enumerate(forests_y):
                ...
                if obj is not None and not isinstance(obj, float):
                    ...
                    print_variable_importance(
                        ...
                        x_name=x_name, names_uo=names_unordered, unordered_dummy_names=dummy_names,
                        gen_cfg=gen_cfg, summary=True, name_label_dict=names_unordered_print,
                        obs_bigdata=int_cfg.obs_bigdata, classification=False, scaler=scaler
                        )
            all_y_name = [*var_cfg.y_name, *y_cent_name, *y_x_name]
            if fill_y_df is None:
                print_descriptive_df(gen_cfg, tree_add_y_lc_df, varnames=all_y_name, summary=False)
            else:
                print_descriptive_df(gen_cfg,
                                     pd.concat((tree_add_y_lc_df, fill_add_y_lc_df), axis=0),
                                     varnames=all_y_name, summary=False
                                     )
        lc_cfg.forests = forests_all
        var_cfg = adjust_y_names(var_cfg, gen_cfg, var_cfg.y_name, y_cent_name, summary=True)
        y_x_df = None
        ...
        y_x_np = np.zeros((len(x_tree_np), len(var_cfg.y_name)), dtype=y_0_dtype)
        if lc_cfg.forests == []:
            raise ValueError('Estimator used for centering was not saved for prediction.')
        ...
        for forest in lc_cfg.forests:   # Loops over training folds
            for idx, forest_y in enumerate(forest):  # Loops over outcomes
                scaler = forest_y[1]
                x_tree_np_ = x_tree_np if scaler is None else scaler.transform(x_tree_np)
    ...
    if fill_y_df is None:
        fill_add_y_lc_df = None
    if gen_cfg.with_output and gen_cfg.verbose and int_cfg.memory_print:
        print_mememory_statistics(gen_cfg, 'Local centering: End')

    return tree_add_y_lc_df, fill_add_y_lc_df, y_x_df, lc_r2_txt


def print_lc_header(gen_cfg: 'GenCfg', lc_cfg: 'LcCfg', title: str = '') -> None:
    """Print the header."""
...
def adjust_y_names(var_cfg: 'VarCfg',
                   gen_cfg: 'GenCfg',
                   y_name_old: list[str],
                   y_name_new: list[str],
                   summary: bool = True
