...
def weight_var_cuda(w0_dat: torch.Tensor,
                    ...
                    sum_tol: float = 1e-8,
                    residual_dat: torch.Tensor | None = None,
                    ) -> tuple[torch.Tensor, torch.Tensor | None, torch.Tensor]:
    """Generate the weight-based variance (CUDA version)."""
    mcf_c.require_torch()  # Raises error if torch import failed
    ...
    if p_cfg.cluster_std and (cl is not None) and (bootstrap < 1):
        if not weighted:
            weights = None
        w_dat, y_dat, _, _, residual_dat = aggregate_cluster_pos_w_cuda(cl, w_dat,
                                                                        y_dat=y_dat,
                                                                        r_dat=residual_dat,
                                                                        norma=normalize,
                                                                        sweights=weights,
    ...
    if residual_dat is not None and bootstrap < 2 and torch.any(residual_dat.abs() > zero_tol):
        # Versions estimator
        est = torch.dot(w_dat, y_dat)
        if se_yes:
            var = torch.dot(w_dat**2, residual_dat**2) # Variance formula for OLS & Ridge regression
    ...
    if keep_some_0 and not only_copy:
        ...
        if zeros_to_switch <= 2:
            ...
            if zeros_to_switch > 0:
                ...
                perm = torch.randperm(zero_idx.numel(), generator=g_cuda, device=device,
                                      )[:zeros_to_switch]
    ...
    if obs < 5:
        ...
        variance = torch.tensor(1.0, device=device, dtype=dtype,) if se_yes else None
    ...
    if se_yes:
        ...
        if bootstrap > 1:
            ...
            for b_idx in range(bootstrap):
                if p_cfg.cluster_std and (cl_rounded is not None) and not only_copy:
                    # Block bootstrap on clusters
                    idx_cl = torch.randint(low=0, high=obs_cl, size=(obs_cl,), generator=g_cuda,
                    ...
                    for cl_i in cl_boot:
                        ...
                        idx_i = torch.nonzero(mask, as_tuple=False,).view(-1)
                        if idx_i.numel() > 0:
                    ...
                    # IID bootstrap
                    idx = torch.randint(low=0, high=obs, size=(obs,), generator=g_cuda,
                                        device=device, dtype=torch.long,
                                        )
        ...
        # Analytic SE
            ...
            if p_cfg.cond_var:
                ...
                if p_cfg.knn:
                    ...
                    exp_y_cond_w, var_y_cond_w = moving_avg_mean_var_cuda(y_s, k,
                                                                          precision=precision,
                                                                          )
                else:
                    ...
                    exp_y_cond_w = nadaraya_watson_cuda(y_s, w_s, w_s,
                                                        p_cfg.nw_kern, band, zero_tol=zero_tol,
                                                        )
                    var_y_cond_w = nadaraya_watson_cuda((y_s - exp_y_cond_w) ** 2, w_s, w_s,
                                                        p_cfg.nw_kern, band,
                                                        zero_tol=zero_tol,
                                                         )
...
def aggregate_cluster_pos_w_cuda(cl_dat: torch.Tensor,
    ...
    r_dat : Tensor, shape (n,) or (n, 1). Residuals (optional).
    ...
    Returns
    -------
    w_agg : Tensor, shape (n_clusters,). Aggregated weights (possibly normalized).
    y_agg : Tensor or None, shape (n_clusters, q). Aggregated outcomes.
    w2_agg : Tensor or None, shape (n_clusters,). Second set of aggregated weights.
    y2_agg : Tensor or None, shape (n_clusters, q). Second set of aggregated outcomes.
    r_agg : Tensor or None, shape (n_clusters, 1). Second set of aggregated outcomes.
    """
    mcf_c.require_torch()
    ...
    if r_dat is None:
        ...
        r_agg = torch.zeros((n_clusters, 1), device=device, dtype=dtype,)
    ...
    for j, cl_val in enumerate(cluster_vals):
        ...
        if r_agg is not None:
            r_cluster = r_dat[in_cluster_pos, :]
            ...
            num = eff_w.unsqueeze(0) @ r_cluster
            r_agg[j, :] = (num / w_agg[j]).squeeze(0)
...
def moving_avg_mean_var_cuda(data: torch.Tensor,
    ...
    mcf_c.require_torch()
    if data.ndim != 1:
        raise TypeError('moving_avg_mean_var_cuda expects a 1d tensor.')
...
def kernel_density_y_cuda(y_dat: torch.Tensor,
    ...
    if y_dat.ndim == 1:
        ...
        raise TypeError('y_dat must be 1d or 2d.')
...
def analyse_weights_cuda(weights: torch.Tensor,
    ...
    share_largest_q = torch.zeros((no_of_treat, 3), device=device, dtype=float_dtype,)
    sum_larger = torch.zeros((no_of_treat, len(p_cfg.q_w)), device=device, dtype=float_dtype,)
    ...
    quantiles = torch.tensor([0.99, 0.95, 0.90], device=device, dtype=float_dtype,)
    few_obs_flag = False
    ...
    for j in range(no_of_treat):
        ...
        if n_pos > 0:
            ...
            if n_pos > 5:
                ...
                for i in range(3):
                    thr = qqq[i] - zero_tol
                    share_largest_q[j, i] = w_pos_abs[w_pos_abs >= thr].sum() * 100.0
    ...
    if ate:
        txt += mcf_ps.txt_weight_stat(nonzero=larger_0, equalzero=equal_0, mean_nonzero=mean_pos,
                                      std_nonzero=std_pos, gini_all=gini_all, gini_nonzero=gini_pos,
    ...
    return (larger_0, equal_0, mean_pos, std_pos, gini_all, gini_pos, share_largest_q, sum_larger,
            obs_larger, txt,
            )


def gini_coeff_pos_cuda(x_dat: torch.Tensor, zero_tol: float = 1e-10,) -> torch.Tensor:
    ...
    if (sss > zero_tol).item():
        ...
        ranks = torch.arange(1, n + 1, device=x_flat.device, dtype=x_flat.dtype,)
