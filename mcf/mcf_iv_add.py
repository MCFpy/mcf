...
def get_weights_iv_local(mcf_: 'ModifiedCausalForest', *,
    ...
    if no_1st_weights:
        ...
        weights_1st_dic, _ = mcf_w.get_weights_mp(data_df, forest_1st_dic,
                                                  reg_round=round_ == 'regular',
                                                  cf_cfg=mcf_1st.cf_cfg, ct_cfg=mcf_1st.ct_cfg,
                                                  gen_cfg=mcf_1st.gen_cfg, int_cfg=mcf_1st.int_cfg,
                                                  p_cfg=mcf_1st.p_cfg, var_cfg=mcf_1st.var_cfg,
                                                  gen_tv_cfg_yes=mcf_1st.gen_tv_cfg.yes,
                                                  )
    weights_redf_dic, _ = mcf_w.get_weights_mp(data_df, forest_redf_dic,
                                               reg_round=round_ == 'regular',
                                               cf_cfg=mcf_redf.cf_cfg, ct_cfg=mcf_redf.ct_cfg,
                                               gen_cfg=mcf_redf.gen_cfg, int_cfg=mcf_redf.int_cfg,
                                               p_cfg=mcf_redf.p_cfg, var_cfg=mcf_redf.var_cfg,
                                               gen_tv_cfg_yes=mcf_redf.gen_tv_cfg.yes,
                                               )
    ...
    if local_effects:
        ...
        weights_local_dic = scale_weights_1st_stage(iate_d_z, weights_redf_dic,
...
def scale_weights_1st_stage(effect_1st: NDArray[Any],
    ...
    if weight_as_sparse:
        ...
        weights = [weight.multiply(effect_1st_inv).tocsr() for weight in weights_redf_dic['weights']
                   ]
    else:
        ...
        for obs_i, weight_i in enumerate(w_redf):
            for treat_j, w_treat_j in enumerate(weight_i):
                ...
                if ate_estimation:
                    ...
                    weights[obs_i][treat_j][1] = w_treat_j[1] / effect_1st[obs_i]
    ...
    for fold in range(mcf_.cf_cfg.folds):
        for round_ in mcf_.cf_cfg.est_rounds:
            ...
            weights_1st_dic, _ = mcf_w.get_weights_mp(data_df, forest_1st_dic,
                                                      reg_round=round_ == 'regular',
                                                      cf_cfg=mcf_1st.cf_cfg, ct_cfg=mcf_1st.ct_cfg,
                                                      gen_cfg=mcf_1st.gen_cfg,
                                                      int_cfg=mcf_1st.int_cfg,
                                                      p_cfg=mcf_1st.p_cfg, var_cfg=mcf_1st.var_cfg,
                                                      gen_tv_cfg_yes=mcf_1st.gen_tv_cfg.yes,
                                                      )
            y_pot_f, _, _, _, txt_w_f = mcf_iate.iate_est_mp(
                 mcf_, weights_1st_dic, None,
                 reg_round=round_ == 'regular', iv_scaling=True,
                 x_tv_df=data_df[mcf_.var_cfg.x_name_tv] if mcf_.gen_tv_cfg.yes else None,
                 )
            if round_ == 'regular':
                y_pot_iate_1st_f = y_pot_f.copy()
                iate_1st_dic, _ = mcf_est.aggregate_pots(mcf_1st, y_pot_iate_1st_f, None,
                                                         txt=txt_w_f, effect_dic= iate_1st_dic,
                                                         fold=fold, title='IATE (1st stage',
                                                         )
            else:
                iate_eff_1st_dic, _ = mcf_est.aggregate_pots(mcf_1st,
                                                             (y_pot_iate_1st_f + y_pot_f) / 2,
                                                             None,
                                                             txt=txt_w_f,
                                                             effect_dic=iate_eff_1st_dic, fold=fold,
                                                             title='IATE eff (1st stage)',
                                                             )
    return iate_1st_dic, iate_eff_1st_dic


def bala_1st_redf(instances: tuple['ModifiedCausalForest', 'ModifiedCausalForest',
    ...
    _, y_pot_f, y_pot_var_f, txt_w_f = mcf_ate.ate_est(mcf_1st, data_df, weights_1st_dic,
                                                       balancing_test=True,
                                                       )
    # Aggregate Balancing results over folds
    bala_1st_dic, _ = mcf_est.aggregate_pots(mcf_1st, y_pot_f, y_pot_var_f,
                                          txt=txt_w_f, effect_dic=bala_1st_dic, fold=fold,
                                          title='Reduced form balancing check: ',
                                          )

    _, y_pot_f, y_pot_var_f, txt_w_f = mcf_ate.ate_est(mcf_redf, data_df, weights_redf_dic,
                                                       balancing_test=True,
                                                       )
    # Aggregate Balancing results over folds
    bala_redf_dic, _ = mcf_est.aggregate_pots(mcf_redf, y_pot_f, y_pot_var_f,
                                              txt=txt_w_f, effect_dic=bala_redf_dic, fold=fold,
                                              title='Reduced form balancing check: ',
                                              )
...
def ate_iv(instances: tuple['ModifiedCausalForest', 'ModifiedCausalForest',
           ...
           local_effects: bool = True,
           ) -> tuple[NDArray[Any], NDArray[Any], dict, dict, NDArray[Any], dict, NDArray[Any], dict
                      ]:
    ...
    w_ate_1st, y_pot_1st_f, y_pot_var_1st_f, txt_w_1st_f = mcf_ate.ate_est(mcf_1st, data_df,
                                                                           weights_1st_dic,
                                                                           iv=False,)
    # Reduced form for current fold
    (w_ate_redf, y_pot_redf_f, y_pot_var_redf_f, txt_w_redf_f) = mcf_ate.ate_est(mcf_redf, data_df,
                                                                                 weights_redf_dic,
                                                                                 iv=False,
                                                                                 )
    # Aggregate ATEs over folds
    ate_1st_dic, _ = mcf_est.aggregate_pots(mcf_1st, y_pot_1st_f, y_pot_var_1st_f,
                                           txt=txt_w_1st_f, effect_dic= ate_1st_dic, fold=fold,
                                           title='ATE (1st stage)',
                                           )
    ate_redf_dic, _ = mcf_est.aggregate_pots(mcf_redf, y_pot_redf_f, y_pot_var_redf_f,
                                             txt=txt_w_redf_f, effect_dic=ate_redf_dic, fold=fold,
                                             title='ATE (reduced form)',
                                             )
    if global_effects:
        ate_1st = y_pot_1st_f[:, 1] - y_pot_1st_f[:, 0]
        weights_global_dic = scale_weights_1st_stage(ate_1st, weights_redf_dic,
                                                     mcf_.int_cfg.weight_as_sparse,)
        w_late_global, y_pot_f_global, y_pot_var_f_global, txt_w_f_global = mcf_ate.ate_est(
            mcf_, data_df, weights_global_dic, iv=True,
            )
        # Aggregate ATEs over folds
        late_global_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_f_global, y_pot_var_f_global,
                                                    txt=txt_w_f_global, effect_dic=late_global_dic,
                                                    fold=fold, title='LATE(global)',
                                                    )
    else:
        w_late_global = late_global_dic = None

    if local_effects:
        w_late_local, y_pot_f_local, y_pot_var_f_local, txt_w_f_local = mcf_ate.ate_est(
            mcf_, data_df, weights_local_dic, iv=True,
            )
        # Aggregate ATEs over folds
        late_local_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_f_local, y_pot_var_f_local,
                                                   txt=txt_w_f_local, effect_dic=late_local_dic,
                                                   fold=fold, title='LATE(local)',
                                                   )
    else:
        w_late_local = late_local_dic = None

    return (w_late_global, w_late_local, late_global_dic, late_local_dic,
...
def bgate_iv(mcf_: 'ModifiedCausalForest',
    ...
    if local_effects:
        ...
        lbgate_local_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_lbgate_f, y_pot_var_lbgate_f,
                                                     txt=txt_w_f, effect_dic=lbgate_local_dic,
                                                     fold=fold, pot_is_list=True,
                                                     title=title+' (local)'
                                                     )
        if y_pot_mlate_lbgate_f is not None:
            lbgate_m_late_local_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_mlate_lbgate_f,
                                                                y_pot_mlate_var_lbgate_f,
                                                                txt=txt_w_f,
                                                                effect_dic=lbgate_m_late_local_dic,
                                                                fold=fold, pot_is_list=True,
                                                                title=title + ' minus LATE (local)',
                                                                )
    else:
    ...
    return (lbgate_global_dic, lbgate_local_dic, lbgate_m_late_global_dic, lbgate_m_late_local_dic,
            lbgate_est_global_dic, lbgate_est_local_dic, txt_b
            )
...
def gate_iv(mcf_: 'ModifiedCausalForest',
    ...
    if global_effects:
        ...
        #         title=title + ' minus LATE (local)'
        #         )
        raise NotImplementedError('LBGATEs, LCGATEs, and LGATEs are not implemented for global '
                                  'compliers.'
                                  )
    ...
    if local_effects:
        (y_pot_lgate_f, y_pot_var_lgate_f, y_pot_mlate_lgate_f,
         ...
         ) = mcf_gate.gate_est(mcf_, data_df, weights_local_dic, w_late_local, iv=True)

        lgate_local_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_lgate_f, y_pot_var_lgate_f,
                                                    txt=txt_w_f, effect_dic=lgate_local_dic,
                                                    fold=fold,
                                                    pot_is_list=True, title=title+' (local)'
                                                 )
        if y_pot_mlate_lgate_f is not None:
            lgate_m_late_local_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_mlate_lgate_f,
                                                               y_pot_mlate_var_lgate_f,
                                                               txt=txt_w_f,
                                                               effect_dic=lgate_m_late_local_dic,
                                                               fold=fold, pot_is_list=True,
                                                               title=title + ' minus LATE (local)',
                                                               )
    else:
    ...
    return (lgate_global_dic, lgate_local_dic, lgate_m_late_global_dic,
            lgate_m_late_local_dic, lgate_est_global_dic, lgate_est_local_dic
            )
...
def iate_iv(mcf_: 'ModifiedCausalForest', *,
            ...
            iv: bool = True,
            title: str = 'LIATE',
            x_tv_df: DataFrame | None = None,
            ) -> tuple[dict, dict, dict, list | NDArray[Any]]:
    """Compute IATEs."""
    (y_pot_f, y_pot_var_f, y_pot_m_ate_f, y_pot_m_ate_var_f,
     txt_w_f) = mcf_iate.iate_est_mp(mcf_, weights_dic, w_ate, reg_round=round_ == 'regular',
                                     iv=True, x_tv_df=x_tv_df if mcf_.gen_tv_cfg.yes else None,
                                     )
    if round_ == 'regular':
        y_pot_iate_f = y_pot_f.copy()
        y_pot_varf = None if y_pot_var_f is None else y_pot_var_f.copy()
        iate_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_iate_f, y_pot_varf,
                                             txt=txt_w_f, effect_dic=iate_dic, fold=fold,
                                             title=title,
                                             )
        if y_pot_m_ate_f is not None:
            titel2 = title + ' minus LATE' if iv else title + ' minus ATE'
            iate_m_ate_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_m_ate_f, y_pot_m_ate_var_f,
                                                       txt=txt_w_f, effect_dic=iate_m_ate_dic,
                                                       fold=fold, title=titel2,
                                                       )
    else:
        y_pot_eff = (y_pot_iate_f + y_pot_f) / 2
        iate_eff_dic, _ = mcf_est.aggregate_pots(mcf_, y_pot_eff, None,
                                                 txt=txt_w_f, effect_dic=iate_eff_dic, fold=fold,
                                                 title=title + ' eff',
                                                 )
