...
def mcf_mse(y_dat: NDArray[np.float64],
    ...
    if cuda and len(y_dat) > min_obs_for_cuda:
        mse_mce, treat_shares, no_of_obs_by_treat = mcf_mse_cuda(y_dat, y_nn, d_dat, w_dat,
                                                                 n_obs=n_obs, mtot=mtot,
                                                                 no_of_treat=no_of_treat,
                                                                 treat_values_list=treat_values,
                                                                 w_yes=w_yes,
                                                                 splitting=splitting,
                                                                 compare_only_to_zero=
                                                                     compare_only_to_zero,
                                                                 pen_mult=pen_mult, precision=32,
                                                                 zero_tol=zero_tol,
                                                                 )
    else:
        if w_yes or mtot in (2, 3):
            mse_mce, treat_shares, no_of_obs_by_treat = mcf_mse_not_numba(
                y_dat, y_nn, d_dat, w_dat,
                n_obs=n_obs, mtot=mtot, no_of_treat=no_of_treat, treat_values=treat_values,
                w_yes=w_yes, splitting=splitting, compare_only_to_zero=compare_only_to_zero,
                pen_mult=pen_mult, zero_tol=zero_tol,
                )
        else:
            if cython:
            ...
            mse_mce, treat_shares, no_of_obs_by_treat = mcf_mse_numba(
                y_dat, y_nn, d_dat,
                n_obs=n_obs, mtot=mtot, no_of_treat=no_of_treat,
                treat_values=np.array(treat_values, dtype=np.int8),
                compare_only_to_zero=compare_only_to_zero,
                pen_mult=pen_mult, penalty=pen_mult > 0, zero_tol=zero_tol,
                )
...
def mcf_mse_cuda(y_dat_np: NDArray[np.float64],
    ...
    # Move inputs to GPU
    y_dat, y_nn, d_dat, w_dat, treat_values = tensors_to_gpu(y_dat_np, y_nn_np, d_dat_np, w_dat_np,
                                                             treat_values_list=treat_values_list,
                                                             w_yes=w_yes,
                                                             )
    device = y_dat.device
    ...
    mse_mce = torch.zeros((no_of_treat, no_of_treat), device=device, dtype=float_dtype,)
    no_of_obs_by_treat = torch.zeros(no_of_treat, device=device, dtype=int_dtype,)
    for m_idx in range(no_of_treat):
        d_m = isclose(d_dat, treat_values[m_idx], atol=zero_tol, rtol=zero_tol)
        y_m = y_dat[d_m]

        if n_m := int(y_m.shape[0]) == 0:
        ...
        if w_yes:
            ...
            if pen_mult > 0:
                ...
                mse_m = mse_m + pen_mult * mean_t_w((d_mf - d_m_mean) ** 2, w_dat, dim=0,)
        else:
            no_of_obs_by_treat[m_idx] = n_m
            ...
            if pen_mult > 0:
                ...
                mse_m = mse_m + pen_mult * (torch.sum(d_mf ** 2) / n_obs - (d_m_mean ** 2))
        ...
        if (mtot != 3) and (m_idx == 0 or not compare_only_to_zero):
            for v_idx in range(m_idx + 1, no_of_treat):
                if mtot == 2:
                    # Variance of effects
                    d_l = isclose(d_dat, treat_values[v_idx], atol=zero_tol, rtol=zero_tol)
                    y_l = y_dat[d_l]
                    if y_l.shape[0] == 0:
                        continue
                    ...
                    d_ml = (isclose(d_dat, treat_values[v_idx], atol=zero_tol, rtol=zero_tol)
                            | isclose(d_dat, treat_values[m_idx], atol=zero_tol, rtol=zero_tol)
                            )
                    ...
                    if w_yes:
                        ...
                        if splitting and (no_of_treat == 2):
                            mce_ml = (mean_t_w(y_nn_m, w_ml, dim=0) * mean_t_w(y_nn_l, w_ml, dim=0)
                                      * (-1)
                                      )
                        else:
                            mce_ml = mean_t_w((y_nn_m - mean_t_w(y_nn_m, w_ml, dim=0))
    ...
    mse_mce_np, treat_shares_np, no_of_obs_by_treat_np, = tensors_to_np_to_cpu(mse_mce,
                                                                               treat_shares,
                                                                               no_of_obs_by_treat,
                                                                               )
...
def mcf_mse_not_numba(y_dat: NDArray[np.float64],
    ...
    for m_idx in range(no_of_treat):
        ...
        if w_yes:
            y_m, w_m = y_dat[d_m].reshape(-1), w_dat[d_m].reshape(-1)
            n_m = np.sum(w_m)
            no_of_obs_by_treat[m_idx] = n_m
            y_m_mean = np.average(y_m, weights=w_m)
            mse_m = np.average(np.square(y_m - y_m_mean), weights=w_m)
            if pen_mult > 0:
                w_dat_r = w_dat.reshape(-1)
                d_m_mean = np.average(d_m, weights=w_dat_r)
                mse_m += pen_mult * np.average(np.square(d_m - d_m_mean), weights=w_dat_r)
        else:
            n_m = len(y_dat[d_m])
            no_of_obs_by_treat[m_idx] = n_m
            ...
            if pen_mult > 0:
                ...
                mse_m += pen_mult * (np.sum(d_m) / n_m - (d_m_mean**2))
        if mtot in (1, 4):
            treat_shares[m_idx] = n_m / n_obs
        if mtot in (1, 3, 4):
            mse_mce[m_idx, m_idx] = mse_m
        if (mtot != 3) and (m_idx == 0 or not compare_only_to_zero):
            ...
            for v_idx in range(m_idx + 1, no_of_treat):
                if mtot == 2:  # Variance of effects mtot = 2
                    d_l = isclose(d_dat, treat_values[v_idx], atol=zero_tol, rtol=zero_tol)
                    if w_yes:
                        y_l_mean = np.average(y_dat[d_l], weights=w_dat[d_l], axis=0)
                    else:
                    ...
                    if w_yes:
                        w_ml = w_dat[d_ml].reshape(-1)
                        if splitting and (no_of_treat == 2):
                            mce_ml = ((np.average(y_nn_m, weights=w_ml, axis=0)) *
                                      (np.average(y_nn_l, weights=w_ml, axis=0)) * (-1)
                                      )
                        else:
                            mce_ml = np.average(
                                (y_nn_m - np.average(y_nn_m, weights=w_ml, axis=0))
                                * (y_nn_l - np.average(y_nn_l, weights=w_ml, axis=0)),
                                weights=w_ml, axis=0
                                )
                    else:
                        aaa = np.average(y_nn_m, axis=0) * np.average(y_nn_l, axis=0)
...
def mcf_mse_numba(y_dat: NDArray[np.float64],
    ...
    for m_idx in range(no_of_treat):
        ...
        for i in range(n_obs):
            if isclose(d_dat[i], tv, atol=zero_tol, rtol=zero_tol):
                v = y_col[i]
                n_m += 1
                sum_y += v
                sum_y2 += v * v
        ...
        if mtot_not_3 and (m_idx == 0 or not compare_only_to_zero):
            for v_idx in range(m_idx + 1, no_of_treat):
                ...
                if mtot_is_2:  # Variance of effects mtot = 2
                    ...
                    sum_y_v = 0.0
                    for i in range(n_obs):
                        if isclose(d_dat[i], tv2, atol=zero_tol, rtol=zero_tol):
                            sum_y_v += y_col[i]
                            n_v += 1
                    ...
                    for i in range(n_obs):
                        di = d_dat[i]
                        if (isclose(di, tv, atol=zero_tol, rtol=zero_tol)
                                or isclose(di, tv2, atol=zero_tol, rtol=zero_tol)):
                            ym = y_nn[i, m_idx]
                            yl = y_nn[i, v_idx]
                            n_ml += 1
                            sum_m += ym
    ...
    return mse_mce, treat_shares if mtot_in_134 else np.zeros(1), no_of_obs_by_treat
...
def add_mse_mce_split(*, mse_mce_l: NDArray[Any],
    ...
    for m_idx in range(no_of_treat):
        obs_m_idx = obs_by_treat[m_idx]
        if obs_m_idx > 0:
            mse_mce[m_idx, m_idx] = ((mse_mce_l[m_idx, m_idx] * obs_by_treat_l[m_idx]
