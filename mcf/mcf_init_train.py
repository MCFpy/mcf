...
from mcf.mcf_init_update_helper import get_alpha
...
class CsCfg:
    ...
    def from_args(cls,
        ...
        t = 0 if gen_cfg.d_type == 'continuous' else (type_ if type_ in (0, 1, 2) else 1)
        q = 1.0 if (quantil is None or not 0.0 <= quantil <= 1.0) else float(quantil)
        mp = 0.01 if (min_p is None or not 0.0 <= min_p <= 0.5) else float(min_p)
        mdt = (0.5 if max_del_train is None or not 0.0 < max_del_train <= 1.0
               else float(max_del_train)
               )
        dcs = detect_const_vars_stop if isinstance(detect_const_vars_stop, bool) else True
        paths: dict[str, Any] = {}
        if gen_cfg.outpath is not None:
            paths = mcf_sys.get_fig_path({}, gen_cfg.outpath, 'common_support', gen_cfg.with_output,
                                         no_csv=False
                                         )
        return cls(type_=t,
                   quantil=q,
...
class LcCfg:
    ...
    OK_ESTIMATORS: ClassVar[tuple[str, ...]] = (
        ...
        'AdaBoost200', 'GradBoost', 'GradBoostDepth6', 'GradBoostDepth12', 'LASSO',
        'NeuralNet', 'NeuralNetLarge', 'NeuralNetLarger', 'Mean', 'automatic'
        )
    ...
    def from_args(cls,
        ...
        share = (0.25 if (cs_share is None or not 0.0999 < float(cs_share) < 0.9001)
                 else float(cs_share)
                 )
        ...
        if estimator is None:
            ...
            try:
                ...
                print('Estimator specified for local centering is not valid.\n'
                      f'Specified estimator {estimator}.\nAllowed estimators: {" ".join(names)}'
                      )
...
class CfCfg:
    ...
    def from_args(cls,
        ...
        n_min_grid_i = 1 if n_min_grid is None or n_min_grid < 1 else round(n_min_grid)
        arg_grid_chk = 1 if alpha_reg_grid is None or alpha_reg_grid < 1 else round(alpha_reg_grid)
        ...
        # shares
        m_share_min_f = (0.1 if m_share_min is None or not (0 < m_share_min <= 1)
                         else float(m_share_min)
                         )
        m_share_max_f = (0.6 if m_share_max is None or not (0 < m_share_max <= 1)
        ...
        penalty_type_s = ('mse_d' if penalty_type is None or penalty_type != 'diff_d'
                          else penalty_type
                          )
        return cls(boot=boot_i,
