...
def train_main(mcf_: 'ModifiedCausalForest', data_df: pd.DataFrame) -> dict:
    ...
    if mcf_.dc_cfg.screen_covariates:   # Only training
        (mcf_.gen_cfg, mcf_.var_cfg, mcf_.var_x_type, mcf_.var_x_values,
         report
    ...
    if mcf_.cs_cfg.type_:
        (tree_df, fill_y_df, rep_sh_del, obs_remain, rep_fig,
         ...
         ) = mcf_cs.common_support_p_score(mcf_, tree_df, fill_y_df, train=True,
                                           p_score_only=False
                                           )
        if mcf_.gen_cfg.with_output:
    ...
    (mcf_.cf_cfg, mcf_.p_ba_cfg, mcf_.forest, time_vi, report
     ) = mcf_fo.train_forest(mcf_, tree_df, fill_y_df)
    ...
    if mcf_.gen_cfg.with_output:
        time_train = mcf_ps.print_timing(
            mcf_.gen_cfg, 'Training', time_string, time_difference, summary=True
            )
...
def predict_main(mcf_: 'ModifiedCausalForest',
    ...
    results : Dictionary.
        ...
        'common_support_probabilities: ': pd.DataFrame containing treatment
            ...
            and a dummy variable indicating whether the observation is inside
            or outside the common support.
    ...
    if mcf_.cs_cfg.type_:
        (data_df, _, report['cs_p_share_deleted'],
         ...
         ) = mcf_cs.common_support_p_score(mcf_, data_df, None, train=False,
                                           p_score_only=False
                                           )
    else:
        cs_pred_prob = None
    ...
    if mcf_.lc_cfg.yes and mcf_.lc_cfg.uncenter_po:
        (_, _, y_pred_x_df, _) = mcf_lc.local_centering(mcf_, data_df, None,
                                                        train=False
                                                        )
    else:
        y_pred_x_df = 0
    ...
    # Initialise dictionaries holding results
    ate_eff, gate_eff = mcf_.gen_cfg.ate_eff, mcf_.gen_cfg.gate_eff
    iate_eff, qiate_eff = mcf_.gen_cfg.iate_eff, mcf_.gen_cfg.qiate_eff

    ate_dic = [None, None] if ate_eff else [None]
    bala_dic = [None, None] if ate_eff else [None]
    bgate_dic = [None, None] if gate_eff else [None]
    bgate_m_ate_dic = [None, None] if gate_eff else [None]
    cbgate_dic = [None, None] if gate_eff else [None]
    cbgate_m_ate_dic = [None, None] if gate_eff else [None]
    gate_dic = [None, None] if gate_eff else [None]
    gate_m_ate_dic = [None, None] if gate_eff else [None]
    iate_dic = [None, None] if iate_eff else [None]
    iate_m_ate_dic = [None, None] if iate_eff else [None]
    qiate_dic = [None, None] if qiate_eff else [None]
    qiate_m_med_dic = [None, None] if qiate_eff else [None]
    qiate_m_opp_dic = [None, None] if qiate_eff else [None]

    gate_est_dic = bgate_est_dic = cbgate_est_dic = qiate_est_dic = None
    ...
    for fold in range(mcf_.cf_cfg.folds):
        for round_ in mcf_.cf_cfg.est_rounds:
            ...
            time_a_start = time()
            if round_ == 'regular' or ate_eff:
                # Estimate ATE n fold
                w_ate, y_pot_f, y_pot_var_f, txt_w_f = mcf_ate.ate_est(
                    mcf_, data_df, weights_dic,
                    )
                # Aggregate ATEs over folds
                ate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_f, y_pot_var_f, txt_w_f, ate_dic[idx_round],
                    fold, title='ATE'
                    )
            else:
                w_ate = None
            ...
            if round_ == 'regular' or ate_eff:
                if mcf_.p_cfg.bt_yes:
                    ...
                    bala_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_f, y_pot_var_f, txt_w_f,
                        bala_dic[idx_round], fold, title='Balancing check: '
                        )
            time_delta_bala += time() - time_b_start

            # BGATE
            time_bgate_start = time()
            if mcf_.p_cfg.bgate and (round_ == 'regular' or gate_eff):
                ...
                bgate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_bgate_f, y_pot_var_bgate_f, txt_w_f,
                    bgate_dic[idx_round], fold, pot_is_list=True, title='BGATE'
                    )
                if y_pot_mate_bgate_f is not None:
                    bgate_m_ate_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_mate_bgate_f, y_pot_mate_var_bgate_f,
                        txt_w_f, bgate_m_ate_dic[idx_round], fold,
                        pot_is_list=True, title='BGATE minus ATE')
            time_delta_bgate += time() - time_bgate_start

            # CBGATE
            time_cbg_start = time()
            if mcf_.p_cfg.cbgate and (round_ == 'regular' or gate_eff):
                (y_pot_cbgate_f, y_pot_var_cbgate_f, y_pot_mate_cbgate_f,
                 y_pot_mate_var_cbgate_f, cbgate_est_dic, txt_w_f, txt_am,
                 ) = mcf_gate.bgate_est(mcf_, data_df, weights_dic,
                                        w_ate, forest_dic, gate_type='CBGATE'
                                        )
                cbgate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_cbgate_f, y_pot_var_cbgate_f, txt_w_f,
                    cbgate_dic[idx_round], fold, pot_is_list=True,
                    title='CBGATE'
                    )
                if y_pot_mate_cbgate_f is not None:
                    cbgate_m_ate_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_mate_cbgate_f, y_pot_mate_var_cbgate_f,
                        txt_w_f, cbgate_m_ate_dic[idx_round], fold,
                        pot_is_list=True, title='CBGATE minus ATE'
                        )
            ...
            # IATE
            time_i_start = time()
            if mcf_.p_cfg.iate and (round_ == 'regular' or iate_eff):
                (y_pot_f, y_pot_var_f, y_pot_m_ate_f, y_pot_m_ate_var_f, txt_w_f
                 ) = mcf_iate.iate_est_mp(mcf_, weights_dic, w_ate,
                                          round_ == 'regular'
                                          )
                iate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_f, y_pot_var_f, txt_w_f, iate_dic[idx_round],
                    fold, title='IATE'
                    )
                if y_pot_m_ate_f is not None:
                    iate_m_ate_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_m_ate_f, y_pot_m_ate_var_f, txt_w_f,
                        iate_m_ate_dic[idx_round], fold, title='IATE minus ATE'
                        )
            ...
            # QIATE
            time_q_start = time()
            if mcf_.p_cfg.qiate and (round_ == 'regular' or qiate_eff):
                ...
                qiate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_qiate_f, y_pot_var_qiate_f, txt_w_f,
                    qiate_dic[idx_round], fold, pot_is_list=True, title='QIATE'
                    )
                if y_pot_mmed_qiate_f is not None:
                    qiate_m_med_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_mmed_qiate_f, y_pot_mmed_var_qiate_f,
                        txt_w_f, qiate_m_med_dic[idx_round], fold,
                        pot_is_list=True, title='QIATE minus QIATE(median))'
                        )
                if y_pot_mopp_qiate_f is not None:
                    qiate_m_opp_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_mopp_qiate_f, y_pot_mopp_var_qiate_f,
                        txt_w_f, qiate_m_opp_dic[idx_round], fold,
                        pot_is_list=True, title='QIATE(q) minus QIATE(1-q))'
                        )
            time_delta_qiate += time() - time_q_start  # QIATE

            # GATE
            time_g_start = time()
            if mcf_.p_cfg.gate and (round_ == 'regular' or gate_eff):
                ...
                gate_dic[idx_round] = mcf_est.aggregate_pots(
                    mcf_, y_pot_gate_f, y_pot_var_gate_f, txt_w_f,
                    gate_dic[idx_round], fold, pot_is_list=True, title='GATE'
                    )
                if y_pot_mate_gate_f is not None:
                    gate_m_ate_dic[idx_round] = mcf_est.aggregate_pots(
                        mcf_, y_pot_mate_gate_f, y_pot_mate_var_gate_f,
                        txt_w_f, gate_m_ate_dic[idx_round], fold,
    ...
    ate_dic = mcf_est.average_vals(ate_dic)
    bala_dic = mcf_est.average_vals(bala_dic)
    gate_dic = mcf_est.average_vals(gate_dic)
    gate_m_ate_dic = mcf_est.average_vals(gate_m_ate_dic)

    bgate_dic = mcf_est.average_vals(bgate_dic)
    bgate_m_ate_dic = mcf_est.average_vals(bgate_m_ate_dic)
    cbgate_dic = mcf_est.average_vals(cbgate_dic)
    cbgate_m_ate_dic = mcf_est.average_vals(cbgate_m_ate_dic)

    qiate_dic = mcf_est.average_vals(qiate_dic)
    qiate_m_med_dic = mcf_est.average_vals(qiate_m_med_dic)
    qiate_m_opp_dic = mcf_est.average_vals(qiate_m_opp_dic)

    iate_dic = mcf_est.average_vals(iate_dic)
    iate_m_ate_dic = mcf_est.average_vals(iate_m_ate_dic)
...
def analyse_main(mcf_: 'ModifiedCausalForest', results: dict) -> dict:
    ...
    if (mcf_.gen_cfg.with_output and mcf_.post_cfg.est_stats and
            mcf_.gen_cfg.return_iate_sp):
        time_start = time()
        ...
        mcf_ps.print_mcf(mcf_.gen_cfg, mcf_.time_strings['time_train'],
                         summary=True)
        mcf_ps.print_mcf(mcf_.gen_cfg, mcf_.time_strings['time_pred'],
                         summary=True)
        mcf_ps.print_timing(mcf_.gen_cfg, 'Analysis of IATE', time_string,
...
# The blinder methods are no longer used, thus no annotations.
...
#     warn('This method of reducing dependence on protected variables is '
#          'deprecated. Use the method fairscores of the OptimalPolicy class '
#          'instead. This method will be removed from the mcf package.',
#          DeprecationWarning
#          )
