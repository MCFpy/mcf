...
from mcf.mcf_estimation_generic import bandwidth_nw_rule_of_thumb
from mcf.mcf_estimation_generic import kernel_proc
from mcf.mcf_general import bound_norm_weights, bound_norm_weights_not_one, progress_clean_memory
from mcf.mcf_weight import sparse_row_indices_weights
...
def qiate_est(mcf_: 'ModifiedCausalForest',
    ...
    if gen_cfg.d_type == 'continuous':
        raise NotImplementedError('QIATE only implemented for discrete treatments. Maybe (!) '
                                  'support for continuous treatments will be added in the future.'
                                  )
    y_dat, weights_all, w_dat, cl_dat = assign_data(weights_dic, gen_cfg, p_cfg)
    d_p, _, w_p, _ = mcf_ate.get_data_for_final_ate_estimation(data_df,
    ...
    iate_q_np, iate_q_key_dic = get_iate_order(y_pot, y_pot_var, no_of_treat, no_of_out, p_cfg)
    if p_cfg.qiate_smooth:
        bandwidth_nw, kernel_nw = get_nadarya_watson_parameters(iate_q_np[:, 0, 0],
    ...
    # comparisons
    w_qiate, w_qiate_unc, w_censored, w_median = initialise_weights_qiate(no_of_q_values,
                                                                          len(iate_q_key_dic),
                                                                          n_y, no_of_out
                                                                          )
    # Optimisations concerning the use of the weights matrix and ray.
    files_to_delete, _, weights_all, maxworkers, txt = optimize_initialize_mp_qiate(weights_all,
                                                                                    gen_cfg,
                                                                                    int_cfg,
                                                                                    with_output
                                                                                    )
    ...
    parameters = (i_d_val, t_probs, no_of_out, bandwidth_nw, kernel_nw, p_cfg.qiate_m_mqiate,
                  p_cfg.qiate_m_opp,
                  )
    ...
    if p_cfg.qiate_m_mqiate:
        _, _, _, _, _, _, w_median, _, _, _ = qiate_q(0.5, 0, data, weights_all,
                                                      iate_q=iate_q,
                                                      qiate_weights_q=qiate_weights_q,
                                                      pot_q=None, dics=dics,
                                                      parameters=parameters_diff,
                                                      iv=iv, weights_only=True, zero_tol=zero_tol,
                                                      )
    else:
    ...
    kw_args = {'iate_q': iate_q, 'pot_q': None,  'dics': dics, 'parameters': parameters_diff,
               'iv': iv, 'weights_only': True, 'zero_tol': zero_tol,
               }
    if p_cfg.qiate_m_opp:
        if maxworkers == 1:
            for q_idx, q_value in enumerate(q_values):
                qiate_weights_q = w_qiate[q_idx], w_qiate_unc[q_idx], w_censored[q_idx], None, None
                (_, _, _, _, _, _, w_mopp_list[q_idx], _, _, _
                 ) = qiate_q(q_value, q_idx, data, weights_all,
                             **kw_args,
                             qiate_weights_q=qiate_weights_q,
                             )
        else:
            if int_cfg.mp_use_old_ray:
                if ray is None or ray_qiate_mp is None:
                ...
                data_ref = mcf_ray.ray_put_all(data)
                still_running = [ray_qiate_mp.remote(q_value, q_idx, data_ref, weights_all,
                                                     **kw_args,
                                                     qiate_weights_q=(w_qiate_dim, w_qiate_unc_dim,
                                                                      w_censored_dim, None, None),
                                                     )
                                 for q_idx, q_value in enumerate(q_values)
                                 ]
                idx = 1
                ...
                while len(still_running) > 0:
                    ...
                    for results_fut_idx in finished_res:
                        ...
                        w_qiate0, w_qiate_unc0, w_censored0 = assign_w_q(w_qiate0, w_qiate_unc0,
                                                                         w_censored0,
                                                                         results_fut_idx,
                                                                         q_idx
                                                                         )
                        w_mopp_list[q_idx] = w_qiate0[q_idx].copy()
                        progress_clean_memory(output=gen_cfg.with_output and gen_cfg.verbose,
                                              current_idx=idx, total=no_of_q_values,
                                              )
                        idx += 1
            else:
                ray_err_txt = 'Ray does not start in QIATE estimation.'
                fail_txt = 'Failed to make executor in QIATE estimation.'
                with forest_executor_with_shared(int_cfg=int_cfg, maxworkers=maxworkers,
                                                 shared_obj=data, shared_name='qiate_data',
                                                 ray_err_txt=ray_err_txt, fail_txt=fail_txt,
                                                 ) as (executor, data_handle, maxworkers):
                    tasks = [TaskSpec(func=qiate_mp_backend,
                                      kwargs={'q_value': q_value,
                                              'q_idx': q_idx,
                                              'data': data_handle,
                    ...
                    for results_fut_idx in map_task_batches(executor=executor, tasks=tasks,
                        ...
                        progress_clean_memory(output=gen_cfg.with_output and gen_cfg.verbose,
                                              current_idx=idx, total=len(q_values),
                                              )
                        idx += 1
    ...
    kw_args = {'iate_q': iate_q, 'dics': dics, 'parameters': parameters, 'iv': iv,
               'zero_tol': zero_tol,
               }
    if maxworkers == 1:
        for q_idx, q_value in enumerate(q_values):
            q_idx_opp = no_of_q_values - 1 - q_idx
            qiate_weights_q = (w_qiate[q_idx], w_qiate_unc[q_idx], w_censored[q_idx], w_median,
                               w_mopp_list[q_idx_opp],
                               )
            if p_cfg.qiate_m_mqiate:
            ...
            results_fut_q = qiate_q(q_value, q_idx, data, weights_all,
                                    qiate_weights_q=qiate_weights_q, pot_q=pot_q,
                                    **kw_args,
                                    )
            y_pot, y_pot_var, y_pot_mmed, y_pot_mmed_var, y_pot_mopp, y_pot_mopp_var = assign_pot_q(
                 y_pot, y_pot_var,
                 y_pot_mmed=y_pot_mmed, y_pot_mmed_var=y_pot_mmed_var, y_pot_mopp=y_pot_mopp,
                 y_pot_mopp_var=y_pot_mopp_var, results_fut_q=results_fut_q, q_idx=q_idx,
                 qiate_m_mqiate=p_cfg.qiate_m_mqiate, qiate_m_opp=p_cfg.qiate_m_opp,
                 )
            w_qiate, w_qiate_unc, w_censored = assign_w_q(w_qiate, w_qiate_unc, w_censored,
                                                          results_fut_q, q_idx,
                                                          )
    else:
        ...
        if int_cfg.mp_use_old_ray:
            ...
            pot_q = (y_pot_dim, y_pot_var_dim, y_pot_mmed_dim, y_pot_mmed_var_dim, y_pot_mopp_dim,
                     y_pot_mopp_var_dim,
                     )
            still_running = [ray_qiate_mp.remote(q_value, q_idx, data_ref, weights_all,
                                                 pot_q=pot_q,
                                                 qiate_weights_q=(w_qiate_dim, w_qiate_unc_dim,
                                                                  w_censored_dim, w_median,
                                                                  w_mopp_list[no_of_q_values - 1
                                                                              - q_idx],),
                                                 weights_only=False,
                                                 **kw_args,
                                                 )
            ...
            while len(still_running) > 0:
                ...
                for results_fut_idx in finished_res:
                    q_idx = results_fut_idx[8]
                    (y_pot, y_pot_var, y_pot_mmed, y_pot_mmed_var, y_pot_mopp, y_pot_mopp_var
                     ) = assign_pot_q(y_pot, y_pot_var,
                                      y_pot_mmed=y_pot_mmed, y_pot_mmed_var=y_pot_mmed_var,
                                      y_pot_mopp=y_pot_mopp, y_pot_mopp_var=y_pot_mopp_var,
                                      results_fut_q=results_fut_idx, q_idx=q_idx,
                                      qiate_m_mqiate=p_cfg.qiate_m_mqiate,
                                      qiate_m_opp=p_cfg.qiate_m_opp,
                                      )
                    w_qiate, w_qiate_unc, w_censored = assign_w_q(w_qiate, w_qiate_unc, w_censored,
                                                                  results_fut_idx, q_idx
                                                                  )
            ...
            *_, finished_res, finished = mcf_ray.ray_del_refs(f1=finished_res, f2=finished,
            ...
            ray_err_txt='Ray does not start in full QIATE estimation.'
            fail_txt='Failed to make executor in full QIATE estimation.'
            with forest_executor_with_shared(int_cfg=int_cfg,
                                             maxworkers=maxworkers,
                                             shared_obj=data,
                                             shared_name='qiate_full_data',
                                             ray_err_txt=ray_err_txt, fail_txt=fail_txt,
                                             ) as (executor, data_handle, maxworkers):
                tasks = [TaskSpec(func=qiate_mp_full_backend,
                                  kwargs={'q_value': q_value,
                                          'q_idx': q_idx,
                                          'data': data_handle,
                                          'weights_all': weights_all,
                                          ...
                                          'pot_q': (y_pot_dim, y_pot_var_dim, y_pot_mmed_dim,
                                                    y_pot_mmed_var_dim, y_pot_mopp_dim,
                                                    y_pot_mopp_var_dim,),
                                          'kw_args': kw_args,
                                          },
                                  name=f'qiate_full_{q_idx}',
                                  )
                         for q_idx, q_value in enumerate(q_values)
                         ]
                idx = 1
    ...
    qiate_est_dic = {'d_values': gen_cfg.d_values,
                     'no_of_out': no_of_out,
                     'var_cfg': var_cfg,
                     'q_values': q_values,
                     'ref_pop_lab': 'All',
    ...
    return (y_pot, y_pot_var, y_pot_mmed, y_pot_mmed_var, y_pot_mopp, y_pot_mopp_var, qiate_est_dic,
            txt
            )


def qiate_mp_backend(*, q_value: Any,
                     q_idx: int,
                     data: Any,
                     weights_all: Any,
                     kw_args: dict[str, Any],
                     qiate_weights_q: tuple[Any, Any, Any, Any, Any],
                     ) -> Any:
    """Backend-agnostic wrapper for one QIATE task."""
    return qiate_q(q_value, q_idx, data, weights_all,
                   **kw_args,
                   qiate_weights_q=qiate_weights_q,
                   multi_ray=True,
                   )
...
def qiate_mp_full_backend(*, q_value: Any,
                          q_idx: int,
                          data: Any,
                          weights_all: Any,
                          qiate_weights_q: tuple[Any, Any, Any, Any, Any],
                          pot_q: tuple[Any, Any, Any, Any, Any, Any],
                          kw_args: dict[str, Any],
                          ) -> Any:
    """Backend-agnostic wrapper for one full QIATE task."""
    return qiate_q(q_value, q_idx, data, weights_all, qiate_weights_q=qiate_weights_q, pot_q=pot_q,
                   weights_only=False, **kw_args, multi_ray=True,
                   )


# @ray.remote
def _ray_qiate_mp_impl(q_value: list[float] | NDArray[np.floating | float],
                       q_value_idx: Any,
    ...
    return qiate_q(q_value, q_value_idx, data, weights_all,
                   iate_q=iate_q, qiate_weights_q=qiate_weights_q, pot_q=pot_q, dics=dics,
                   parameters=parameters, multi_ray=True, iv=iv, weights_only=weights_only,
                   zero_tol=zero_tol,
                   )


ray_qiate_mp = ray.remote(_ray_qiate_mp_impl) if ray is not None else None
...
def qiate_q(q_value: list[float] | NDArray[np.floating | float],
    ...
    i_d_val, t_probs, no_of_out, bandw_q, kernel, qiate_m_mqiate, qiate_m_opp = parameters
    ...
    if multi_ray:  # Needed because object is otherwise 'read_only'
        w_qiate_q_dim, w_qiate_unc_q_dim, w_censored_q_dim, w_median, w_opp = qiate_weights_q
        w_qiate_q = np.zeros(w_qiate_q_dim)
        w_qiate_unc_q = np.zeros(w_qiate_unc_q_dim)
        w_censored_q = np.zeros(w_censored_q_dim)
        if not weights_only:
            (y_pot_q_dim, y_pot_var_q_dim, y_pot_mmed_q_dim, y_pot_mmed_var_q_dim, y_pot_mopp_q_dim,
             y_pot_mopp_var_q_dim,) = pot_q
        ...
        w_qiate_q, w_qiate_unc_q, w_censored_q, w_median, w_opp = qiate_weights_q

        if not weights_only:
            (y_pot_q, y_pot_var_q, y_pot_mmed_q, y_pot_mmed_var_q, y_pot_mopp_q, y_pot_mopp_var_q
             ) = pot_q
            ...
            for n_idx in range(n_x):               # relevant n_p observations
                for t_idx in range(2):             # Both relevant treatments
                    ...
                    if int_cfg.weight_as_sparse:
                        ...
                        # weight_i = weights_q[comp_idx][out_idx][t_idx].getrow(n_idx).tocsr()
                        # w_index = weight_i.indices.copy()
                        # w_i = weight_i.data.copy()
                        w_mat = weights_q[comp_idx][out_idx][t_idx]
                        w_index, w_i = sparse_row_indices_weights(w_mat, n_idx)
                    else:  # Ind weights > 0
                        w_index = weights_q[comp_idx][out_idx][n_idx][t_idx][0].copy()
                        w_i = weights_q[comp_idx][out_idx][n_idx][t_idx][1].copy()
                    if gen_cfg.weighted:
                        w_i = w_i * w_dat[w_index].reshape(-1)
                    w_i_sum = np.sum(w_i)
                    if abs(w_i_sum) > zero_tol and not 1-sum_tol < w_i_sum < 1+sum_tol:
                        w_i = w_i / w_i_sum
                    if gen_cfg.weighted:
                        w_i = w_i * w_p_q[n_idx]
                    ...
                    if p_cfg.choice_based_sampling:
                        ...
                        i_pos = i_d_val[np.isclose(d_p[n_idx], d_values[treat],
    ...
    # Step 2: Get potential outcomes for particular q_value
            ...
            for t_idx in range(2):
                w_median_ = w_median[comp_idx, t_idx, :, out_idx] if w_median is not None else None
                w_opp_ = w_opp[comp_idx, t_idx, :, out_idx] if w_opp is not None else None

                (w_qiate_q[comp_idx, t_idx, :, out_idx], w_diff_med, w_diff_opp,
                 w_censored_q[comp_idx, t_idx, out_idx], w_qiate_unc_q[comp_idx, t_idx, :, out_idx]
                                  ...
                                  w_mmed=w_median_, w_mopp=w_opp_,
                                  compute_w_diff_med=qiate_m_mqiate,
                                  compute_w_diff_opp=qiate_m_opp,
                                  max_weight_share=p_cfg.max_weight_share,
                                  iv=iv, zero_tol=int_cfg.zero_tol, sum_tol=int_cfg.sum_tol,
                                  p_ba_yes=p_ba_cfg.yes,
                                  )
                if not weights_only:
                    ret = mcf_est.weight_var(
                        w_qiate_q[comp_idx, t_idx, :, out_idx],
                        y_dat[:, out_idx], cl_dat, p_cfg,
                        residual_dat=None,
                        weighted=gen_cfg.weighted, weights=w_dat, bootstrap=p_cfg.se_boot_qiate,
                    ...
                    if qiate_m_mqiate:
                        ret2 = mcf_est.weight_var(w_diff_med, y_dat[:, out_idx], cl_dat, p_cfg,
                                                  residual_dat=None,
                    ...
                    if qiate_m_opp:
                        ret2 = mcf_est.weight_var(w_diff_opp, y_dat[:, out_idx], cl_dat, p_cfg,
    ...
    if weights_only:
        return (None, None, None, None, None, None, w_qiate_q, w_qiate_unc_q, q_value_idx,
                w_censored_q
                )
    ...
    return (y_pot_q, y_pot_var_q, y_pot_mmed_q, y_pot_mmed_var_q, y_pot_mopp_q, y_pot_mopp_var_q,
            w_qiate_q, w_qiate_unc_q, q_value_idx, w_censored_q
            )


def assign_pot_q(y_pot: NDArray[any],
    ...
    return y_pot, y_pot_var, y_pot_mmed, y_pot_mmed_var, y_pot_mopp, y_pot_mopp_var


def assign_w_q(w_qiate: NDArray[Any],
...
def w_qiate_func(sum_w_qiate: NDArray[Any],
    ...
    if max_weight_share < 1:
        if iv:
            w_qiate_q, _, w_censored_q = bound_norm_weights_not_one(w_qiate_q, max_weight_share,
                                                                    zero_tol=zero_tol,
                                                                    sum_tol=sum_tol,
                                                                    )
        else:
            w_qiate_q, _, w_censored_q = bound_norm_weights(w_qiate_q,
...
def get_w_rel_q(iate_q_np: NDArray[Any],
    ...
    weights = [[None for _ in range(no_of_outcomes)] for _ in range(no_of_comparisons)]
    w_q = [[None for _ in range(no_of_outcomes)] for _ in range(no_of_comparisons)]
    relevant_data = [[None for _ in range(no_of_outcomes)] for _ in range(no_of_comparisons)]

    if ray is not None and isinstance(weights_all_ref, ray.ObjectRef):
    ...
    for value in iate_q_key_dic.values():   # Treatment comparisons
        for out_idx in range(no_of_outcomes):
            ...
            if w_is_csr:
                weights[value[0]][out_idx] = [weights_all[t_idx][relevant_data_points, :]
...
def initialise_weights_qiate(no_of_qval: int,
...
def optimize_initialize_mp_qiate(weights_all: Any,
    ...
    if gen_cfg.mp_parallel < 1.5:
        ...
        if gen_cfg.mp_automatic:
            maxworkers = mcf_sys.find_no_of_workers(gen_cfg.mp_parallel, gen_cfg.sys_share / 2,
                                                    zero_tol=int_cfg.zero_tol,
                                                    )
        else:
    ...
    #                                        ray_err_txt
    #                                            ='Ray does not start up in QIATE estimation.'
    #                                        )
    #     mcf_ray.print_object_store(gen_cfg, int_cfg.mem_object_store_3)
    #     weights_all_ref = mcf_ray.ray_put_all(weights_all)
    # else:
...
def assign_data(weights_dic: dict,
                gen_cfg: 'GenCfg',
                p_cfg: dict,
...
def get_comparison_labels(no_of_treat: int,
    ...
    jdx = 0
    treat_comp_label = [None for _ in range(round(no_of_treat * (no_of_treat - 1) / 2))]
...
def get_iate_order(y_pot: NDArray[Any],
    ...
    obs = len(y_pot)
    quantile_key_dic = {}
    quantile_np = np.empty((obs, round(no_of_treat * (no_of_treat - 1) / 2), no_of_out))
    ...
    for t1_idx in range(no_of_treat - 1):
        ...
        for t2_idx in range(t1_idx + 1, no_of_treat):
            ...
            for o_idx in range(no_of_out):
                ...
                quantile_np[:, running_idx, o_idx] = (np.argsort(np.argsort(iate)) + 0.5) / obs

                if p_cfg.qiate_bias_adjust:
                    iate_se = np.sqrt(y_pot_var[:, t2_idx, o_idx] + y_pot_var[:, t1_idx, o_idx])
...
def bias_adjust_qiate(iate_quantile_np: NDArray[Any],
    ...
    OLD_SIMULATION = False                          # pylint: disable=C0103
    ...
    if simex:
        ...
        for g_idx, noise_level in enumerate(grid):
            iate_est_simul = iate_np.reshape(-1, 1) + est_error_simul_np * np.sqrt(noise_level)
        ...
        rank_iate_adj_np = simex_diff_rank_adjustment(iate_rank_np, iate_se_np,
                                                      iate_simul_rank_mean, grid
                                                      )
        ...
        iate_quantile_adj_np = np.clip(
            delta_quantile_adjustment(iate_quantile_np, iate_simul_rank_mean / obs),
            0, 1
            )
    else:
        iate_bias_adjust_np = bias_adjust_value(iate_np, iate_quantile_np, iate_se_np,)
...
def bias_adjust_value(iate_np: NDArray[Any],
    ...
    obs = len(iate_np)
    bias = norm.ppf(iate_quantile_np.clip(min=tolerance_0_1, max=1 - tolerance_0_1),
                    loc=np.zeros(obs),
                    scale=iate_se_np,
                    )
...
def simex_diff_rank_adjustment(iate_est_rank: NDArray[Any],
    ...
    for gidx, noise_level in enumerate(grid):
        ...
        x_var[:, :, gidx] = make_x_simex_diff(x_dim, ones, iate_est_se, noise_level)
...
def qiate_effects_print(mcf_: 'ModifiedCausalForest',
    ...
    try:
        ...
        for o_idx in range(no_of_out):
            if gen_cfg.with_output:
                txt += '\n' + '-' * 100 + f'\nOutcome variable: {var_cfg.y_name[o_idx]}      '
            ...
            for q_idx, _ in enumerate(q_values):
                ret = mcf_est.effect_from_potential(y_pot[q_idx][:, :, o_idx],
                                                    y_pot_var[q_idx][:, :, o_idx],
                                                    gen_cfg.d_values, continuous=continuous,
                                                    sequential=True,
                ...
                if m_med_yes:
                    ret = mcf_est.effect_from_potential(y_pot_m_med[q_idx][:, :, o_idx],
                                                        y_pot_m_med_var[q_idx][:, :, o_idx],
                                                        gen_cfg.d_values, continuous=continuous,
                                                        sequential=True,
                                                        sequential_dic
                                                            =qiate_est_dic['iate_q_key_dic']
                                                        )
                ...
                if m_opp_yes:
                    ret = mcf_est.effect_from_potential(y_pot_m_opp[q_idx][:, :, o_idx],
            ...
            if gen_cfg.with_output:
                ...
                txt += mcf_ps.print_effect_z(ret_qiate, ret_qiate_mmed, q_values, 'QIATE', 'ATE',
                                             print_output=False, gates_minus_previous=False,
                                             qiate=True,
                                             gmopp_r=ret_qiate_mopp, z_values_org=None,
                                             gen_tv_cfg_yes=mcf_.gen_tv_cfg.yes,
                                             )
                txt += '\n' + mcf_ps.print_se_info(p_cfg.cluster_std, p_cfg.se_boot_gate)
                txt += mcf_ps.print_minus_ate_info(gen_cfg.weighted, print_it=False)
        med_f = med_f_se = None
        if gen_cfg.with_output:
                    ...
                    for e_idx, e_lab in enumerate(effect_type_label):
                        ...
                        if not continuous and effects is not None and e_idx != 2:
                            figure_disc = make_qiate_figures_discr(
                                e_lab + ' ' + ' ' + o_lab + ' ' + t_lab, q_values, effects, ste,
                                int_cfg=int_cfg, p_cfg=p_cfg, med= med_f, med_se=med_f_se,
                                cdf_pdf=cdf_pdf)

                        figure_file = figure_disc if figure_cont is None else figure_cont
                        # vs0 = int(t_lab[-1]) == int(gen_cfg.d_values[0])
                        vs0 = np.isclose(int(t_lab[-1]), gen_cfg.d_values[0],
                                         atol=int_cfg.zero_tol, rtol=int_cfg.zero_tol,
                                         )
                        if vs0 and e_idx != 2 and (
                                ((m_med_yes and e_idx == 1) or (not m_med_yes and e_idx == 0))):
    ...
    return (qiate, qiate_se, qiate_mmed, qiate_mmed_se, qiate_mopp, qiate_mopp_se, figure_list)


def make_qiate_figures_discr(titel: str,
                             q_values: list,
                             effects: NDArray[Any],
    ...
    file_name_f_jpeg = p_cfg.paths['qiate_fig_pfad_jpeg'] / f'{titel_f}fill.jpeg'
    ...
    if cdf_pdf:
        ...
        if with_se:
            axs.fill_between(q_values, upper, lower, alpha=0.3, color='b', label=label_ci)
        line_med = '-r'
        if med is not None:
            label_med = 'QIATE(0.5)'
            if med_se is not None and with_se:
                axs.fill_between(q_values, med_upper.reshape(-1), med_lower.reshape(-1),
    ...
    if not cdf_pdf and with_se:
        ...
        axe.legend(loc=int_cfg.legend_loc, shadow=True, fontsize=int_cfg.fontsize)
        titel_tmp = titel[:-4] + ' ' + titel[-4:]
        titel_tmp = titel_tmp.replace('vs', ' vs ')
        ...
        if med_se is not None:
            ...
            effects_et_al = np.concatenate((upper, effects, lower, med, med_upper, med_lower,
                                            q_values_np),
                                           axis=1,
                                           )
            cols = ['upper', 'QIATE(q)', 'lower', 'QIATE(0.5)', 'med_upper', 'med_lower', 'q_values'
                    ]
        else:
            cols = ['upper', 'QIATE(q)', 'lower', 'QIATE(0.5)', 'q_values']
            effects_et_al = np.concatenate((upper, effects, lower, med, q_values_np), axis=1)
