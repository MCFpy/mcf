...
if TYPE_CHECKING:
    ...
    from mcf.mcf_init import GenCfg, IntCfg, VarCfg, PCfg
...
def ate_est(mcf_: 'ModifiedCausalForest',
    ...
    gen_tv_cfg, p_ba_cfg = mcf_.gen_tv_cfg, mcf_.p_ba_cfg
    ...
    if gen_cfg.with_output and gen_cfg.verbose and print_output:
        if balancing_test:
            ...
            print('\n\nComputing average outcomes for all-in-1-treatment allocations')
        else:
            print('\n\nComputing ATEs')
    ...
    if int_cfg.weight_as_sparse:
        for i in range(n_p):
            ...
            for t_ind, _ in enumerate(d_values):
                ...
                #     if gen_tv_cfg.yes:
                #         i_pos = ind_d_val[isclose(d_p[i, 0], d_values, atol=zero_tol,
                #                                   rtol=zero_tol)
                #                           ]
                #     else:
                #         i_pos = ind_d_val[isclose(d_p[i], d_values, atol=zero_tol, rtol=zero_tol)]
            ...
            # if p_cfg.atet:
            #     if gen_tv_cfg.yes:
            #         # w_ate[ind_d_val[d_p[i, 0] == d_values]+1, :, :] += w_add
            #         w_ate[ind_d_val[isclose(d_p[i, 0], d_values, atol=zero_tol, rtol=zero_tol)]+1,
            #               :, :] += w_add
            #     else:
            #         # w_ate[ind_d_val[d_p[i] == d_values]+1, :, :] += w_add
            #         w_ate[ind_d_val[isclose(d_p[i], d_values, atol=zero_tol, rtol=zero_tol)]+1,
            #               :, :] += w_add
            if p_cfg.atet:
                d_value = d_p[i, 0] if gen_tv_cfg.yes else d_p[i]
        ...
        for i, weight_i in enumerate(weights):
            ...
            for t_ind, _ in enumerate(d_values):
                ...
                # if p_cfg.choice_based_sampling:
                #     i_pos = ind_d_val[isclose(d_p[i], d_values, atol=zero_tol, rtol=zero_tol)]
                #     w_add[t_ind, weight_i[t_ind][0]] = w_i * t_probs[int(i_pos)]
                # else:
    ...
    if p_ba_cfg.yes and no_of_ates > 1 and p_ba_cfg.adj_method == 'w_obs':
        weights_eval = mcf_ba.get_weights_eval_ba(w_ate, no_of_treat, zero_tol=int_cfg.zero_tol)
    else:
        weights_eval = None
        ...
        for ta_idx in range(no_of_treat):
            if not iv and (-zero_tol < sumw[a_idx, ta_idx] < zero_tol):
                ...
                if w_ate_only:
                    ...
                    txt += (f'\nATE weights: {w_ate[a_idx, ta_idx, :]} ATE weights are all zero. '
                            'Not good. Redo statistic without this variable. '
            ...
            if p_ba_cfg.yes:
                w_ate[a_idx, ta_idx, :] = mcf_ba.bias_correction_wregr(
                    w_ate[a_idx, ta_idx, :],
                    y_dat[:, 0],
                    ba_data,
                    int_dtype=np.float64, out_dtype=np.float32,
                ...
                if continuous:
                    for i, (w01, w10) in enumerate(zip(i_w01, i_w10)):
                        ...
                        ret = mcf_est.weight_var(
                            w_ate_cont, y_dat[:, o_idx], cl_dat,  p_cfg,
                            residual_dat=vres,
...
def ate_effects_print(mcf_: 'ModifiedCausalForest',
    ...
    """Compute ATE(T)'s from potential outcomes and print them."""
    gen_cfg, ct_cfg, p_cfg = mcf_.gen_cfg, mcf_.ct_cfg, mcf_.p_cfg
    var_cfg, int_cfg, gen_tv_cfg = mcf_.var_cfg, mcf_.int_cfg, mcf_.gen_tv_cfg
    ...
    if gen_tv_cfg.yes:
        d_values = gen_tv_cfg.d_dict['main_treat']
        d_sub_values = gen_tv_cfg.d_dict['sub_treat']
        no_of_sub_treat = [len(vals) for vals in d_sub_values]
    else:
        d_values = ct_cfg.d_values_dr_np if continuous else gen_cfg.d_values
        d_sub_values = no_of_sub_treat = None
    ...
    if gen_cfg.with_output:
        ...
        if balancing_test:
            ...
            txt += '\nAverage Treatment Effects Estimation ' + extra_title + '\n' + '-' * 100
        txt += '\n'
        if pred_alloc:
            txt += 'Average Outcome'
        else:
        ...
        txt += '\n' + '-' * 100
        if p_cfg.se_boot_ate > 1:
            txt += f'\nBootstrap standard errors with {p_cfg.se_boot_ate:<6} replications'
        for o_idx, out_name in enumerate(y_name):
            ...
            for a_idx in range(no_of_ates):
                if a_idx == 0:
                    ...
                    txt += f'\n   Reference population: Treatment group: {d_values[a_idx-1]}'
                if pred_alloc:
                ...
                if gen_tv_cfg.yes:
                    t_idx = 0
                    for main_idx in range(no_of_treat):
                        for sub_idx in range(no_of_sub_treat[main_idx]):
                            txt += (f'\n{d_values[main_idx]:<4} '
                                    f'{d_sub_values[main_idx][sub_idx]:<4} '
                                    )
                            txt += ate_effects_print_helper(
                                y_pot, y_pot_var, y_pred_lc_ate, a_idx=a_idx, t_idx=t_idx,
                                o_idx=o_idx, ate_no_se_only=mcf_.p_cfg.ate_no_se_only,
                                lc_yes=lc_yes,
                                )
                            t_idx += 1
                        txt += '\n' + '- ' * 50
                    ...
                    for t_idx in range(no_of_treat):
                        txt += (f'\n{d_values[t_idx]:>10.5f} ' if continuous
                                else f'\n{d_values[t_idx]:<9} '
                                )
                        txt += ate_effects_print_helper(y_pot, y_pot_var, y_pred_lc_ate,
                                                        a_idx=a_idx, t_idx=t_idx, o_idx=o_idx,
                                                        ate_no_se_only=mcf_.p_cfg.ate_no_se_only,
                                                        lc_yes=lc_yes,
                                                        )
        if pred_alloc:
    ...
    no_all_treat = sum(no_of_sub_treat) if gen_tv_cfg.yes else no_of_treat
    if continuous:  # only comparison to zero
        ate = np.empty((len(y_name), no_of_ates, no_all_treat - 1), dtype=np.float64)
    else:
        ate = np.empty((len(y_name), no_of_ates, round(no_all_treat * (no_all_treat - 1) / 2)),
                       dtype=np.float64
                       )
    ...
    try:
        for o_idx, out_name in enumerate(y_name):
            ...
            for a_idx in range(no_of_ates):
                ...
                est, stderr, t_val, p_val, effect_list = mcf_est.effect_from_potential(
                    pot_y_ao, pot_y_var_ao, d_values,
                     continuous=continuous, se_yes=not mcf_.p_cfg.ate_no_se_only,
                ...
                if gen_cfg.with_output:
                    txt += mcf_ps.print_effect(est, stderr, t_val, p_val, effect_list,
                                               continuous=continuous, gen_tv_cfg_yes=gen_tv_cfg.yes,
                                               )
                    mcf_ps.effect_to_csv(
                        est, stderr, t_val, p_val, effect_list,
                        path=p_cfg.paths['ate_iate_fig_pfad_csv'], label=label_ate+out_name
                        )
                    txt += mcf_ps.print_se_info(p_cfg.cluster_std, p_cfg.se_boot_ate)
                    if continuous and not balancing_test and a_idx == 0:
                        dose_response_figure(out_name, var_cfg.d_name[0], est, stderr,
                                             d_values=d_values[1:], int_cfg=int_cfg, p_cfg=p_cfg,
    ...
    if gen_cfg.with_output:
        ...
        mcf_ps.print_mcf(gen_cfg, effect_dic['txt_weights'] + txt, summary=False)

    return ate, ate_se, effect_list


def ate_effects_print_helper(y_pot: NDArray[Any],
                             y_pot_var: NDArray[Any] | None,
                             y_pred_lc_ate: NDArray[Any] | None, *,
                             a_idx: int = None, t_idx: int = None,
                             o_idx: int = None,
                             ate_no_se_only: bool = False, lc_yes: bool = True,
                             ) -> str:
    """Print output in this function to avoid code duplication."""
    txt = f' {y_pot[a_idx, t_idx, o_idx]:>12.6f}'
    if not ate_no_se_only:
        sqrt_var = np.sqrt(y_pot_var[a_idx, t_idx, o_idx])
        txt += f'   {sqrt_var:>12.6f}'
    if lc_yes:
        y_adjust = y_pot[a_idx, t_idx, o_idx] + y_pred_lc_ate.iloc[o_idx]
...
def dose_response_figure(y_name: str, d_name: str,
    ...
    file_name_jpeg = p_cfg.paths['ate_iate_fig_pfad_jpeg'] / f'{file_title}.jpeg'
    file_name_pdf = p_cfg.paths['ate_iate_fig_pfad_pdf'] / f'{file_title}.pdf'
    file_name_csv = p_cfg.paths['ate_iate_fig_pfad_csv'] / f'{file_title}plotdat.csv'
    ...
    axs.fill_between(d_values, upper, lower, alpha=0.3, color='b', label=label_ci)
    ...
    if with_output:
        ...
        effects_et_al = np.concatenate((upper, effects, lower, d_values_np), axis=1)
