...
def screen_adjust_variables(mcf_: 'ModifiedCausalForest',
    ...
    if variables_deleted:
        gen_cfg, var_cfg, var_x_type, var_x_values = adjust_variables(
...
def adjust_variables(mcf_: 'ModifiedCausalForest',
    ...
    vnew_cfg.name_ordered = mcf_gp.adjust_vars_vars(var_cfg.name_ordered,
                                                    del_var
                                                    )
    vnew_cfg.name_unordered = mcf_gp.adjust_vars_vars(var_cfg.name_unordered,
                                                      del_var
                                                      )
    vnew_cfg.x_name_balance_test = mcf_gp.adjust_vars_vars(
            var_cfg.x_name_balance_test, del_var)
    vnew_cfg.x_name_always_in = mcf_gp.adjust_vars_vars(
        var_cfg.x_name_always_in, del_var)
...
def clean_data(mcf_: 'ModifiedCausalForest',
    ...
    if train:
        namen_to_inc = mcf_gp.add_var_names(
            var_cfg.id_name, var_cfg.y_name, var_cfg.cluster_name,
            var_cfg.w_name, var_cfg.d_name, var_cfg.y_tree_name,
            var_cfg.x_name_balance_test, var_cfg.x_name)
        if gen_cfg.d_type == 'continuous':
            namen_to_inc.append(*var_cfg.grid_nn_name)
        if gen_cfg.iv:
            namen_to_inc = mcf_gp.add_var_names(namen_to_inc, var_cfg.iv_name)
    else:
        namen_to_inc = mcf_gp.add_var_names(
            var_cfg.id_name, var_cfg.w_name, var_cfg.x_name_balance_test,
            var_cfg.x_name
            )
        if mcf_.p_cfg.d_in_pred:
            namen_to_inc = mcf_gp.add_var_names(namen_to_inc, var_cfg.d_name)
            if gen_cfg.d_type == 'continuous':
                namen_to_inc.append(*var_cfg.grid_nn_name)
        if mcf_.p_cfg.cluster_std:
            namen_to_inc = mcf_gp.add_var_names(namen_to_inc,
                                                var_cfg.cluster_name
                                                )
    ...
    namen_to_inc = mcf_gp.include_org_variables(namen_to_inc[:],
                                                data_df.columns)
    data_df, var_cfg.id_name = clean_reduce_data(
        data_df, namen_to_inc, gen_cfg, var_cfg.id_name,
        int_cfg.descriptive_stats, obs_bigdata=int_cfg.obs_bigdata)
...
def clean_reduce_data(data_df: pd.DataFrame,
    ...
    if add_missing_vars:
        add_df = add_missing_vars_fct(data_df.columns, names_to_inc,
                                      len(data_df), gen_cfg.with_output)
    ...
    if gen_cfg.with_output:
        ...
        if descriptive_stats:
            mcf_ps.print_descriptive_df(gen_cfg, data_new_df, varnames='all',
                                        summary=False)
...
def print_prime_value_corr(data_train_dic: dict,
                           gen_cfg: Any,
                           summary: bool = False
                           ) -> None:
...
def create_xz_variables(mcf_: 'ModifiedCausalForest',
    ...
    if train:
        if gen_cfg.d_type == 'continuous':
            ...
            data_df = recode_treat_cont(data_df, var_cfg, ct_cfg,
                                        zero_tol=int_cfg.zero_tol,
                                        )
        else:
            d1_unique = np.unique(np.int32(np.round(
                data_df[var_cfg.d_name].to_numpy())))
    else:
    ...
    if gen_cfg.agg_yes:
        if not var_cfg.z_name_cont == []:
            ...
            if z_name_cont_remove:  # changed 25.8.23
                z_name_ord = [name for name in var_cfg.z_name_ord
                              if name not in z_name_cont_remove]
                z_name = [name for name in var_cfg.z_name
                          if name not in z_name_cont_remove]
        ...
        if var_cfg.x_name_balance_bgate:
            bad_vars = [var for var in var_cfg.x_name_balance_bgate
                        if var not in var_cfg.x_name
                        ]
            if bad_vars:
    ...
    for variable in var_cfg.x_name:
        if variable in var_cfg.name_ordered:  # Ordered variable
            ...
            if int_cfg.max_cats_cont_vars < (k-2):
                ...
                var_cfg = mcf_gp.substitute_variable_name(
                    var_cfg, variable, new_variable
                    )
                if gen_cfg.with_output and gen_cfg.verbose:
            ...
            var_cfg = mcf_gp.substitute_variable_name(
                var_cfg, variable, prime_variable
                )
            if gen_cfg.with_output and gen_cfg.verbose:
    ...
    if not var_cfg.id_name or isinstance(var_cfg.id_name[0], str):
    ...
    if gen_cfg.with_output and int_cfg.descriptive_stats and train:
        mcf_ps.print_descriptive_df(gen_cfg, data_df, varnames='all',
                                    summary=False)
...
def recode_treat_cont(data_df: pd.DataFrame,
    ...
    d_dat_np = data_df[var_cfg.d_name].to_numpy()
    data_df[var_cfg.d_name] = (data_df[var_cfg.d_name] -
                               data_df[var_cfg.d_name].min()
                               )
    d_dat_np = np.where(d_dat_np < zero_tol, 0, d_dat_np)
    d_discr_w_pd = get_d_discr(data_df, d_dat_np, ct_cfg.grid_w_val,
                               var_cfg.grid_w_name, zero_tol=zero_tol
                               )
    d_discr_dr_pd = get_d_discr(data_df, d_dat_np, ct_cfg.grid_dr_val,
                                var_cfg.grid_dr_name, zero_tol=zero_tol)
    if np.all(ct_cfg.grid_nn_val == ct_cfg.grid_w_val):
        data_df = pd.concat([data_df, d_discr_w_pd, d_discr_dr_pd], axis=1)
    else:
        d_discr_nn_pd = get_d_discr(data_df, d_dat_np, ct_cfg.grid_nn_val,
                                    var_cfg.grid_nn_name,
                                    zero_tol=zero_tol,
                                    )
...
def get_d_discr(data: pd.DataFrame,
    ...
    d_discr = np.where((d_dat_np > zero_tol) & (d_discr == 0), grid_val[1],
                       d_discr
                       )
...
def get_x_data(data_df: pd.DataFrame,
               x_name: list[str]
...
def data_frame_vars_lower(data_df: pd.DataFrame) -> tuple[pd.DataFrame,
                                                          list[str]
                                                          ]:
