...
def check_data_estrisk(optp_: 'OptimalPolicy', data_df: pd.DataFrame) -> None:
    ...
    if not var_polscore_se_name:
        raise ValueError('No information on Standard Errors of Policy Score. '
                         ...
                         'for estimation errors in those scores. Programme is terminated.'
                         )
    ...
    if data_df[optp_.var_cfg.polscore_se_name].isna().any().any():
        raise ValueError('Missing values detected in standard errors of policy score.')


def prepare_data_fair(optp_: 'OptimalPolicy', data_df: pd.DataFrame,) -> pd.DataFrame:
    ...
    var_cfg.protected_ord_name = case_insensitve(var_cfg.protected_ord_name.copy())
    var_cfg.protected_unord_name = case_insensitve(var_cfg.protected_unord_name.copy())
    var_cfg.polscore_name = case_insensitve(var_cfg.polscore_name.copy())
    data_df.columns = case_insensitve(data_df.columns.tolist())
    var_cfg.material_ord_name = case_insensitve(var_cfg.material_ord_name.copy())
    var_cfg.material_unord_name = case_insensitve(var_cfg.material_unord_name.copy())
    ...
    protected_ord = var_available(var_cfg.protected_ord_name, list(data_df.columns),
                                  needed='must_have',
                                  )
    protected_unord = var_available(var_cfg.protected_unord_name, list(data_df.columns),
                                    needed='must_have',
                                    )
    material_ord = var_available(var_cfg.material_ord_name, list(data_df.columns),
                                 needed='must_have',
                                 )
    material_unord = var_available(var_cfg.material_unord_name, list(data_df.columns),
                                   needed='must_have',
                                   )
    if not (protected_ord or protected_unord):
        raise ValueError('Neither ordered nor unordered protected features specified. Fairness '
                         'adjustment is impossible without specifying at least one protected '
                         'feature.'
                         )
    ...
    if protected_ord:
        del_x_var_list = [var for var in var_cfg.x_ord_name if var in var_cfg.protected_ord_name]
        optp_.var_cfg.x_ord_name = [var for var in var_cfg.x_ord_name
    ...
    if protected_unord:
        ...
        optp_.var_cfg.prot_mat_no_dummy_name.extend(var_cfg.protected_unord_name.copy())
...
def prepare_data_for_classifiers(
    ...
    data_df.columns = case_insensitve(data_df.columns.tolist())
    x_name = []
    x_ordered = var_available(var_cfg.x_ord_name, list(data_df.columns), needed='must_have')
    x_unordered = var_available(var_cfg.x_unord_name, list(data_df.columns), needed='must_have')
    if x_ordered:
    ...
    if x_name_train is not None:
        if x_name != x_name_train:
            ...
            raise ValueError('Names (order) of features in transformed data does not fit to '
                             'training names.'
    ...
    match (bool(x_ordered), bool(x_unordered)):
        ...
        case (True, False):
            x_dat_np = x_ord_np.copy()
        case (False, True):
            x_dat_np = x_dummies_np.copy()
        case _:
...
def prepare_data_bb_pt(optp_: 'OptimalPolicy',
    ...
    if gen_cfg.method != 'best_policy_score':
        if var_cfg.x_ord_name:
            x_ordered = var_available(var_cfg.x_ord_name, var_names, needed='must_have')
        else:
            x_ordered = False

        if var_cfg.x_unord_name:
            x_unordered = var_available(var_cfg.x_unord_name, var_names, needed='must_have')
        else:
            x_unordered = False
    ...
    if gen_cfg.method in ('best_policy_score', 'bps_classifier'):
        bb_rest_variable = var_available(var_cfg.bb_restrict_name, var_names, needed='nice_to_have')
        if bb_rest_variable and var_cfg.bb_restrict_name[0] not in names_to_inc:
...
def classify_var_for_pol_tree(optp_: 'OptimalPolicy',
    ...
    for var in all_var_names:
        ...
        if var in var_cfg.x_ord_name:
            ...
            if np.sum(np.abs(values-values_round)) > optp_.int_cfg.sum_tol:
                raise TypeError('Categorical variables must be coded as integers.')
    ...
    if list_of_missing_vars:
        raise ValueError(f'{' '.join(list_of_missing_vars)} is neither '
                         'contained in list of ordered nor in list of unordered variables.')
...
def prepare_data_eval(optp_: 'OptimalPolicy',
    ...
    polscore_desc_ok = var_available(var_cfg.polscore_desc_name, var_names, needed='nice_to_have')
    if (var_cfg.polscore_desc_name is not None
            and (not polscore_desc_ok and len(var_cfg.polscore_desc_name) > no_of_treat)):
        # Try again, when removing the adjusted variable
        var_cfg.polscore_desc_name = var_cfg.polscore_desc_name[:-no_of_treat]
        polscore_desc_ok = var_available(var_cfg.polscore_desc_name, var_names,
                                         needed='nice_to_have')
    polscore_ok = var_available(var_cfg.polscore_name, var_names, needed='nice_to_have')
    desc_var_list = []
    if var_available(var_cfg.bb_restrict_name, var_names, needed='nice_to_have'):
    ...
    if var_available(var_cfg.x_unord_name, var_names, needed='nice_to_have'):
        desc_var_list.extend(var_cfg.x_unord_name)
    if var_available(var_cfg.protected_ord_name, var_names, needed='nice_to_have'):
        desc_var_list.extend(var_cfg.protected_ord_name)
    if var_available(var_cfg.protected_unord_name, var_names, needed='nice_to_have'):
        desc_var_list.extend(var_cfg.protected_unord_name)

    if optp_.gen_cfg.variable_importance:
        x_in = var_available(optp_.var_cfg.vi_x_name, var_names, needed='nice_to_have')
        dum_in = var_available(optp_.var_cfg.vi_to_dummy_name, var_names, needed='nice_to_have')
        if not (x_in or dum_in):
            print('WARNING: Variable importance requires the specification '
                  ...
                  'Since they are not specified, variable_importance is not conducted.'
                  )
