...
from mcf import mcf_data as mcf_data
...
def predict_different_allocations_main(self,
    ...
    data_df = basic_data_compability_diff_allocations(self, data_df, allocations_df)
    mcf_init_update.int_update_pred(self, len(data_df))
    mcf_init_update.post_update_pred(self, data_df)
    if self.gen_cfg.with_output:
        mcf_ps.print_dic_values_all(self, summary_top=True, summary_dic=False, train=False)
    ...
    if self.gen_cfg.with_output and self.gen_cfg.verbose:
        mcf_data.print_prime_value_corr(self.data_train_dict, self.gen_cfg, summary=False)
    ...
    if self.cs_cfg.type_:
        (data_df, _, report['alloc_cs_p_share_deleted'],
         ...
         ) = mcf_cs.common_support_p_score(self, data_df, None, train=False, p_score_only=False)
    else:
    ...
    if self.lc_cfg.yes and self.lc_cfg.uncenter_po:
        _, _, y_pred_x_df, _ = mcf_lc.local_centering(self, data_df, fill_y_df=None, train=False)
    else:
        y_pred_x_df = 0
    ...
    only_one_fold_one_round = self.cf_cfg.folds == 1 and len(self.cf_cfg.est_rounds) == 1
    for fold in range(self.cf_cfg.folds):
        ...
        weights_dic, _ = mcf_w.get_weights_mp(data_df, forest_dic,
                                              reg_round='regular', with_output=True,
                                              cf_cfg=self.cf_cfg, ct_cfg=self.ct_cfg,
                                              gen_cfg=self.gen_cfg, int_cfg=self.int_cfg,
                                              p_cfg=self.p_cfg, var_cfg=self.var_cfg,
                                              gen_tv_cfg_yes=self.gen_tv_cfg.yes,
                                              )
        time_delta_weight += time() - time_w_start

        time_a_start = time()
        # Estimate ATE n fold
        _, y_pot_f, y_pot_var_f, txt_w_f = mcf_ate.ate_est(self, data_df, weights_dic,
                                                           pred_alloc=True
                                                           )
        # Aggregate ATEs over folds
        ate_dic, _ = mcf_est.aggregate_pots(self, y_pot_f, y_pot_var_f,
                                            txt=txt_w_f, effect_dic=ate_dic, fold=fold,
        ...
        time_i_start = time()
        if allocations_df is not None:
            w_alloc_f_np, w_alloc_var_f_np, names, txt_w_f = alloc_est(self,
                                                                       data_df, weights_dic,
    ...
    time_a_start = time()
    ate, ate_se, ate_effect_list = mcf_ate.ate_effects_print(self,
                                                             ate_dic, y_pred_x_df,
                                                             balancing_test=False, pred_alloc=True,
                                                             )
    time_delta_ate += time() - time_a_start
    ...
    results = {
        ...
        'ate': ate, 'ate_se': ate_se, 'ate_effect_list': ate_effect_list, 'alloc_df': alloc_df,
        }
    if self.gen_cfg.with_output:
    ...
    if self.gen_cfg.with_output:
        ...
        time_pred = mcf_ps.print_timing(self.gen_cfg, 'Predictions for allocations',
                                        time_string, time_difference, summary=True
                                        )
...
def alloc_est(mcf_: 'ModifiedCausalForest',
    ...
    for alloc_idx in range(no_of_alloc):
        if -int_cfg.sum_tol < sumw[alloc_idx] < int_cfg.sum_tol:
            if mcf_.gen_cfg.with_output:
                txt += (f'\nAloc name: {alloc_names[alloc_idx]}) '
                        f'\nAllocation weights: {w_alloc[alloc_idx, :]}'
                        'Weights are all zero. Not good. Redo statistic without this variable.'
        ...
        if mcf_.p_cfg.max_weight_share < 1:
            w_alloc[alloc_idx, :], _, share = mcf_gp.bound_norm_weights(
                 w_alloc[alloc_idx, :],
                 max_weight_share=mcf_.p_cfg.max_weight_share, zero_tol=int_cfg.zero_tol,
                 sum_tol=int_cfg.sum_tol, negative_weights_possible=p_ba_yes,
                 )
            if mcf_.gen_cfg.with_output:
    ...
    for alloc_idx in range(no_of_alloc):
        for out_idx in range(no_of_out):
            # TODO: hier bias adjustment, oder vorher? Oder gar nicht
            ret = mcf_est.weight_var(
                w_alloc[alloc_idx, :], y_dat[:, out_idx],
                cl_dat, mcf_.p_cfg,
                residual_dat=None,
                weighted=mcf_.gen_cfg.weighted, weights=w_dat,
    ...
    for alloc1_idx, alloc1_name in enumerate(alloc_names):
        ...
        for alloc2_idx, alloc2_name in enumerate(alloc2_names, start=alloc1_idx + 1):
            alloc_diff_names.append(alloc1_name + '_m_' + alloc2_name)
            for out_idx in range(no_of_out):
                ret = mcf_est.weight_var(w_alloc[alloc1_idx, :] - w_alloc[alloc2_idx, :],
                                         y_dat[:, out_idx],
                                         cl_dat, mcf_.p_cfg,
                                         residual_dat=None,
                                         weighted=mcf_.gen_cfg.weighted, weights=w_dat,
...
def alloc_effects_print(mcf_: 'ModifiedCausalForest',
    ...
    if mcf_.gen_cfg.with_output:
        ...
        for out_idx, out_name in enumerate(mcf_.var_cfg.y_name):
            ...
            if lc_yes:
                w_alloc_np[:len(alloc_single_names), out_idx] += y_pred_lc_avg.iloc[out_idx]
            txt += '\n' + '- ' * 50
            txt += '\n    Outcomes of different allocations'

            stderr, t_val, p_val = mcf_est.compute_inference(w_alloc_np[:, out_idx],
            ...
            add_txt = (f'\nNumber of observations used for these calculations: {obs}.'
...
def basic_data_compability_diff_allocations(self,
    ...
    if len(data_df) != len(allocations_df):
        raise ValueError(f'Length of DataFrame containing features ({len(data_df)}) and '
                         f'DataFrame containing allocations {len(allocations_df)} must be the same.'
                         )
    ...
    if not unique_values_all.issubset(set(d_values)):
        ...
        raise ValueError(f'Treatment values in specified allocations {unique_values_all} '
                         'must be contained in treatment values used in training '
                         f'{set(d_values)}. The following values were not used in training: {diff}'
                         )
    ...
    data_both_df = pd.concat((data_df.reset_index(drop=True),
                              allocations_df.reset_index(drop=True)),
                             axis=1,
                             )
    return data_both_df
