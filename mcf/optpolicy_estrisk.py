...
def adjust_scores_for_estimation_risk(optp_: 'OptimalPolicy',
    ...
    scores_np = data_df[optp_.var_cfg.polscore_name].to_numpy(copy=True)
    scores_se_np = data_df[optp_.var_cfg.polscore_se_name].to_numpy(copy=True)

    # Adjusted scores
    scores_estrisk_np = scores_np - estriskcfg.value * scores_se_np

    # Convert numpy array of fair scores to pandas dataframe
    score_estrisk_name = [name + '_estrisk'
                          for name in optp_.var_cfg.polscore_name]
    score_estrisk_df = pd.DataFrame(scores_estrisk_np, columns=score_estrisk_name)
...
def estrisk_stats(optp_: 'OptimalPolicy',
                  data_df: pd.DataFrame,
                  score_estrisk_name: list[str],
                  ) -> str:
    """Compute descriptive statistics of fairness adjustments."""
    score_names = optp_.var_cfg.polscore_name
...
def change_variable_names_estrisk(optp_: 'OptimalPolicy',
                                  score_estrisk_name: list[str],
                                  ) -> None:
    """Change variable names to account for risk adjusted scores."""
    if optp_.var_cfg.polscore_desc_name is not None:
        optp_.var_cfg.polscore_desc_name.extend(optp_.var_cfg.polscore_name.copy())
    else:
        optp_.var_cfg.polscore_desc_name = optp_.var_cfg.polscore_name.copy()
    # Remove duplicates from list without changing order
    optp_.var_cfg.polscore_desc_name = remove_duplicates(optp_.var_cfg.polscore_desc_name)
    optp_.var_cfg.polscore_name = score_estrisk_name
