...
from mcf.mcf_print_stats import del_added_chars
...
def moving_avg_mean_var(data: NDArray[Any],
    ...
    if k >= obs:
        ...
        try:
            ...
            mean = np.convolve(data.reshape(-1), weights, mode='valid')
        ...
        if mean_and_var:
            ...
            try:
                ...
                mean_s = np.convolve(data_s.reshape(-1), weights, mode='valid')
...
def gini_coeff_pos(x_dat: NDArray[Any], zero_tol: float = 1e-10) -> float | int:
    ...
    if sss > zero_tol:
        ...
        gini = 1 - 2 * (np.sum((ranks - 1) * x_dat[rrr]) + sss) / (len(x_dat) * sss)
        return gini
...
def gini_coeff_all(x_dat: NDArray[Any]) -> float:
    ...
    gini = (2.0 * np.sum(ranks * x_sorted)) / (n * np.sum(x_sorted)) - (n + 1.0) / n
...
def random_forest_scikit(x_train: NDArray[Any],
                         y_train: NDArray[Any],
                         x_pred: NDArray[Any], *,
    ...
    variable_importance : Boolean. OOB based variable importance (mutated x). Default is False.
    ...
    variable_importance_oobx : Bool. Use OOB obs for VI (computational costly). Default is False.
    ...
    for nval in n_min:
        for mval in no_features:
            for aval in alpha:
                regr = RandomForestRegressor(n_estimators=boot, min_samples_leaf=round(nval),
                ...
                if with_output and not ((len(no_features) == 1) and (len(n_min) == 1)
                                        and (len(alpha) == 1)
                                        ):
                    txt += (f'\n N_min: {nval:2}  M_Try: {mval:2} Alpha: {aval:5.3f}'
    ...
    if variable_importance:
        if variable_importance_oob_flag:
            vi_names_shares, txt_vi = variable_importance_oob(
                x_name=x_name, var_im_groups=var_im_groups, no_of_vars=no_of_vars, y_1d=y_1d,
                pred_oob=pred_oob, with_output=with_output, var_im_with_output=var_im_with_output,
                x_train=x_train, boot=boot, n_opt=n_opt, m_opt=m_opt, workers=workers,
                max_depth=max_depth, a_opt=a_opt, max_leaf_nodes=max_leaf_nodes
                )
        else:
            vi_names_shares, txt_vi = variable_importance_testdata(
                x_name=x_name, var_im_groups=var_im_groups, y_all=y_1d, with_output= with_output,
                var_im_with_output=var_im_with_output, x_all=x_train, boot=boot, n_opt=n_opt,
                m_opt=m_opt, workers=workers, max_depth=max_depth, a_opt=a_opt,
                max_leaf_nodes=max_leaf_nodes
                )
        txt += txt_vi
    else:
    ...
    if pred_uncertainty and (pred_t_flag or pred_p_flag):
        ...
        if symmetric:
            ...
            quant = np.quantile(resid_oob, [(1-pu_ci_level)/2, 1-(1-pu_ci_level)/2])
            if pred_t_flag:
    ...
    return pred_p, pred_t, oob_best, vi_names_shares, pu_ci_p, pu_ci_t, forest, txt
...
def variable_importance_oob(*, x_name: list[str],
    ...
    for indx, name_to_delete in enumerate(x_to_check):
        if with_output or var_im_with_output:
            print(f'{name_to_delete}', end='')
        mse_ypred_vi = get_r2_for_vi_oob(
            indices_of_x=indices_of_x, x_name=x_name, name_to_delete=name_to_delete,
            x_train=x_train, y_1d=y_1d, boot=boot, n_opt=n_opt, m_opt=m_opt, workers=workers,
            max_depth=max_depth, a_opt=a_opt, max_leaf_nodes=max_leaf_nodes,
            )
...
def variable_importance_testdata(*, x_name: list[str],
                                 var_im_groups: list[list[str]] | None,
                                 y_all: NDArray[Any],
                                 with_output: bool,
                                 var_im_with_output: bool,
    ...
    x_train, x_test, y_train, y_test = train_test_split(x_all, y_all,
                                                        test_size=share_test_data, random_state=42,
                                                        )
    regr_vi = RandomForestRegressor(n_estimators=boot, min_samples_leaf=round(n_opt),
                                    max_features=m_opt,
    ...
    for indx, name_to_randomize in enumerate(x_to_check):
        ...
        r2_vi = get_r2_test(x_test, y_test, name_to_randomize, x_name, regr_vi, rng=rng)
    ...
    if var_im_groups:
        ...
        loss_in_r2_dummy = 0
    x_names_sorted, vim_sorted, txt_vi = print_vi(x_to_check, var_im_groups, loss_in_r2_single,
                                                  loss_in_r2_dummy, var_im_with_output
                                                  )
    vi_names_shares = (x_names_sorted, vim_sorted, txt,)
    txt += txt_vi
...
def print_vi(x_to_check: list[str],
    ...
    if var_im_groups:
        ...
        loss_in_r2_single = np.concatenate((loss_in_r2_single, loss_in_r2_dummy,))
...
def best_regression(x_np: NDArray[Any],
                    ...
                    obs_bigdata: int | float = 1_000_000,
    ...
    if (len(y_np) > obs_bigdata and max_workers is not None and max_workers > 1):
        max_workers /= 2
    ...
    regress_dic, estimator, key_list = init_scikit(boot, seed, max_workers, estimator,
                                                   with_lasso=False,
                                                   )
            ...
            for idx, key in enumerate(key_list):
                if regress_dic[key]['obj'] is None:
                    ...
                    if regress_dic[key]['transform_x']:
                        ...
                        y_pred[index_pred, idx] = regress_dic[key]['obj'].predict(x_test_scaled)
                    else:
                        regress_dic[key]['obj'].fit(x_train, y_train)
                        y_pred[index_pred, idx] = regress_dic[key]['obj'].predict(x_test)
    ...
    return best_method, best_params, best_lables, best_mse, best_transform_x, mse_string
...
def init_scikit(boot: int,
    ...
    regr_dic = {}
    key_list = ['rf', 'rfl5', 'rfls5', 'svr', 'svr2', 'svr4', 'ada', 'ada100', 'ada200',
                'gboost', 'gboost6',  'gboost12', 'nnet', 'nnet_l1', 'nnet_l2', 'mean',
                ]
    if with_lasso:
    ...
    regr_dic['rf']['params'] = {'n_estimators': boot, 'max_features': 'sqrt',
    ...
    regr_dic['svr']['params'] = {'kernel': 'rbf', 'gamma': 'scale', 'tol': 1e-3, 'epsilon': 0.1,
    ...
    regr_dic['ada']['params'] = {'estimator': None, 'n_estimators': 50, 'learning_rate': 1.0,
    ...
    regr_dic['gboost']['params'] = {'loss': 'squared_error', 'n_estimators': 200, 'max_depth': 3,
                                    'random_state': seed
                                    }
    ...
    if with_lasso:
        regr_dic['lasso']['params'] = {'max_iter': 1000, 'copy_X': True, 'cv': 5,
                                       'n_jobs': max_workers, 'random_state': seed
                                       }
    regr_dic['nnet']['params'] = {'alpha': 0.0001, 'max_iter': 2000, 'random_state': seed,
    ...
    for key in key_list:
        ...
        if regr_dic[key]['params'] is None:
            ...
            regr_dic[key]['obj'] = regress_instance(regr_dic[key]['method'], regr_dic[key]['params']
                                                    )
    if estimator == 'automatic':
...
def regress_instance(method: str, parameters_dict: dict) -> None:
    ...
    if method in ('SupportVectorMachine', 'SupportVectorMachineC2', 'SupportVectorMachineC4',):
...
def best_classifier(x_np: NDArray[Any],
    ...
    while True:
        x_train, x_test, y_train, y_test = train_test_split(x_np, y_np,
                                                            test_size=test_share, random_state=seed
                                                            )
    ...
    methods = ('RandomForestClass5', 'RandomForestClass2', 'NNetClass', 'AdaClass',)
...
def classif_instance(method: str, parameters_dict: dict) -> None:
    ...
    valid_methods = ('RandomForestClass5', 'RandomForestClass2', 'NNetClass', 'AdaClass',)
    raise ValueError(f'{method} not considered. Change method. Only '
                     f'{" ".join(valid_methods)} allowed.')
...
def honest_tree_explainable(x_dat: NDArray[Any],
    ...
    for depth in depth_grid:
        ...
        plt_all.append(plot)
        plot_h = plot_tree_fct(tree_inst_h, feature_names, depth, honest=True, y_name=title)
    ...
    results_dict = {'plots': plt_all, 'plots_h': plt_h_all, 'fit': fit_all, 'fit_h': fit_h_all}

    results_dict['fit_title'] = 'R squared' if tree_type == 'regression' else 'Accuracy score'
...
def plot_tree_fct(tree_inst: TreeEstimator,
    ...
    tree.plot_tree(tree_inst, filled=False, feature_names=feature_names, rounded=True, ax=ax)
...
def bayes_factor(x: pd.Series, y: pd.Series, alpha: float = 1.0) -> float:
    ...
    df = pd.concat((x.reset_index(drop=True), y.reset_index(drop=True)), axis=1).dropna()
    x_clean = df.iloc[:, 0]
    y_clean = df.iloc[:, 1]
    obs = len(df)
