...
if TYPE_CHECKING:
    from mcf.mcf_init import GenCfg, IntCfg, CtGrid, VarCfg
    from mcf.mcf_init_train import CfCfg
    from mcf.mcf_init_predict import PCfg


def get_weights_mp(data_df: DataFrame,
                   forest_dic: dict, *,
                   gen_cfg: 'GenCfg',
                   var_cfg: 'VarCfg',
                   int_cfg: 'IntCfg',
    ...
    if int_cfg.weight_as_sparse_splits is None:
        if len(data_df) < 5000:
            ...
            int_cfg.weight_as_sparse_splits = round(
                len(forest_dic['y_train_df']) * len(data_df) / cf_cfg.folds / (base_dim ** 2)
                )
        int_cfg.weight_as_sparse_splits = max(int_cfg.weight_as_sparse_splits, 1)
    x_dat_all = mcf_gp.to_numpy_big_data(data_df[var_cfg.x_name], int_cfg.obs_bigdata)

    kw_args_w_inner = {'cf_cfg': deepcopy(cf_cfg), 'ct_cfg': ct_cfg, 'gen_cfg': gen_cfg,
                       'int_cfg': int_cfg, 'p_cfg': p_cfg, 'with_output': with_output,
                       'bigdata_train': int_cfg.bigdata_train, 'subtreatments': gen_tv_cfg_yes,
                       'print_progress': print_progress,
                       }
    if int_cfg.weight_as_sparse_splits == 1 or not int_cfg.weight_as_sparse:
        (weights, y_dat, x_bala, cl_dat, w_dat, prog_dat, prop_dat, x_ba_dat, d_dat_train,
         x_dat_train,) = get_weights_mp_inner(forest_dic, x_dat_all, **kw_args_w_inner)
    else:
        ...
        if gen_cfg.outpath is None or not gen_cfg.outpath.is_dir():
            ...
            base_file = base_name.with_name(f'{base_name.name} {temp_weight_name}')
        else:
            base_file = gen_cfg.outpath / temp_weight_name
        x_dat_list = np.array_split(x_dat_all, int_cfg.weight_as_sparse_splits, axis=0)
        ...
        for idx, x_dat in enumerate(x_dat_list):  # Das ist der Mastersplit
            ...
            if print_yes:
                ...
                print('\n\nWeights computation with additional splitting. ',
                      ...
                      '\nComputing weight matrix (inner loop).'
                      )
            if idx == len(x_dat_list) - 1:  # Last iteration, get x_bala
                no_x_bala_return = False

            (weights_i, y_dat, x_bala, cl_dat, w_dat, prog_dat, prop_dat, x_ba_dat, d_dat_train,
             x_dat_train,) = get_weights_mp_inner(forest_dic, x_dat,
                                                  **kw_args_w_inner,
            ...
            if int_cfg.weight_as_sparse_splits > 1:
                ...
                for d_idx, weight_di in enumerate(weights_i):
                    f_name_d_idx = base_file.parent / (base_file.name + f'{idx}_{d_idx}.npz')
            ...
            if int_cfg.bigdata_train and (int_cfg.mp_use_old_ray or int_cfg.mp_backend == 'ray'):
                duration = time() - time_start
                reference_duration, _ = mcf_ray.check_ray_shutdown_legacy(
                    gen_cfg, reference_duration, duration, gen_cfg.mp_parallel,
                    max_multiplier=5,
                    with_output=gen_cfg.with_output and gen_cfg.verbose and with_output,
    ...
    no_of_treat = len(ct_cfg.grid_w_val) if gen_cfg.d_type == 'continuous' else gen_cfg.no_of_treat
    txt = mcf_sys.print_size_weight_matrix(weights, int_cfg.weight_as_sparse, no_of_treat)

    runde = 2 - int(reg_round) if isinstance(reg_round, bool) else 2 - int(reg_round == 'regular')

    if print_yes:
        mcf_ps.print_mcf(gen_cfg, f'Round {runde}. ' + txt, summary=False)
        if int_cfg.memory_print:
            mcf_sys.print_mememory_statistics(gen_cfg, 'Prediction: End of weights estimation')

    return ({'weights': weights,
            'y_dat_np': y_dat, 'x_bala_np': x_bala, 'cl_dat_np': cl_dat, 'w_dat_np': w_dat,
            'prog_dat_np': prog_dat, 'prop_dat_np': prop_dat,
            'x_ba_dat': x_ba_dat, 'd_dat': d_dat_train, 'x_tv_dat': x_dat_train,
            },
            time() - start_time,
            )
...
def get_weights_mp_inner(forest_dic: dict,
                         x_dat: NDArray[Any], *,
                         cf_cfg: 'CfCfg', ct_cfg: 'CtGrid', gen_cfg: 'GenCfg', int_cfg: 'IntCfg',
                         p_cfg: 'PCfg',
    ...
    ct_cfg :  Dataclass CtGrid. Variables.
    ...
    d_dat: N_y x 2 Numpy array or None. Treatment info with subtreatments
    x_dat: N_y x 2 Numpy array or None. X for subtreatment regressions.
    """
    print_yes = gen_cfg.with_output and gen_cfg.verbose and with_output
    progress = print_yes and print_progress
    use_tqdm, output_clean = mcf_gp.tqdm_setup(tqdm, progress)
    y_dat = mcf_gp.to_numpy_big_data(forest_dic['y_train_df'], int_cfg.obs_bigdata)

    d_dat = np.int32(np.round(forest_dic['d_train_df'].to_numpy(copy=True)))
    if subtreatments:
        d_dat_export = d_dat.copy()
        d_dat = d_dat[:, 0]
        x_dat_export = forest_dic['x_train_df']
    else:
        d_dat_export = x_dat_export = None
        if d_dat.shape[1] == 2:
            d_dat = d_dat[:, 0]

    prog_dat = forest_dic['prog_dat_np']  # None if bias adj. is not used
    prop_dat = forest_dic['prop_dat_np']  # None if bias adj. is not used
    x_ba_dat = forest_dic['x_ba_dat_np']  # None if bias adj. is not used
    ...
    if gen_cfg.d_type != 'continuous':
        d_values_0_treatm1 = np.array_equal(np.sort(np.asarray(gen_cfg.d_values, dtype=np.intp)),
    ...
    cl_dat = (mcf_gp.to_numpy_big_data(forest_dic['cl_train_df'], int_cfg.obs_bigdata)
              if p_cfg.cluster_std
              else 0
              )
    w_dat = forest_dic['w_train_df'].to_numpy(copy=True) if gen_cfg.weighted else 0
    x_bala = 0
    if p_cfg.bt_yes and not no_x_bala_return:
        x_bala = mcf_gp.to_numpy_big_data(forest_dic['x_bala_df'], int_cfg.obs_bigdata)
    ...
    print_runtime_info(gen_cfg, int_cfg, maxworkers, txt_method='weight matrix')

    mp_over_boots = False if maxworkers == 1 else bool(int_cfg.mp_weights_type == 2)
    no_of_treat = len(ct_cfg.grid_w_val) if gen_cfg.d_type == 'continuous' else gen_cfg.no_of_treat

    kw_args = {'cf_cfg': cf_cfg, 'ct_cfg': ct_cfg, 'gen_cfg': gen_cfg, 'int_cfg': int_cfg,
               'treat_is_integer': treat_is_integer, 'd_values_0_treatm1': d_values_0_treatm1,
               }
    if maxworkers == 1 or mp_over_boots:
        weights = initialise_weights(n_x, n_y, no_of_treat, int_cfg.weight_as_sparse)
        split_forest = False
        iterator_idx = range(n_x)
        if use_tqdm:
            iterator_idx = tqdm(iterator_idx, total=n_x, desc='Weights',
                                leave=False, dynamic_ncols=True
                                )
        for idx in iterator_idx:  # Iterate over i
            results_fut_idx = weights_obs_i(idx, n_y, forest_dic['forest'], x_dat, d_dat,
                                            **kw_args,
                                            mp_over_boots=mp_over_boots, maxworkers=maxworkers,
                                            split_forest=False,
                                            )
        ...
        if print_yes:
            txt = ('\nOperational characteristics of weight estimation I '
                   ...
                   f'\nAverage # of observations per chunck: {n_x / no_of_splits_i:5.2f} '
                   )
        ...
        if (not int_cfg.weight_as_sparse) and split_forest:
            if print_yes:
                ...
                txt += ('\nBootstrap splitting requires using sparse matrices.'
                        '\nProgramme continues without bootstrap splitting but may crash due to '
                        'insufficient memory.'
                        )
        ...
        for b_i, boots_ind in enumerate(boot_indx_list):
            ...
            if split_forest:
                # get subforest
                forest_temp = forest_dic['forest'][boots_ind[0]: boots_ind[-1]+1]
            ...
            if int_cfg.mp_use_old_ray:
                if not ray.is_initialized():
                    if ray is None or ray_weights_many_obs_i is None:
                    ...
                    _, no_of_workers = mcf_ray.init_ray_with_fallback(
                        maxworkers, gen_cfg,
                ...
                d_dat_ref, x_dat_ref, forest_ref = mcf_ray.ray_put_all(d_dat, x_dat,
                                                                       forest_dic['forest'],
                                                                       )
                still_running = [ray_weights_many_obs_i.remote(idx_list, n_y,
                                                               forest_ref, x_dat_ref, d_dat_ref,
                                                               **kw_args,
                ...
                pbar = (tqdm(total=no_of_splits_i, desc='Weights', leave=False, dynamic_ncols=True)
                ...
                try:
                            ...
                            if pbar is not None:
                                ...
                                if progress:
                                    ...
                                    if jdx == 0:
                                        ...
                                        print(f' {jdx+1:2} ({no_of_splits_i}){td_str}', end='')
                            jdx += 1
                            ...
                            for idx, val_list in enumerate(results_fut_idx[0]):
                                if int_cfg.weight_as_sparse:
                                    for d_idx in range(no_of_treat):
                                        ...
                                        # weights_obs = results_fut_idx[1][idx][d_idx][1]
                                        # idx_resized = np.resize( indices, (len(indices), 1))
                                        # weights[d_idx][val_list, idx_resized] = weights_obs
                                        indices = results_fut_idx[1][idx][d_idx][0]
                                        weights_obs = results_fut_idx[1][idx][d_idx][1]

                                        cols = np.asarray(indices, dtype=np.intp).ravel()
                                        vals = np.asarray(weights_obs, dtype=np.float32).ravel()

                                        if cols.size != vals.size:
                                            raise RuntimeError('Weight reconstruction mismatch: '
                                                               f'row={int(val_list)}, '
                                                               f'treatment={d_idx}, '
                                                               f'cols={cols.size}, vals={vals.size}'
                                                               )
                                        if cols.size:
                                            weights[d_idx][int(val_list), cols] = vals
                                else:
                    ...
                    if pbar is not None:
                        pbar.close()
                (x_dat_ref, forest_ref, d_dat_ref, finished_res, finished
                 ) = mcf_ray.ray_del_refs(x_dat_ref, forest_ref, d_dat_ref,
                                          f1=finished_res, f2=finished,
                                          mp_ray_del=int_cfg.mp_ray_del,
                                          )
            else:
                shared_weight_data = {'d_dat': d_dat, 'x_dat': x_dat, 'forest': forest_dic['forest']
                                      }
                fail_txt='Failed to make executor in weight estimation 1.'
                with forest_executor_with_shared(int_cfg=int_cfg, maxworkers=maxworkers,
                                                 shared_obj=shared_weight_data,
                                                 shared_name='weight_estimation_1_data',
                                                 ray_err_txt=ray_err_txt, fail_txt=fail_txt,
                                                 ) as (executor, data_handle, maxworkers):
                    tasks = [TaskSpec(func=weights_many_obs_i_backend,
                                      kwargs={'idx_list': idx_list,
                                              'n_y': n_y,
                                              'data': data_handle,
                                              'kw_args': kw_args,
                                              'split_forest': split_forest,
                                              },
                                      name=f'weights_many_obs_i_{jdx}',
                                      )
                             for jdx, idx_list in enumerate(all_idx_split)
                             ]
                    jdx = 0
                    time_start_parallel = time() if bigdata_train else 0
                    pbar = (tqdm(total=no_of_splits_i, desc='Weights', leave=False,
                                 dynamic_ncols=True)
                            if use_tqdm else None
                            )
                    try:
                        for results_fut_idx in map_task_batches(executor=executor, tasks=tasks,
                            ...
                            if pbar is not None:
                                ...
                                if progress:
                                    if bigdata_train:
                                        time_diff = (time() - time_start_parallel) / (jdx + 1)
                                        td_str = ' (' + str(int(np.round(time_diff))) + 's)'
                                    else:
                                        td_str = ''
            ...
            if split_forest:
                ...
                if print_yes:
                    print()
                    mcf_gp.print_size_weight_matrix(weights_all, int_cfg.weight_as_sparse,
                                                    no_of_treat
                                                    )
                    _, _, _, _, txt = mcf_sys.memory_statistics()
                    mcf_ps.print_mcf(gen_cfg, ' ' + txt, summary=False)
        ...
        if bigdata_train and int_cfg.mp_use_old_ray:
            duration = time() - time_start
            reference_duration, _ = mcf_ray.check_ray_shutdown_legacy(
                gen_cfg, reference_duration, duration, no_of_workers,  # pylint: disable=E0606
                max_multiplier=5, with_output=print_yes,
    ...
    if split_forest:
        ...
        weights = normalize_weights(weights_all, no_of_treat, int_cfg.weight_as_sparse, n_x)
    weights = tuple(weights)
    if ((empty_leaf_counter > 0) or (merge_leaf_counter > 0)) and print_yes:
        txt = (f'\n\n{merge_leaf_counter:5} observations attributed in merged leaves'
               f'\n{empty_leaf_counter:5} observations attributed to leaf w/o observations'
               )
        mcf_ps.print_mcf(gen_cfg, txt, summary=True)

    return (weights, y_dat, x_bala, cl_dat, w_dat, prog_dat, prop_dat, x_ba_dat, d_dat_export,
            x_dat_export,
            )


def weights_many_obs_i_backend(*, idx_list: Any,
                               n_y: int,
                               data: dict[str, Any],
                               kw_args: dict[str, Any],
                               split_forest: bool,
                               ) -> Any:
    """Backend-agnostic wrapper for one weight-estimation chunk."""
    return weights_many_obs_i(idx_list, n_y, data['forest'], data['x_dat'], data['d_dat'],
                              **kw_args,
                              split_forest=split_forest,
                              )
...
def normalize_weights(weights: list,
    ...
    for d_idx in range(no_of_treat):
        if sparse_m:
            ...
            for i in range(n_x):
                weights[i][d_idx][1] = weights[i][d_idx][1] / np.sum(weights[i][d_idx][1])
                weights[i][d_idx][1] = weights[i][d_idx][1].astype(np.float32)

    return weights


def normal_weights_sparse_loop(weights: sparse.csr_matrix, n_x: int) -> sparse.csr_matrix:
    """Normalize sparse weight matrix (which is the heavy duty case."""
    row_sum = 1 / weights.sum(axis=1)
    for i in range(n_x):
        weights[i, :] = weights[i, :].multiply(row_sum[i])
    weights = weights.astype(np.float32, casting='same_kind')

    return weights


def normal_weights_sparse(weights: sparse.csr_matrix, n_x: int) -> sparse.csr_matrix:
    """Normalize sparse weight matrix (which is the heavy duty case."""
    row_sum_inv = 1 / weights.sum(axis=1)
    # Create a diagonal matrix with the reciprocals of row sums
    diag_matrix = sparse.csr_array((row_sum_inv.toarray().ravel(), (range(n_x), range(n_x))))
    # Multiply the original sparse matrix by the diagonal matrix
    normalized_weights = weights @ diag_matrix
    # Convert to float32
    normalized_weights = normalized_weights.astype(np.float32, casting='same_kind')

    return normalized_weights


def initialise_weights(n_x: int, n_y: int, no_of_treat: int, weight_as_sparse: bool,) -> list[Any]:
...
def weights_obs_i(idx: int,
    ...
    ct_cfg : CtGrid dataclass. Parameters
    gen_cfg : GenCfg dataclass. Parameters
    int_cfg : Instance of Dataclass. Parameters
    mp_over_boots : Bool. Multiprocessing at level of bootstraps. Default is False.
    ...
    if weight_with_triplet:
        rows_parts: list[NDArray[np.integer]] = []
        cols_parts: list[NDArray[np.integer]] = []
        vals_parts: list[NDArray[np.floating]] = []
    else:
        dtype = np.float32 if bigdata_train else np.float64
        weights_i_np = np.zeros((n_y, no_of_treat + 1), dtype=dtype)
        weights_i_np[:, 0] = np.arange(n_y)
        buffer = np.zeros((n_y, no_of_treat), dtype=dtype)

    kw_args = {'n_y': n_y, 'no_of_treat': no_of_treat, 'd_values': d_values,
               'continuous': continuous, 'bigdata_train': bigdata_train,
               'zero_tol': int_cfg.zero_tol, 'd_values_0_treatm1': d_values_0_treatm1,
               }
    if mp_over_boots:
        ray_err_txt='Ray does not start in weight estimation.'
        if int_cfg.mp_use_old_ray:
            if (ray is None
                or ray_weights_obs_i_inside_boot_triplet is None
                    or ray_weights_obs_i_inside_boot is None
                    ):
                raise ImportError('int_cfg.mp_use_old_ray=True, but ray is not installed. '
                                  'Install ray or use the new backend with '
                                  'mp_use_old_ray=False.'
            ...
            d_dat_ref = mcf_ray.ray_put_all(d_dat)

            if weight_with_triplet:  # Added Oct, 30, 2025
                still_running = [ray_weights_obs_i_inside_boot_triplet.remote(forest[boot],
                                                                              x_dat_i=x_dat_i,
                                                                              d_dat=d_dat_ref,
                                                                              **kw_args,
                                                                              )
                ...
                while len(still_running) > 0:
                    ...
                    for result in finished_res:
                        if result[3]:
                            ...
                            # np.add.at(weights_i_np, (result[0], 1 + result[1]), result[2])
                            rows_parts.append(result[0])
                            cols_parts.append(result[1])
                            vals_parts.append(result[2])
            else:
                still_running = [ray_weights_obs_i_inside_boot.remote(forest[boot],
                                                                      x_dat_i=x_dat_i,
                                                                      d_dat=d_dat_ref,
                                                                      **kw_args,
                                                                      treat_is_integer=
            ...
            shared_weight_estimation_data = {'d_dat': d_dat}

            fail_txt='Failed to make executor in weight estimation.'
            with forest_executor_with_shared(int_cfg=int_cfg, maxworkers=maxworkers,
                                             shared_obj=shared_weight_estimation_data,
                                             shared_name='weight_estimation_data',
                                             ray_err_txt=ray_err_txt, fail_txt=fail_txt,
                                             ) as (executor, data_handle, maxworkers_):
                if weight_with_triplet:
                    tasks = [TaskSpec(func=weights_obs_i_inside_boot_triplet_backend,
                                      kwargs={'forest_boot': forest[boot],
                                              'x_dat_i': x_dat_i,
                                              'data': data_handle,
                                              'kw_args': kw_args,
                                              },
                                      name=f'weights_obs_triplet_{boot}',
                                      )
                             for boot in range(cf_cfg.boot)
                             ]
                else:
                    tasks = [TaskSpec(func=weights_obs_i_inside_boot_backend,
                                      kwargs={'forest_boot': forest[boot],
                                              'x_dat_i': x_dat_i,
                                              'data': data_handle,
                                              'kw_args': kw_args,
                                      ...
                                      name=f'weights_obs_{boot}',
                                      )
                             for boot in range(cf_cfg.boot)
                             ]
                for result in map_task_batches(executor=executor, tasks=tasks, int_cfg=int_cfg,
        ...
        for boot in range(cf_cfg.boot):
            if weight_with_triplet:  # Added Oct, 30, 2025
                rows, cols, vals, empty_leaf = weights_obs_i_inside_boot_triplet(forest[boot],
                                                                                 x_dat_i=x_dat_i,
                                                                                 d_dat=d_dat,
                                                                                 **kw_args,
                                                                                 )
                ...
                weights_ij_np, empty_leaf = weights_obs_i_inside_boot(forest[boot],
                                                                      x_dat_i=x_dat_i, d_dat=d_dat,
                                                                      **kw_args,
                                                                      treat_is_integer=
                                                                          treat_is_integer,
                                                                      buffer=buffer,
    ...
    if weight_with_triplet:
        weights_i = final_trans_from_triplets(rows_parts, cols_parts, vals_parts,
                                              n_y=n_y, no_of_treat=no_of_treat, normalize=normalize,
                                              zero_tol=1e-14,
                                              )
    else:
        weights_i = final_trans(weights_i_np, no_of_treat, normalize)
...
def weights_obs_i_inside_boot_triplet_backend(*, forest_boot: Any,
                                              x_dat_i: Any,
                                              data: dict[str, Any],
                                              kw_args: dict[str, Any],
                                              ) -> Any:
    """Backend-agnostic wrapper for one bootstrap weight task (triplet)."""
    return weights_obs_i_inside_boot_triplet(forest_boot,
                                             x_dat_i=x_dat_i, d_dat=data['d_dat'],
                                             **kw_args,
                                             )


def weights_obs_i_inside_boot_backend(*, forest_boot: Any,
                                      x_dat_i: Any,
                                      data: dict[str, Any],
                                      kw_args: dict[str, Any],
                                      treat_is_integer: bool,
                                      buffer: Any,
                                      ) -> Any:
    """Backend-agnostic wrapper for one bootstrap weight task."""
    return weights_obs_i_inside_boot(forest_boot,
                                     x_dat_i=x_dat_i, d_dat=data['d_dat'],
                                     **kw_args,
...
# @ray.remote
def _ray_weights_obs_i_inside_boot_impl(forest_b: dict, *,
                                        x_dat_i: NDArray[Any],
                                        n_y: int,
                                        no_of_treat: int,
    ...
    weights_ij_np, empty_leaf = weights_obs_i_inside_boot(
        forest_b,
        x_dat_i=x_dat_i, n_y=n_y, no_of_treat=no_of_treat, d_values=d_values, d_dat=d_dat,
        continuous=continuous, bigdata_train=bigdata_train, treat_is_integer=treat_is_integer,
        buffer=buffer, d_values_0_treatm1=d_values_0_treatm1, zero_tol=zero_tol,
        )
    return weights_ij_np, empty_leaf


ray_weights_obs_i_inside_boot = (ray.remote(_ray_weights_obs_i_inside_boot_impl) if ray is not None
                                 else None
                                 )


# @ray.remote
def _ray_weights_obs_i_inside_boot_triplet_impl(forest_b: dict, *,
                                                x_dat_i: NDArray[Any],
                                                n_y: int,
                                                no_of_treat: int,
                                                d_values: Sequence[int | float],
                                                ...
                                                zero_tol: float = 1e-10,
                                                           ...
                                                           NDArray[np.floating], bool
                                                           ]:
    """Allow for MP at bootstrap level (intermediate procedure) with ray."""
    rows, cols, vals, empty_leaf = weights_obs_i_inside_boot_triplet(
        forest_b, x_dat_i=x_dat_i, n_y=n_y, no_of_treat=no_of_treat, d_values=d_values, d_dat=d_dat,
        continuous=continuous, bigdata_train=bigdata_train, d_values_0_treatm1=d_values_0_treatm1,
        zero_tol=zero_tol,
        )
    return rows, cols, vals, empty_leaf


ray_weights_obs_i_inside_boot_triplet = (ray.remote(_ray_weights_obs_i_inside_boot_triplet_impl)
                                         if ray is not None
                                         else None
                                         )
...
def weights_obs_i_inside_boot(forest_b: dict, *,
    ...
    if continuous:
        ...
        return weights_obs_i_inside_boot_treat_cont(forest_b,
                                                    x_dat_i=x_dat_i, n_y=n_y,
                                                    no_of_treat=no_of_treat, d_values=d_values,
                                                    d_dat=d_dat, bigdata_train=bigdata_train,
                                                    zero_tol=zero_tol,
                                                    )
    # Discrete treatments
    return weights_obs_i_inside_boot_treat_disc(forest_b,
                                                x_dat_i=x_dat_i, n_y=n_y, no_of_treat=no_of_treat,
                                                d_values=d_values, d_dat=d_dat,
...
def weights_obs_i_inside_boot_triplet(forest_b: dict, *,
                                      x_dat_i: NDArray[Any],
                                      n_y: int,
                                      no_of_treat: int,
                                      d_values: Sequence[int | float],
    ...
    """Allow for MP at bootstrap level (intermediate procedure)."""
    # This is the default branch
    kw_args = {'bigdata_train': bigdata_train, 'zero_tol': zero_tol,}
    if continuous:
        # Only 2 returns
    ...
    return weights_obs_i_inside_boot_treat_disc_triplets(forest_b, x_dat_i, no_of_treat, d_values,
                                                         d_dat,
                                                         d_values_0_treatm1=d_values_0_treatm1,
                                                         **kw_args,
                                                         )
...
def weights_obs_i_inside_boot_treat_cont(forest_b: dict, *,
    ...
    for treat in d_values[1:]:
        ...
        leaf_id = mcf_fo_add.get_terminal_leaf_no(forest_b, x_dat_i_t, zero_tol=zero_tol,)
        leaf_id_list.append(leaf_id)
        if fill_y_empty_leave[leaf_id] == 1 or fill_y_indices_list[leaf_id] is None:
    ...
    if not empty_leaf:
        fb_lid_14_list = [fill_y_indices_list[leaf_id] for leaf_id in leaf_id_list]
        if bigdata_train:
        ...
        for jdx, fb_lid_14 in enumerate(fb_lid_14_list):
            ...
            if np.any(indices_ibj_pos):  # any valid observations?
                ...
                weights_ij_np[fb_lid_14_indi_pos, jdx+1] += 1 / n_x_i_pos
            else:
...
def weights_obs_i_inside_boot_treat_disc_triplets(forest_b: dict,
                                                  x_dat_i: NDArray[Any],
                                                  no_of_treat: int,
                                                  d_values: Sequence[int | float],
    ...
    if not d_values_0_treatm1:
        d_dat = encode_treatments(d_dat, d_values, zero_tol=zero_tol)
    ...
    leaf_id = mcf_fo_add.get_terminal_leaf_no(forest_b, x_dat_i, zero_tol=zero_tol,)
    leaf_inds = inds_list[leaf_id]
    if empty_flags[leaf_id] == 1 or leaf_inds is None:
        return (np.empty(0, dtype=np.intp), np.empty(0, dtype=np.intp),
                np.empty(0, dtype=dtype_float), True,
                )
    leaf_inds = np.asarray(leaf_inds, dtype=np.intp)
    if leaf_inds.size == 0:
        return leaf_inds, leaf_inds, np.empty(0, dtype=dtype_float), True
    ...
    if (counts[:no_of_treat] == 0).any():
        return leaf_inds, d_leaf, np.empty(0, dtype=dtype_float), True

    vals = (1.0 / counts[d_leaf]).astype(dtype_float, copy=False)

    return leaf_inds, d_leaf, vals, False
...
def weights_obs_i_inside_boot_treat_disc(forest_b: dict, *,
    ...
    if out_buffer is None:
        ...
        if out_buffer.shape != (n_y, no_of_treat):
            raise ValueError(f'out_buffer has wrong shape; expected {n_y, no_of_treat}')
        if out_buffer.dtype != dtype:
            raise TypeError(f'out_buffer has wrong dtype; expected {dtype} got {out_buffer.dtype}')
        weights = out_buffer
        weights.fill(0)
...
def final_trans_from_triplets(rows_parts: list[NDArray[np.integer]],
                              cols_parts: list[NDArray[np.integer]],
                              vals_parts: list[NDArray[np.floating]], *,
                              n_y: int,
                              no_of_treat: int,
    ...
    """Convert bootstrap triplets into per-treatment sparse weight lists.

    This replaces the dense n_y x no_of_treat accumulator for discrete treatments.
    """
    if not rows_parts:
    ...
    vals = np.concatenate(vals_parts).astype(np.float64, copy=False)

    if not rows.size == cols.size == vals.size:
        raise RuntimeError(f'Triplet length mismatch: rows={rows.size}, cols={cols.size}, '
                           f'vals={vals.size}'
                           )
    if rows.size == 0:
        return [[np.empty(0, dtype=np.int32), np.empty(0, dtype=np.float32)]
    ...
    if rows.min() < 0 or rows.max() >= n_y:
        raise RuntimeError(f'Triplet row index out of range: '
                           f'min={rows.min()}, max={rows.max()}, n_y={n_y}'
                           )
    if cols.min() < 0 or cols.max() >= no_of_treat:
        raise RuntimeError(f'Triplet treatment index out of range: '
                           f'min={cols.min()}, max={cols.max()}, no_of_treat={no_of_treat}'
                           )
    key = cols * int(n_y) + rows

    unique_key, inverse = np.unique(key, return_inverse=True)
    summed_vals = np.bincount(inverse, weights=vals)

    keep = np.abs(summed_vals) > zero_tol
    unique_key = unique_key[keep]
    summed_vals = summed_vals[keep]

    unique_cols = unique_key // int(n_y)
    unique_rows = unique_key % int(n_y)

    weights_i: list[list[NDArray[Any]]] = []

    for t_idx in range(no_of_treat):
        mask = unique_cols == t_idx
        sel_idx = unique_rows[mask].astype(np.int32, copy=False)
        sel_w = summed_vals[mask]

        if normalize and sel_w.size:
            total = sel_w.sum()
            if total > 0:
                sel_w = sel_w / total

        weights_i.append([sel_idx, sel_w.astype(np.float32, copy=False),])

    return weights_i


def weights_to_csr(weights: list[sparse.lil_matrix], no_of_treat: int) -> sparse.csr_matrix:
...
# @ray.remote
def _ray_weights_many_obs_i_impl(idx_list: list[int],
                                 n_y: int,
                                 forest: list[list],
                                 x_dat: NDArray[Any],
                                 d_dat: NDArray[Any], *,
                                 cf_cfg: 'CfCfg', ct_cfg: 'CtGrid', gen_cfg: 'GenCfg',
                                 int_cfg: 'IntCfg',
                                 split_forest: bool = False,
                                 treat_is_integer: bool = True,
                                 d_values_0_treatm1: bool = True,
    ...
    return weights_many_obs_i(idx_list, n_y, forest, x_dat, d_dat, cf_cfg=cf_cfg,
                              ct_cfg=ct_cfg, gen_cfg=gen_cfg, int_cfg=int_cfg,
                              split_forest=split_forest, treat_is_integer=treat_is_integer,
                              d_values_0_treatm1=d_values_0_treatm1,
                              )


ray_weights_many_obs_i = ray.remote(_ray_weights_many_obs_i_impl) if ray is not None else None
...
def weights_many_obs_i(idx_list: list[int],
    ...
    for idx, val in enumerate(idx_list):
        ret = weights_obs_i(val, n_y, forest, x_dat, d_dat,
                            cf_cfg=cf_cfg, ct_cfg=ct_cfg, gen_cfg=gen_cfg, int_cfg=int_cfg,
                            mp_over_boots=False, maxworkers=1,
                            split_forest=split_forest,
                            treat_is_integer=treat_is_integer,
                            d_values_0_treatm1=d_values_0_treatm1,
                            )
...
def encode_treatments(d_dat: NDArray[np.floating | np.integer],
    ...
    if np.issubdtype(dv.dtype, np.integer) and np.array_equal(dv, np.arange(dvsize)):
    ...
    in_range = (pos < dvsize) & (np.isclose(dv_sorted[pos], raw, atol=zero_tol, rtol=zero_tol))
    if not in_range.all():
...
def sparse_row_indices_weights(weight_mat: Any,
                               row_idx: int,
                               ) -> tuple[NDArray[np.integer], NDArray[np.floating]]:
    """Extract nonzero column indices and values from one sparse row.

    Works for both scipy sparse matrices and sparse arrays.
    """
    if hasattr(weight_mat, 'getrow'):
        row = weight_mat.getrow(row_idx)
    else:
        # csr_array has no getrow(); slicing keeps the result 2-dimensional.
        row = weight_mat[row_idx: row_idx + 1, :]
