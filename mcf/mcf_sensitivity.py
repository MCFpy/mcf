...
from mcf.mcf_estimation_generic import moving_avg_mean_var
from mcf import mcf_general as mcf_gp
from mcf import mcf_general_sys as mcf_sys
from mcf.mcf_init_sens import SensCfg
...
def sensitivity_main(mcf_: 'ModifiedCausalForest',
    ...
    sens_iate : Boolean (or None), optional
        ...
        is passed, and it contains IATEs, then the default value is True, and False otherwise.

    sens_iate_se : Boolean (or None), optional
        Compute Standard errors of IATEs for sensitivity analysis. Default is False.
    ...
    sens_replications : Integer (or None), optional.
        Number of replications for simulating placebo treatments. Default is 2.

    sens_reference_population: integer or float (or None)
        Defines the treatment status of the reference population used by
        the sensitivity analysis. Default is to use the treatment with most observed observations.
    ...
    mcf_.sens_cfg = SensCfg.from_args(mcf_.p_cfg,
    ...
    results_avg, plots_iate, txt_ate = sensitivity_analysis(
        mcf_, train_df, predict_df, mcf_.gen_cfg.with_output, iate_df=iate_df, seed=9345467,
        )
    ...
    if mcf_.gen_cfg.with_output:
        ...
        mcf_ps.print_timing(mcf_.gen_cfg, 'Sensitiviy analysis', time_string, time_difference,
                            summary=True
                            )
    return results_avg
...
def sensitivity_analysis(mcf_: 'ModifiedCausalForest',
    ...
    treat_probs_np, train_reduced_df, treat_name, treat_values = get_treat_probs_del_treat(
        mcf_, train_df.copy(), with_output
        )
    ...
    for scenario in mcf_.sens_cfg.scenarios:
        ...
        for repl in range(mcf_.sens_cfg.replications):
            ...
            train_placebo_df, d_probs = simulate_placebo(scenario, treat_probs_np, train_reduced_df,
                                                         treat_name=treat_name,
                                                         treat_values=treat_values, rng=rng,
                                                         )
            if with_output:
                d_probs_str = [str(round(p * 100, 2)) + '%' for p in d_probs]
                mcf_ps.print_mcf(mcf_.gen_cfg,
                                 f'\nSensitivity - Scenario: {scenario}, Replication: {repl+1}, '
                                 f'Simulated treatment shares: {", ".join(d_probs_str)}',
                                 summary=True)
    ...
    if with_output:
        for scenario in mcf_.sens_cfg.scenarios:
            ...
            if mcf_.sens_cfg.gate or mcf_.sens_cfg.bgate or mcf_.sens_cfg.cbgate:
                print_output_gate(mcf_repl, results_avg_dict, scenario)
            if mcf_.sens_cfg.iate:
                placebo_iate_plot = print_iate(mcf_repl, results_avg_dict, iate_df, scenario,)
...
def print_iate(mcf_: 'ModifiedCausalForest',
    ...
    if names_iate_se is not None:
        t_values_np, t_values_df = cluster_var_t_value(data_df, names_iate, names_iate_se,
        ...
        for cluster_no in post_cfg.kmeans_no_of_groups:
            cluster_lab_tmp = KMeans(n_clusters=cluster_no,
        ...
        cl_means = t_values_df.groupby(by=cluster_lab_np).mean(numeric_only=True)
...
def plot_iate(mcf_: 'ModifiedCausalForest',
    ...
    iate_sim = mcf_gp.to_numpy_big_data(data_df[name_iate], mcf_.int_cfg.obs_bigdata)
    iate_est = mcf_gp.to_numpy_big_data(iate_df[name_iate], mcf_.int_cfg.obs_bigdata)
    ...
    axe.plot(iate_est, np.zeros_like(iate_est), "black", label='_nolegend_', linestyle='--')
    axe.set_ylabel('Value of IATEs')
    axe.set_xlabel('Estimated IATEs (sorted)')
    axe.set_title(f'Placebo vs estimated IATEs: {name_iate}')
    axe.legend(loc=mcf_.int_cfg.legend_loc, shadow=True, fontsize=mcf_.int_cfg.fontsize)
    ...
    plt.show()
    plt.close()
    data_to_save = np.concatenate((iate_sim.reshape(-1, 1), iate_est.reshape(-1, 1)), axis=1)
    datasave = pd.DataFrame(data=data_to_save, columns=(name_iate+'_sim', name_iate+'_est'))
...
def print_output_gate(mcf_: 'ModifiedCausalForest', results_avg_dict: dict, scenario: str,) -> None:
    """Print output of sensitivity analysis for gate and bgate."""
    results_scen_dict = results_avg_dict[scenario]
    continuous = mcf_.gen_cfg.d_type == 'continuous'
    d_values = mcf_.ct_cfg.d_values_dr_np if continuous else mcf_.gen_cfg.d_values
                    ...
                    for o_idx, out_name in enumerate(mcf_.var_cfg.y_name):
                        ...
                        for a_idx in range(gate_z.shape[1]):
                            ...
                            if continuous and a_idx == 0:
                                mcf_ate.dose_response_figure(out_name, mcf_.var_cfg.d_name[0], est,
                                                             stderr,
                                                             d_values=d_values[1:],
                                                             int_cfg=mcf_.int_cfg, p_cfg=mcf_.p_cfg,
                                                             with_output=mcf_.gen_cfg.with_output,
                                                             )
            mcf_ps.print_mcf(mcf_.gen_cfg, txt, summary=True)


def print_output_ate(mcf_: 'ModifiedCausalForest', results_avg_dict: dict, scenario: str,) -> str:
    ...
    for o_idx, out_name in enumerate(mcf_.var_cfg.y_name):
        ...
        for a_idx in range(ate.shape[1]):
            if a_idx == 0:
                ...
                txt += f'   Reference population: Treatment group {d_values[a_idx-1]}'
            txt += '\n' + '- ' * 50
            est, stderr = ate[o_idx, a_idx], ate_se[o_idx, a_idx]
            t_val, p_val = get_t_val_pal(est, stderr)
            txt += mcf_ps.print_effect(est, stderr, t_val, p_val, effect_list,
                                       continuous=continuous, small_p_val_only=True
                                       )
            if continuous and a_idx == 0:
                mcf_ate.dose_response_figure(out_name, mcf_.var_cfg.d_name[0], est, stderr,
                                             d_values=d_values[1:], int_cfg=mcf_.int_cfg,
                                             p_cfg=mcf_.p_cfg, with_output=mcf_.gen_cfg.with_output,
                                             )
    mcf_ps.print_mcf(mcf_.gen_cfg, txt, summary=True)
...
def get_t_val_pal(est: NDArray[Any], stderr: NDArray[Any],) -> tuple[NDArray[Any], NDArray[Any]]:
...
def average_sens_results(mcf_: 'ModifiedCausalForest', results_all_dict: dict,) -> dict:
        ...
        for idx, result_repl_org in enumerate(results_all_dict[scenario]):
            ...
            if idx == 0:
                ...
                for statistic in result_repl:
                    if statistic in avg_inside_list and result_repl[statistic] is not None:
                        for list_idx, elem in enumerate(result_repl[statistic]):
                            results_dict_avg[scenario][statistic][list_idx] += elem
                            if final:
                                results_dict_avg[scenario][
                                    statistic][list_idx] /= no_of_replications
                    elif statistic in avg_numpy:   # Average numpy array
                        results_dict_avg[scenario][statistic] += result_repl[statistic]
                        if final:
                            results_dict_avg[scenario][statistic] /= no_of_replications
        if iate:
            results_dict_avg[scenario][iate_df] = average_iate(iate_df_list, id_name)

    return results_dict_avg


def average_iate(iate_df_list: list[pd.DataFrame], id_name: str,) -> pd.DataFrame:
...
def simulate_placebo(scenario: str,
...
def get_treat_probs_del_treat(mcf_: 'ModifiedCausalForest',
                              data_df: pd.DataFrame,
                              with_output: bool
    ...
    if mcf_copy.dc_cfg.screen_covariates:   # Only training
        (mcf_copy.gen_cfg, mcf_copy.var_cfg, mcf_copy.var_x_type, mcf_copy.var_x_values, _
    ...
    if with_output:
        mcf_ps.print_mcf(gen_cfg,
                         ...
                         f'treatment probabilities (by {sens_cfg.cv_k}-fold cross-validation)',
                         summary=True
                         )
    ...
    d_np = mcf_gp.to_numpy_big_data(data_df[d_name], int_cfg.obs_bigdata).ravel()
    if names_unordered:  # List is not empty
        x_df, dummy_names = mcf_data.dummies_for_unord(x_df, names_unordered,
                                                       data_train_dict=data_train_dic
                                                       )
    max_workers = 1 if int_cfg.replication else gen_cfg.mp_parallel
    classif = RandomForestClassifier(n_estimators=mcf_copy.cf_cfg.boot, max_features='sqrt',
    ...
    keep = np.isclose(d_np, group_value, atol=int_cfg.zero_tol, rtol=int_cfg.zero_tol).ravel()
    ...
    if with_output:
        ...
        txt += (f'Value {group_value}, # of observations: {obs_keep} '
                f'\nShare of observations kept: {obs_keep/obs:4.2%}'
                '\nSensivity analysis continues with observations kept only'
                + '\n' + '-' * 100
                )
