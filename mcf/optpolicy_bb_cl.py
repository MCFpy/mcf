...
def bps_classifier_allocation(optp_: 'OptimalPolicy',
    ...
    for alloc_name in alloc_name_bb:
        ...
        method, params, lables, score = mcf_est_g.best_classifier(x_dat_scaled_np, d_np, seed=seed,
                                                                  boot=1000, max_workers=None,
                                                                  test_share=0.25
                                                                  )
...
def bps_class_prediction_only(optp_: 'OptimalPolicy', data_df: DataFrame) -> tuple[DataFrame, str]:
    ...
    x_dat_scaled_np, _, _ = op_data.prepare_data_for_classifiers(
        data_df, optp_.var_cfg, scaler=scaler, x_name_train=optp_.bps_class_dict['x_name_train']
        )
