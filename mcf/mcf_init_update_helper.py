...
def int_update_train(mcf_: 'ModifiedCausalForest') -> None:
    ...
    int_cfg.mp_ray_shutdown = ray_shut_down(int_cfg.mp_ray_shutdown,cf_cfg.n_train_eff)
    ...
    if cf_cfg.n_train_eff < 20_000:
        ...
        min_obj_str_n_1 = ((cf_cfg.n_train_eff / 60_000) * (cf_cfg.boot / 1_000)
                           * (cf_cfg.m_grid * cf_cfg.n_min_grid * cf_cfg.alpha_reg_grid / 12)
                           * (120 * 1024 * 1024 * 1024) * 5
            )
        min_obj_str_n_2 = ((cf_cfg.n_train_eff / 60_000) * (cf_cfg.boot / 1_000)
                           * (120 * 1024 * 1024 * 1024)
                           )
        ...
        int_cfg.mem_object_store_1 = min(0.7 * memory.available, int_cfg.mem_object_store_1)
        int_cfg.mem_object_store_2 = min(0.5 * memory.available, int_cfg.mem_object_store_2)
    ...
    if (system() == 'Windows' and cf_cfg.n_train <= 250_000) or ray is None:
        int_cfg.mp_backend = 'joblib'
    else:
        int_cfg.mp_backend = 'ray'
...
def gen_update_train(mcf_: 'ModifiedCausalForest',
    ...
    d_dat = (data_df[var_cfg.d_name[0]].to_numpy()
             if mcf_.gen_tv_cfg.yes else data_df[var_cfg.d_name].to_numpy()
             )
    gen_cfg.d_values = np.unique(np.int32(np.round(d_dat))).tolist()
    gen_cfg.no_of_treat = len(gen_cfg.d_values)
    if len(data_df) > mcf_.int_cfg.obs_bigdata and gen_cfg.mp_parallel > 1:
        gen_cfg.mp_parallel = int(gen_cfg.mp_parallel * 0.75)

    mcf_.gen_cfg = gen_cfg


def gen_tv_update_train(mcf_: 'ModifiedCausalForest',
                        data_df: DataFrame
                        ) -> None:
    """Adjust cross-validation for number of training observations."""
    obs = len(data_df)
    if mcf_.gen_tv_cfg.cv_k is None:
        mcf_.gen_tv_cfg.cv_k = mcf_initvals.determine_cv_k(obs)
    if len(mcf_.gen_tv_cfg.penalize_version) == 1:
        mcf_.gen_tv_cfg.penalize_version *= mcf_.gen_cfg.no_of_treat
    if len(mcf_.gen_tv_cfg.penalize_version) != mcf_.gen_cfg.no_of_treat:
        raise ValueError('gen_tv_penalize_version must be a list or tuple of length equal to the '
                         'number of main treatments (or a single Boolean).'
                         )


def gen_tv_update_pred(mcf_: 'ModifiedCausalForest') -> None:
    """Recode treatment information to update dataclass."""
    gen_tv_cfg = mcf_.gen_tv_cfg
    d_all_values, no_of_treat_per_main = [], []
    main_vals = gen_tv_cfg.d_dict['main_treat']
    sub_vals = gen_tv_cfg.d_dict['sub_treat']
    for m_idx, m_val in enumerate(main_vals):
        if sub_vals[m_idx]:
            no_of_treat_per_main.append(len(sub_vals[m_idx]))
            for s_val in sub_vals[m_idx]:
                d_all_values.append((m_val, s_val,))
        else:
            d_all_values.append((m_val, 0,))
            no_of_treat_per_main.append(1)

    mcf_.gen_tv_cfg.d_all_values = np.asarray(d_all_values, dtype=np.int8)
    mcf_.gen_tv_cfg.no_of_treat_all = len(d_all_values)
    mcf_.gen_tv_cfg.no_of_treat_per_main = no_of_treat_per_main
    mcf_.gen_tv_cfg.d_all_values_one_label = joint_d_values(main_vals, sub_vals, as_string=False)
...
def var_update_train(mcf_: 'ModifiedCausalForest', data_df: DataFrame) -> None:
    ...
    if isinstance(d_name, (list, tuple)):
        for d_idx, d_val in enumerate(d_name):
            d_name[d_idx] = mcf_gp.adjust_var_name(d_val, data_df.columns.tolist())
    else:
        d_name = mcf_gp.adjust_var_name(d_name, data_df.columns.tolist())

    mcf_.var_cfg.d_name = d_name
...
def ct_update_train(mcf_: 'ModifiedCausalForest', data_df: DataFrame) -> None:
    ...
    if gen_cfg.d_type == 'continuous':
        ...
        (ct_cfg.w_to_dr_int_w01, ct_cfg.w_to_dr_int_w10,
         ...
         ct_cfg.d_values_dr_np) = interpol_weights(ct_cfg.grid_dr, ct_cfg.grid_w, ct_cfg.grid_w_val,
...
def lc_update_train(mcf_: 'ModifiedCausalForest', data_df):
    ...
    if mcf_.lc_cfg.cs_cv_k is None:
        mcf_.lc_cfg.cs_cv_k = mcf_initvals.determine_cv_k(obs)


def pa_ba_update_train(cv_k, obs):
    """Adjust folds for CV for number of training observations."""
    if cv_k is None:
        return mcf_initvals.determine_cv_k(obs)
...
def cs_update_train(mcf_: 'ModifiedCausalForest') -> None:
    ...
    if cs_cfg.adjust_limits < 0:
        raise ValueError('Negative common support adjustment factor is not possible.')
    mcf_.cs_cfg = cs_cfg
...
def cf_update_train(mcf_: 'ModifiedCausalForest', data_df: DataFrame) -> None:
    ...
    vcount = data_df.groupby(var_cfg.d_name[0]).size()  # pylint:disable=E1101
    obs_by_treat = vcount.to_numpy()
    if abs(n_train - obs_by_treat.sum()) > len(obs_by_treat):
        raise RuntimeError(f'Counting treatments does not work. n_d_sum:'
    ...
    if not isinstance(cf_cfg.chunks_maxsize, (int, float)) or cf_cfg.chunks_maxsize < 100:
        cf_cfg.baseline = 250_000 if mcf_.low_mem_cfg.yes else 100_000
        cf_cfg.chunks_maxsize = get_chunks_maxsize_forest(cf_cfg.baseline, cf_cfg.n_train,
    ...
    cf_cfg.subsample_share_eval = min(cf_cfg.subsample_share_forest * cf_cfg.subsample_factor_eval,
                                      1
                                      )
    n_d_subsam = (obs_by_treat_eff.min() * int_cfg.share_forest_sample
    ...
    if (cf_cfg.p_diff_penalty is None or cf_cfg.p_diff_penalty < 0):
        if cf_cfg.mtot == 4:
            ...
            if gen_cfg.d_type == 'discrete':
                cf_cfg.p_diff_penalty *= np.sqrt(gen_cfg.no_of_treat * (gen_cfg.no_of_treat - 1) / 2
                                                 )
        ...
        if cf_cfg.mtot == 4:
            ...
            if not 0 <= cf_cfg.p_diff_penalty <= 1:
                raise ValueError('Probability of using p-score > 1. Programm stopped.')
    ...
    if cf_cfg.n_min_min is None or cf_cfg.n_min_min < 1:
        cf_cfg.n_min_min = round(max((n_d_subsam**0.4) / 10, 1.5) * gen_cfg.no_of_treat)
    else:
        cf_cfg.n_min_min = round(cf_cfg.n_min_min)
    if cf_cfg.n_min_max is None or cf_cfg.n_min_max < 1:
        cf_cfg.n_min_max = round(max(n_d_subsam**0.5 / 10, 2) * gen_cfg.no_of_treat)
    else:
    ...
    if gen_cfg.d_type == 'discrete':
        if cf_cfg.n_min_treat is None or cf_cfg.n_min_treat < 1:
            cf_cfg.n_min_treat = round(max((cf_cfg.n_min_min + cf_cfg.n_min_max) / 2
        ...
        if cf_cfg.n_min_min < min_leaf_size:
            ...
            mcf_ps.print_mcf(gen_cfg, 'Minimum leaf size adjusted. Smallest ',
                             f' leafsize set to: {cf_cfg.n_min_min}', summary=True
                             )
    else:
    ...
    if cf_cfg.n_min_grid == 1:
        cf_cfg.n_min_min = cf_cfg.n_min_max = round((cf_cfg.n_min_min + cf_cfg.n_min_max) / 2)
        cf_cfg.n_min_values = cf_cfg.n_min_min
    else:
        if cf_cfg.n_min_grid == 2:
            n_min = np.hstack((cf_cfg.n_min_min, cf_cfg.n_min_max))
        else:
            n_min = np.linspace(cf_cfg.n_min_min, cf_cfg.n_min_max, cf_cfg.n_min_grid)
...
def low_mem_update_train(mcf_: 'ModifiedCausalForest', n_train: int) -> None:
    """Update low_memory configuration with size of training data."""
    if mcf_.low_mem_cfg.max_chunksize is None or mcf_.low_mem_cfg.max_chunksize < 1:
        max_size = 1_000
        min_size = 10
        base_level = 10_000
        if n_train > base_level:
            max_chunksize = max(max_size - (n_train - base_level)**0.5, min_size)
        else:
            max_chunksize = max_size

        mcf_.low_mem_cfg.max_chunksize = max_chunksize
...
def p_update_pred(mcf_: 'ModifiedCausalForest',
    ...
    if p_cfg.bgate_sample_share is None or p_cfg.bgate_sample_share <= 0:
        p_cfg.bgate_sample_share = (1 if n_pred < 1000
    ...
    if gen_cfg.d_type == 'continuous':
        ...
        if mcf_.gen_tv_cfg.yes:
            p_cfg.d_in_pred = (var_cfg.d_name[0] in data_df.columns
                               and var_cfg.d_name[1] in data_df.columns
                               )
        else:
            d_name = (var_cfg.d_name[0] if isinstance(var_cfg.d_name, (list, tuple))
                      else var_cfg.d_name
                      )
            p_cfg.d_in_pred = d_name in data_df.columns   # Update boolean
    if not p_cfg.d_in_pred:
        if p_cfg.atet or p_cfg.gatet:
            mcf_ps.print_mcf(gen_cfg,
                             'Treatment variable not in prediction data. ATET '
                             'and GATET cannot be computed.', summary=True
                             )
    ...
    if p_cfg.choice_based_sampling is True:
        if len(p_cfg.choice_based_probs) != gen_cfg.no_of_treat:
            raise ValueError('Choice based sampling. Rows in choice '
                             'probabilities do not correspond to number of treatments.'
                             )
        ...
        # Normalize
        p_cfg.choice_based_probs = mcf_initvals.p_cb_normalize(p_cfg.choice_based_probs,
                                                               gen_cfg.no_of_treat
                                                               )
    else:
...
def post_update_pred(mcf_: 'ModifiedCausalForest', data_df: DataFrame) -> None:
    ...
    if (post_cfg.kmeans_no_of_groups is None
        ...
        if middle < 7:
            post_cfg.kmeans_no_of_groups = [middle-2, middle-1, middle, middle+1, middle+2]
        else:
            post_cfg.kmeans_no_of_groups = [middle-4, middle-2, middle, middle+2, middle+4]
    else:
        if not isinstance(post_cfg.kmeans_no_of_groups, list):
            post_cfg.kmeans_no_of_groups = list(post_cfg.kmeans_no_of_groups)
            post_cfg.kmeans_no_of_groups = [round(a) for a in post_cfg.kmeans_no_of_groups]
...
def name_unique(all_names: list[str]) -> list[str]:
    ...
    seen = set()
    name_unique_ = [item for item in all_names if item not in seen and not seen.add(item)]
...
def get_ray_del_defaults(_mp_ray_del_user: Any) -> Any:
    ...
    if _mp_ray_del_user is None:
        ...
        if len(_mp_ray_del) > 2:
            raise ValueError(f'Too many parameters for _mp_ray_del{_mp_ray_del}')
        if not isinstance(_mp_ray_del, tuple):
            raise TypeError(f'mp_ray_del is no Tuple {_mp_ray_del}')
        if not all(i in possible_vals for i in _mp_ray_del):
            raise TypeError(f'Wrong values for _mp_ray_del {_mp_ray_del}')
...
def ray_shut_down(ray_shutdown: bool | None, len_data: int) -> bool:
    ...
    if not isinstance(ray_shutdown, bool):
        raise TypeError('mp_ray_shutdown must be either None or Boolean')
...
def ct_grid(user_grid: list[int | float], defaultgrid: list[int | float]) -> list[int | float]:
    """Help for initialisation."""
...
def interpol_weights(ct_grid_dr: float | int,
    ...
    for i, (val, val1) in enumerate(zip(ct_grid_w_val[:-1], ct_grid_w_val[1:])):
    ...
    if len(treat_val_np) != len(np.unique(treat_val_np)):
        raise ValueError('Continuous treatment needs higher precision (maybe treatment '
                         'is too discrete to be treated as continuous.'
                         )
...
def sub_size(n_train: int, share_mult: float, max_share: float | int) -> float:
    ...
    subsam_share = max(min(subsam_share, max_share), (2 * (n_train / 2)**0.5) / n_train)

    return subsam_share


def bootstrap(se_boot: int | None, cut_off: int, bnr: int, cluster_std: bool,) -> int | bool:
...
def get_chunks_maxsize_forest(base_level: int, obs: int, no_of_treat: int) -> int:
    """Compute optimal chunksize for forest splitting."""
    return round(base_level + (max(obs - base_level, 0) ** 0.8) / (no_of_treat - 1))


def var_helper(var_cfg: 'VarCfg') -> tuple[Any, Any, Any]:
